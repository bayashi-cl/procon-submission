...
void solve() {
    ...
    auto recon = [&](int l, int r, auto recon) -> int {
        if (l >= r) return -1;
        int root = pre[pre_top];
        ++pre_top;
        int mid = -1;
        for (int i = l; i < r; ++i) {
            if (in[i] == root) mid = i;
        }
        int left_child = recon(l, mid, recon);
        int right_child = recon(mid + 1, r, recon);
        child[root] = {left_child, right_child};
        return root;
    };

    int r = recon(0, n, recon);

    Vec post_order;
    auto dfs = [&](int now, auto dfs) {
        if (now == -1) return;
        auto [l, r] = child[now];
        dfs(l, dfs);
        dfs(r, dfs);
