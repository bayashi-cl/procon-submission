...
void solve() {
    auto n = input<int>();
    Vec prev(n, -1), sib(n, -1), deg(n), dep(n), hgt(n);
    vector<Pa> child(n);

    for (int i = 0; i < n; ++i) {
        auto [id, left, right] = input<int, 3>();
        child[id] = {left, right};
        if (left != -1) {
            prev[left] = id;
            deg[id]++;
    ...
    auto dfs = [&](int now, int depth, auto dfs) {
        if (now == -1) return -1;
        dep[now] = depth;
        int h1 = dfs(child[now].first, depth + 1, dfs);
        int h2 = dfs(child[now].second, depth + 1, dfs);
        int hight = max(h1, h2) + 1;
        hgt[now] = hight;
        return hight;
    };
    int hight = dfs(root, 0, dfs);
    hgt[root] = hight;
    // node id: parent = p, sibling = s, degree = deg, depth = dep, height = h,
    // type
