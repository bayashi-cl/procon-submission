...
int iSP(const Segment& s, const Point& c) { return iSP(s.p, s.q, c); }
int iSP(const Line& l, const Point& c) { return iSP(l.p, l.q, c); }
...
namespace bys {
void solve() {
    auto [p1, p2] = input<geo::Point, 2>();
    geo::Line l(p1, p2);
    auto q = input<int>();
    for (int i = 0; i < q; ++i) {
        auto p = input<geo::Point>();
