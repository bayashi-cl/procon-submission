...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        if (s[i] == '_') {
            ...
            down.pop_back();
            int p = i - d_pos;
            while ((!area.empty()) && (area.back().second > d_pos)) {
                p += area.back().first;
                area.pop_back();
            }
            area.push_back({p, d_pos});
        } else {
            down.push_back(i);
        }
    }
    Vec ans;
    for (auto [a, l] : area) ans.push_back(a);
    print(std::accumulate(std::begin(ans), std::end(ans), 0));
    cout << ans.size();
    if (ans.empty()) {
        cout << endl;
    } else {
