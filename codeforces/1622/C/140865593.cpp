...
void Solver::solve() {
    ...
    ll t = std::accumulate(a.begin(), a.end(), 0LL) - k;
    if (t <= 0) EXIT(0);
    if (n == 1) EXIT(t);
    ...
    ll mi = a.back();
    ll not_mi = std::distance(a.begin(), std::find(a.begin(), a.end(), mi));
    ll ans = t;
    auto ok = [&](ll x, int w) {
        if (x < 0) return false;
        ll re = t - x;
    ...
    for (ll i : Range(n - 1)) {
        ll m2 = meguru_bisect(iceil(t + 1, i + 1), -1LL, ok, i + 1);
        ll m1 = m2 == 0 ? min(i + 1, not_mi) : i + 1;
        chmin(ans, m1 + m2);
    }
    print(ans);
}
