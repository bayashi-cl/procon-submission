...
void Solver::solve() {
    ...
    auto ss = scanner.readvec<string>(n);
    set<string> wait2, wait3;
    DEBUG(n);
    for (auto&& s : ss) {
        DEBUG(s);
        int l = s.length();
        if (l == 1) {
            ...
            if (wait2.find(s) != wait2.end()) EXIT("YES");
        } else {
            wait2.insert(s.substr(0, 2));
            wait3.insert(s);
            reverse(s.begin(), s.end());
            if (wait3.find(s) != wait3.end()) EXIT("YES");
            if (wait3.find(s.substr(0, 2)) != wait3.end()) EXIT("YES");
        }
    }
    print("NO");
}
