...
void Solver::solve() {
    ...
    ll gcd_p = 0;
    for (auto pi : p) gcd_p = std::gcd(gcd_p, pi);
    ll gcd_q = 0;
    for (auto qi : q) gcd_q = std::gcd(gcd_q, qi);

    if (std::all_of(q.begin(), q.end(), [&](ll x) { return x % gcd_p != 0; })) {
        EXIT(gcd_p);
    }
    if (std::all_of(p.begin(), p.end(), [&](ll x) { return x % gcd_q != 0; })) {
        EXIT(gcd_q);
    }
    print(0);
}
