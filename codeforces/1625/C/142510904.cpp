...
namespace bys {
void Solver::solve() {
    auto [n, l, k] = input<int, ll, int>();
    auto d = input<ll>(n);
    d.push_back(l);
    vector<ll> dist;
    for (int i : Range(n)) dist.push_back(d[i + 1] - d[i]);
    auto a = input<ll>(n);
    ...
    for (int i : Range(n)) {
        for (int j : Range(k + 1)) {
            for (int m : Range(k + 1)) {
                if (i + m + 1 > n || j + m > k) break;
                chmin(dp[i + m + 1][j + m], dp[i][j] + (d[i + m + 1] - d[i]) * a[i]);
            }
        }
    }
    ll ans = LINF;
    for (auto&& t : dp[n]) chmin(ans, t);
    print(ans);
}
}  // namespace bys
