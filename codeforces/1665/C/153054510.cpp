...
void Solver::solve() {
    ...
    for (auto pi : p) child_cnt[pi]++;
    vector<int> parent;
    for (auto ci : child_cnt) {
        if (ci != 0) parent.push_back(ci);
    }
    sort(parent.begin(), parent.end(), std::greater<int>());
    int sz = parent.size();
    int ans = sz + 1;
    for (int i : irange(sz)) {
        parent[i] = max(0, parent[i] - (sz + 1 - i));
    }
    auto ok = [&](int x) {
        if (x < 0) return false;
        int cnt = 0;
        for (auto pi : parent) cnt += max(0, pi - x);
        return cnt <= x;
    };
    int ex = meguru_bisect(n, -1, ok);
    print(ans + ex);
}
}  // namespace bys
