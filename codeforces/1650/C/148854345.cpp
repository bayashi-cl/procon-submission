...
void Solver::solve() {
    ...
    vector<std::array<int, 3>> point;
    for (int i : irange(1, m + 1)) {
        auto [x, w] = scanner.read<int, 2>();
        point.push_back({x, w, i});
    }
    sort(point.begin(), point.end(), ItemSorter<array<int, 3>, 1>());
    int e = m - 2 * n;
    point.erase(point.end() - e, point.end());
    sort(point.begin(), point.end(), ItemSorter<array<int, 3>, 0>());
    int ans = 0;
    for (auto [x, w, i] : point) ans += w;
    print(ans);
    for (int i : irange(n)) {
        print(point[i][2], point[2 * n - i - 1][2]);
    }
    print();
}
}  // namespace bys
