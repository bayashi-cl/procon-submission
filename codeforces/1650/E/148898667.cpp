...
namespace bys {
void Solver::solve() {
    auto [n, d] = scanner.read<int, 2>();
    vector<int> a = {0};
    for (UV : irange(n)) a.emplace_back(scanner.read<int>());

    int id = -1;
    int min_diff = INF;
    for (int i : irange(n)) {
        int diff = a[i + 1] - a[i];
        if (chmin(min_diff, diff)) id = i;
    }
    vector<int> cand;
    if (id != 0) cand.push_back(id);
    cand.push_back(id + 1);
    int ans = 0;
    for (auto c : cand) {
        vector<int> aa;
        for (int i : irange(a.size())) {
            if (i != c) aa.push_back(a[i]);
        }
        int min_diff = INF, max_diff = -1;
        for (int i : irange(n - 1)) {
            int diff = aa[i + 1] - aa[i] - 1;
            chmin(min_diff, diff);
            chmax(max_diff, diff);
        }
        int interval = min(min_diff, max((int)ceildiv(max_diff, 2) - 1, d - aa.back() - 1));
        chmax(ans, interval);
    }
    print(ans);
}
