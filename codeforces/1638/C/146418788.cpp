...
#line 3 "/home/bayashi/dev/byslib/data/sparse_table.hpp"
namespace bys {
template <class A>
class SparseTable {
...
#line 2 "/home/bayashi/dev/byslib/math/algebra.hpp"
namespace bys {
template <class T>
struct Add {
    using value_type = T;
...
void Solver::solve() {
    ...
    vector<int> pos(n);
    for (int i : irange(n)) pos[p[i]] = i;
    SparseTable<Max<int>> st(pos);
    int ma = -1;
    int ans = 0;
    for (int i : irange(n)) {
        if (i > ma) ++ans;
        chmax(ma, st.query(0, p[i] + 1));
    }
    print(ans);
}
}  // namespace bys
