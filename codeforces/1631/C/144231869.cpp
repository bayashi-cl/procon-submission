...
namespace bys {
bool check(vector<Pa>& ans, int k) {
    int s = 0;
    for (auto&& [a, b] : ans) s += a & b;
    return s == k;
}
void Solver::solve() {
    ...
    if (k == 0) {
        ...
        ans.push_back({1, 3});
        ans.push_back({0, 3 ^ msk});
    } else {
        for (int i : Range(1, n / 2)) {
            if ((i == k) || ((i ^ msk) == k)) continue;
            ans.push_back({i, i ^ msk});
        }
        ans.push_back({0, k ^ msk});
        ans.push_back({msk, k});
    }
    for (auto&& e : ans) print(e);
    // DEBUG(IT, check(ans, k));
}
}  // namespace bys
