...
namespace bys {
void Solver::solve() {
    auto [n, m, k] = scanner.read<int, int, ll>();
    ...
    auto ok = [&](int i) -> bool {
        ...
        while (!que.empty()) {
            ...
            for (auto&& nxt : graph[top]) {
                if (a[nxt.src] <= th and a[nxt.dest] <= th) {
                    if (--deg[nxt.dest] == 0) que.push(nxt.dest);
                }
            }
        }
        if ((int)topo.size() != n) return true;
        ...
        for (auto now : topo) {
            for (auto&& nxt : graph[now]) {
                if (a[nxt.src] <= th and a[nxt.dest] <= th) {
                    chmax(dp[now], dp[nxt.dest]);
                }
            }
            dp[now]++;
            if (dp[now] >= k) return true;
        }
        return false;
    };

    auto res = meguru_bisect(n, -1, ok);
    print(res == n ? -1 : b[res]);
}
}  // namespace bys
