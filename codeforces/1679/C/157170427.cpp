...
void Solver::solve() {
    ...
    BinaryIndexedTree<int> row_bit(n), col_bit(n);
    for (UV : irange(q)) {
        ...
        if (t == 1) {
            ...
            auto [x1, y1] = scanner.read<Int1, 2>();
            auto [x2, y2] = scanner.read<int, 2>();
            bool low_flg = row_bit.sum(x1, x2) == x2 - x1;
            bool col_flg = col_bit.sum(y1, y2) == y2 - y1;

            print(low_flg or col_flg ? "Yes" : "No");
        }
    }
}
}  // namespace bys
