...
namespace bys {
bool is_beautiful(int x, int d) { return x % d == 0 && x % (d * d) != 0; }
void Solver::solve() {
    auto [x, d] = scanner.read<ll, 2>();
    vector<ll> good;
    for (auto div : make_divisor(x)) {
        if (is_beautiful(div, d)) good.push_back(div);
    }
    reverse(good.begin(), good.end());
    int sz = good.size();
    int cnt = 0;
    FixPoint dfs([&](auto&& self, ll prod, int crr) -> void {
        if (cnt >= 2) return;
        if (prod == x) {
            ++cnt;
            return;
