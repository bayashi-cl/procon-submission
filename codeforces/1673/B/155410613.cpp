...
void Solver::solve() {
    ...
    set<char> cs;
    for (auto si : s) cs.insert(si);
    vector<char> c(cs.begin(), cs.end());
    int csz = c.size();

    bool ans_flg = true;
    for (auto [i, j] : IndexCombinations<2>(csz)) {
        char u = c[i], v = c[j];
        bool last_u = false, last_v = false;
        for (auto si : s) {
            if (si == u) {
                if (last_u) ans_flg = false;
                last_u = true;
                last_v = false;
            } else if (si == v) {
                if (last_v) ans_flg = false;
                last_v = true;
                last_u = false;
            }
        }
    }
    print(ans_flg ? "YES" : "NO");
}
}  // namespace bys
