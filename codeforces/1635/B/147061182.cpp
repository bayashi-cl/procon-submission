...
void Solver::solve() {
    ...
    for (int i : irange(1, n - 1)) {
        if (a[i - 1] < a[i] && a[i] > a[i + 1]) lm.push_back({i, a[i]});
    }
    int sz = lm.size();
    lm.push_back({INF, INF});
    int ans = 0;
    for (int i = 0; i < sz; ++i) {
        ++ans;
        if (lm[i].first + 2 == lm[i + 1].first) {
            a[lm[i].first + 1] = max(lm[i].second, lm[i + 1].second);
            ++i;
        } else {
            a[lm[i].first] = max(a[lm[i].first - 1], a[lm[i].first + 1]);
        }
    }
    print(ans);
    print(a);
