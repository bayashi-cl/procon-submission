...
void Solver::solve() {
    ...
    auto ok = [&](int x) {
        ...
        for (int i : reversed(Range(2, n))) {
            if (hh[i].first + hh[i].second < x) return false;
            ll d = min(hh[i].first, hh[i].first + hh[i].second - x) / 3;
            hh[i - 1].second += d;
            hh[i - 2].second += 2 * d;
        }
        return hh[0].first + hh[0].second >= x &&
               hh[1].first + hh[1].second >= x;
    };
