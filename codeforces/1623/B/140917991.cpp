...
void Solver::solve() {
    auto n = input<int>();
    auto lr = input<Pa>(n);
    set<Pa> s(lr.begin(), lr.end());
    vector<pair<Pa, int>> lr_id, ans;
    for (int i = 0; i < n; ++i) {
        lr_id.push_back({lr[i], i});
        ans.push_back({lr[i], i});
    }

    sort(lr_id.begin(), lr_id.end(), [](pair<Pa, int>& a, pair<Pa, int>& b) {
        return a.first.second - a.first.first > b.first.second - b.first.first;
    });
    for (auto&& [ra, id] : lr_id) {
        s.erase(ra);
        int l_r = ra.first - 1;
        for (auto&& [l, r] : s) {
            if (l == ra.first) chmax(l_r, r);
        }
        ans[id].second = l_r + 1;
    }
