...
void solve() {
    auto n = input<string>();
    reverse(std::begin(n), std::end(n));
    int ans = INF;

    if (auto itr_0 = std::find(std::begin(n), std::end(n), '0');
        itr_0 != n.end()) {
        int sel_0 = std::distance(n.begin(), itr_0);
        // 00
        auto itr_00 = std::find(itr_0 + 1, std::end(n), '0');
        if (itr_00 != n.end()) {
            int sel_00 = std::distance(itr_0 + 1, itr_00);
            chmin(ans, sel_0 + sel_00);
        }
        // 50
        auto itr_05 = std::find(itr_0 + 1, std::end(n), '5');
        if (itr_05 != n.end()) {
            int sel_05 = std::distance(itr_0 + 1, itr_05);
            chmin(ans, sel_0 + sel_05);
        }
    }

    if (auto itr_5 = std::find(std::begin(n), std::end(n), '5');
        itr_5 != n.end()) {
        int sel_5 = std::distance(n.begin(), itr_5);
        // 25
        auto itr_52 = std::find(itr_5 + 1, std::end(n), '2');
        if (itr_52 != n.end()) {
            int sel_52 = std::distance(itr_5 + 1, itr_52);
            chmin(ans, sel_5 + sel_52);
        }
        // 75
        auto itr_57 = std::find(itr_5 + 1, std::end(n), '7');
        if (itr_57 != n.end()) {
            int sel_57 = std::distance(itr_5 + 1, itr_57);
            chmin(ans, sel_5 + sel_57);
        }
    }
    print(ans);
}
...
int main() {
    bys::init();
    auto t = bys::input<int>();
    for (int i = 0; i < t; ++i) bys::solve();
    return 0;
}
