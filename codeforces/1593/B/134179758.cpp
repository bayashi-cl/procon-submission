...
void solve() {
    auto n = input<string>();
    int ans = INF;
    auto search = [&](int start, char c) -> int {
        auto itr = std::find(n.rbegin() + start, n.rend(), c);
        if (itr == n.rend()) return -1;
        return std::distance(n.rbegin() + start, itr);
    };

    vector<string> cand = {"00", "25", "50", "75"};
    for (auto&& c : cand) {
        if (int res = search(0, c[1]); res != -1) {
            if (int res2 = search(res + 1, c[0]); res2 != -1) {
                chmin(ans, res + res2);
            }
        }
    }
