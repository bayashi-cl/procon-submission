...
#line 3 "/home/bayashi/dev/byslib/data/ordered_pair.hpp"
...
#line 4 "/home/bayashi/dev/byslib/graph/depth_first.hpp"
...
void Solver::solve() {
    ...
    if (std::find_if(deg.begin(), deg.end(), [](auto x) { return x >= 3; }) != deg.end()) EXIT(-1);
    auto itr = std::find(deg.begin(), deg.end(), 1);
    int leaf = std::distance(deg.begin(), itr);
    DepthFirstSearch dfs(graph, leaf);
    for (int i : Range(n - 1)) {
        int id = edge[{dfs.pre_order[i], dfs.pre_order[i + 1]}];
        ans[id] = i & 1 ? 2 : 3;
    }
    print(ans);
}
