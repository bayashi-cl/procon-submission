...
void Solver::solve() {
    ...
    for (int i : Range(n - 1)) {
        graph[as[i].second].push_back(as[i + 1].second);
        graph[bs[i].second].push_back(bs[i + 1].second);
    }
    std::queue<int> que;
    que.push(as.back().second);
    que.push(bs.back().second);

    string ans(n, '0');
    ans[as.back().second] = '1';
    ans[bs.back().second] = '1';
    while (!que.empty()) {
        int top = que.front();
        que.pop();
