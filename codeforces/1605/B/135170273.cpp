...
void solve() {
    ...
    VecVec ans;
    while (true) {
        auto itr_0 = std::find(s.rbegin(), s.rend(), '0');
        if (itr_0 == s.rend()) break;
        int last_0_idx = n - std::distance(s.rbegin(), itr_0) - 1;
        auto itr_1 = std::find(s.begin(), s.end(), '1');
        if (itr_1 == s.end()) break;
        int first_1_idx = std::distance(s.begin(), itr_1);
        if (last_0_idx < first_1_idx) break;

        vector<int> sw;
        vector<int> zero, one;
        for (int i = first_1_idx; i <= last_0_idx; ++i) {
            if (s[i] == '0') {
                zero.push_back(i);
            } else {
                one.push_back(i);
        ...
        for (int i = 0; i < (int)min(zero.size(), one.size()); ++i) {
            if (one[i] < zero[i]) {
                sw.push_back(one[i]);
                sw.push_back(zero[i]);
            } else {
