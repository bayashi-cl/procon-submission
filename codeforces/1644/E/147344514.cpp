...
void Solver::solve() {
    ...
    for (int i : irange(row.size())) row[i] = row.back() - row[i];
    for (int i : irange(col.size())) col[i] = col.back() - col[i];

    if (s[0] == 'R') {
        int first_d = std::find(s.begin(), s.end(), 'D') - s.begin();
        for (int i : irange(first_d)) col[i] = n - 1;
    } else {
        int first_r = std::find(s.begin(), s.end(), 'R') - s.begin();
        for (int i : irange(first_r)) row[i] = n - 1;
    }
    ll ans = n * n;
    for (auto&& e : row) ans -= e;
    for (auto&& e : col) ans -= e;
    print(ans);
}
}  // namespace bys
