...
void Solver::solve() {
    ...
    vector<int> m2 = {1, 4, 2, 6, 3, 5};
    vector<int> m3 = {1, 3, 5, 2, 6, 4, 7};
    int d = n / 4;
    int m = n % 4;
    vector<int> ans;
    for (int i : Range(d - 1)) {
    ...
    if (m == 0) {
        for (int e : base) ans.push_back(4 * (d - 1) + e);
    } else if (m == 1) {
        for (int e : base) ans.push_back(4 * (d - 1) + e);
        ans.push_back(n);
    } else if (m == 2) {
        for (int e : m2) ans.push_back(4 * (d - 1) + e);
    } else if (m == 3) {
        for (int e : m3) ans.push_back(4 * (d - 1) + e);
    }
    print(ans);
    // DEBUG(check(ans));
}
