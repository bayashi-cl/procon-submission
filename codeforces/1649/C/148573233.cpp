...
void Solver::solve() {
    ...
    auto c = scanner.readvec<int>(n, m);
    ll ans = 0;
    vector<pair<ll, ll>> col1(100001);
    for (ll j : irange(m)) {
        for (int i : irange(n)) {
            int k = c[i][j];
            ans += j * col1[k].first - col1[k].second;
            col1[k].first++;
            col1[k].second += j;
        }
    }
    vector<pair<ll, ll>> col2(100001);
    for (ll i : irange(n)) {
        for (int j : irange(m)) {
            int k = c[i][j];
            ans += i * col2[k].first - col2[k].second;
            col2[k].first++;
            col2[k].second += i;
        }
    }
