...
bool cross(Z a, Z b, Z c, Z d) {
    double d1 = ((b - a) * std::conj(c - a)).imag();
    double d2 = ((b - a) * std::conj(d - a)).imag();
    double d3 = ((d - c) * std::conj(a - c)).imag();
    double d4 = ((d - c) * std::conj(b - c)).imag();
    return (d1 * d2 < 0.0 && d3 * d4 < 0.0);
}

void solve() {
    auto [ax, ay, bx, by] = input<double, 4>();
    auto n = input<int>();
    Z a = Z(ax, ay);
    Z b = Z(bx, by);
    vector<Z> fig;
    for (int i = 0; i < n; ++i) {
        auto [x, y] = input<double, 2>();
        fig.push_back({x, y});
    }
    int cnt = 0;
    for (int i = 0; i < n - 1; ++i) {
        if (cross(a, b, fig[i], fig[i + 1])) cnt++;
    }
    if (cross(a, b, fig[n - 1], fig[0])) cnt++;
    print(cnt / 2 + 1);
}
}  // namespace bys
