...
void solve() {
    ...
    compress.push_back({a[0], 1, a[0]});
    for (int i = 1; i < n; ++i) {
        if (a[i] == a[i - 1]) {
            compress.push_back({a[i], 1, a[i]});
        } else {
            compress.back()[1]++;
            compress.back()[2] = a[i];
        }
    ...
    for (int i = 0; i < sz; ++i) {
        int cnt = compress[i][1];
        if (i != 0)
            if (compress[i][0] == compress[i - 1][2]) cnt += compress[i - 1][1];
        if (i != sz - 1)
            if (compress[i][2] == compress[i + 1][0]) cnt += compress[i + 1][1];
        chmax(ans, cnt);
    }
    print(ans);
}
