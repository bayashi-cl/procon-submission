...
void Solver::solve() {
    ...
    auto graph = read_adj_uv(n, m);
    auto k = scanner.read<int>();
    auto c = scanner.readvec<Int1>(k);

    vector dist(k, vector<ll>(k));
    for (auto [i, ci] : cenumerate(c)) {
        auto res = breadth_first_search(graph, ci);
        for (auto [j, to] : cenumerate(c)) {
            if (res.cost[to] == LINF) EXIT(-1);
            dist[i][j] = res.cost[to];
        }
    }

    vector dp(1 << k, vector<ll>(k, LINF));
    for (int s : irange(1, 1 << k)) {
        bool flg = __builtin_popcount(s) == 1;
        for (int f : irange(k)) {
            if (flg and pop(s, f)) dp[s][f] = 1;
            if (dp[s][f] == LINF) continue;
            for (int t : irange(k)) {
                chmin(dp[s | (1 << t)][t], dp[s][f] + dist[f][t]);
            }
        }
    }
    print(*std::min_element(dp.back().begin(), dp.back().end()));
}
}  // namespace bys
