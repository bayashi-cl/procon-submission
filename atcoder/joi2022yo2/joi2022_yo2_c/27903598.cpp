...
void Solver::solve() {
    ...
    for (int i : Range(1, h + 1)) {
        for (int j : Range(1, w + 1)) {
            int population = cs.sum(0, 0, i, j);
            bool flg = true;
            vector<int> row = {0}, col = {0};
            Pa f = {0, 0};
            for (int k : Range(1, h + 1)) {
                Pa t = {k, j};
                int p = cs.sum(f, t);
                if (p < population) {
                    continue;
            ...
            if (row.back() != h) continue;
            if (!flg) continue;
            f = {0, 0};
            for (int k : Range(1, w + 1)) {
                Pa t = {i, k};
                int p = cs.sum(f, t);
                if (p < population) {
                    continue;
            ...
            if (col.back() != w) continue;
            if (!flg) continue;

            for (int ii : Range(row.size() - 1)) {
                for (int jj : Range(col.size() - 1)) {
                    if (cs.sum(row[ii], col[jj], row[ii + 1], col[jj + 1]) !=
                        population) {
                        flg = false;
                    }
                }
