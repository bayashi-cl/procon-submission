...
struct Point {
    ...
    Point operator-(const Point& rh) const { return Point(x - rh.x, y - rh.y); }
    Point operator*(const ld rh) const { return Point(x * rh, y * rh); }
    Point operator/(const ld rh) const { return Point(x / rh, y / rh); }
    Point operator+=(const Point& rh) { x += rh.x; y += rh.y; return *this; }
    Point operator-=(const Point& rh) { x -= rh.x; y -= rh.y; return *this; }
    Point operator*=(const ld rh) { x *= rh; y *= rh; return *this; }
    Point operator/=(const ld rh) { x /= rh; y /= rh; return *this; }
    ...
    ld dot(const Point& rh) const { return x * rh.x + y * rh.y; }
    ld det(const Point& rh) const { return x * rh.y - y * rh.x; }
...
struct Segment : Line {
    ...
    ld length() const { return to_Point().norm(); }
    Point internal_division(ld m, ld n) const {
        return (p * n + q * m) / (m + n);
    }
...
ld angle(const Line& a, const Line& b) {
...
ld distance(const Segment& s, const Segment& t) {
