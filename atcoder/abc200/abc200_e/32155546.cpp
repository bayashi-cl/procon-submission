...
void Solver::solve() {
    ...
    k -= 1;
    vector dp(4, vector<ll>(3 * n + 1));
    dp[0][0] = 1;
    for (int i : irange(1, 4)) {
        auto cs = cumsum(dp[i - 1]);
        for (auto j : irange(1, 3 * n + 1)) {
            dp[i][j] = cs.fold(max(0, j - n), j);
        }
    }
    int sum = 0;
    for (; sum <= n * 3; ++sum) {
    ...
    for (int a : irange(1, n + 1)) {
        auto r = sum - a;
        auto b_min = max(r - n, 1);
        auto b_max = min(r - 1, n);
        auto b_cnt = max(b_max - b_min + 1, 0);
        if (k < b_cnt) {
            auto b = b_min + k;
            auto c = r - b;
            print(a, b, c);
