...
#line 5 "math_and_algorithm_bf/main.cpp"

namespace bys {
void Solver::solve() {
    auto n = scanner.read<int>();
    auto abc = scanner.readvec<array<ll, 3>>(n);
    ld ans = 0.0;
    for (int i : Range(n)) {
        for (int j : Range(i + 1, n)) {
            ...
            auto [p, q, r] = abc[j];
            if (a * q == p * b) continue;
            auto y = ld(p * c - r * a) / ld(p * b - q * a);
            auto x = ld(c - b * y) / (ld)a;

            bool flg = true;
            for (auto&& [s, t, u] : abc) {
                if (!isclose(s * x + t * y, (ld)u) && s * x + t * y > u) {
                    flg = false;
                    break;
                }
            }
            if (flg) chmax(ans, x + y);
        }
    }
    print(ans);
}
