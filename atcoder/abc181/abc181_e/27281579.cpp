...
void solve() {
    ...
    sort(w.begin(), w.end());
    auto dist = [&](ll x) {
        auto itr = std::lower_bound(w.begin(), w.end(), x);
        ll res = LINF;
        if (itr != w.end()) {
            chmin(res, abs(*itr - x));
        }
        if (itr != w.begin()) {
            chmin(res, abs(*(itr - 1) - x));
        }
        return res;
    };
    Vec left(n), right(n);
    for (int i = 0; i < n - 1; i += 2) {
        left[i + 1] = h[i + 1] - h[i];
        right[i + 2] = h[i + 2] - h[i + 1];
    }
    CumulativeSum cs_left(left), cs_right(right);
    cs_left.construct();
    cs_right.construct();
    ...
    for (int i = 0; i < n; ++i) {
        ll cost = dist(h[i]);
        if (i % 2 == 0) {
            cost += cs_left.sum(0, i) + cs_right.sum(i + 1, n);
        } else {
            cost += cs_left.sum(0, i - 1) + cs_right.sum(i + 2, n);
            cost += h[i + 1] - h[i - 1];
        }
        chmin(ans, cost);
    }
