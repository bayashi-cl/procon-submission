...
def main(**kwargs) -> None:
    s = [sinput() for _ in range(3)]
    cs = list(set(chain.from_iterable(s)))
    sz = len(cs)
    if sz > 10:
        print("UNSOLVABLE")
        return

    idx = [[cs.index(c) for c in si] for si in s]
    tops = [idx[i][0] for i in range(3)]

    for perm in permutations(range(10), sz):
        if any(perm[t] == 0 for t in tops):
            continue
        ...
        if all(ni > 0 for ni in n) and n[0] + n[1] == n[2]:
            print(*n, sep="\n")
            break
    else:
        print("UNSOLVABLE")
