...
void solve() {
    ...
    Vec imos(2402);
    for (int i = 0; i < n; ++i) {
        auto se = input<string>();
        int s = std::stoi(se.substr(0, 4));
        s -= s % 5;
        int e = std::stoi(se.substr(5, 4));
        e = (e + 4) / 5 * 5;
        if (e % 100 == 60) e = (e + 99) / 100 * 100;
        imos[s]++;
        imos[e]--;
    }
    ...
    bool rain = false;
    cout << std::setfill('0') << std::right;
    for (int i = 0; i < 2401; ++i) {
        if (imos[i]) {
            if (!rain) cout << std::setw(4) << i << '-';
            rain = true;
        } else {
            if (rain) cout << std::setw(4) << i << endl;
            rain = false;
        }
    }
}
