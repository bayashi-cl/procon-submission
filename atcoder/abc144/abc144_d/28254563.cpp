...
#line 3 "/home/bayashi/dev/byslib/geometry/base.hpp"
...
#line 3 "abc144_d/main.cpp"

namespace bys {
void Solver::solve() {
    auto [a, b, x] = input<int, 3>();
    int full = a * a * b;
    if (x * 2 >= full) {
        int space = full - x;
        ld k = ld(space * 2) / ld(a * a);
        print(geo::degree(std::atan2(k, a)));
    } else {
        ld k = ld(x * 2) / ld(a * b);
        print(geo::degree(std::atan2(b, k)));
    }
}
}  // namespace bys
