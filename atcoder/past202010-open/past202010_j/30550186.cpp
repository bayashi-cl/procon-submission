...
namespace bys {
void Solver::solve() {
    auto [n, m, ab, ac, bc] = scanner.read<int, 5>();
    auto s = scanner.read<string>();
    DynamicAdjacencyList graph(n + 3);
    for (UV : irange(m)) {
        auto [u, v, c] = scanner.read<Int1, Int1, ll>();
        graph.add_edge(u, v, c);
    ...
    int b = n + 1;
    int c = n + 2;
    for (auto [i, si] : enumerate(s)) {
        if (si == 'A') {
            graph.add_edge(i, a, 0);
            graph.add_edge(b, i, ab);
            graph.add_edge(c, i, ac);
        } else if (si == 'B') {
            graph.add_edge(a, i, ab);
            graph.add_edge(i, b, 0);
            graph.add_edge(c, i, bc);
        } else {  // si == 'C
            graph.add_edge(a, i, ac);
            graph.add_edge(b, i, bc);
            graph.add_edge(i, c, 0);
        }
    }
    print(dijkstra(graph, 0).cost[n - 1]);
}
}  // namespace bys
