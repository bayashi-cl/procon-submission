...
void solve() {
    auto n = input<int>();
    auto s = input<string>(5);

    VecVec cost(n, Vec(3, 5));
    ...
    VecVec dp(n + 1, Vec(3, INF));
    dp[0] = {0, 0, 0};
    for (int i = 1; i <= n; ++i) {
        for (int j = 0; j < 3; ++j) {
            for (int k = 0; k < 3; ++k) {
                if (j == k) continue;
                chmin(dp[i][j], dp[i - 1][k] + cost[i - 1][j]);
            }
        }
    }

    print(*std::min_element(std::begin(dp.back()), std::end(dp.back())));
}
}  // namespace bys
