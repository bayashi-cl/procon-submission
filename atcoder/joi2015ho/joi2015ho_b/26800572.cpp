...
void solve() {
    ...
    int pari = n % 2;
    // [l,r]
    vector memo(2 * n, vector<ll>(2 * n, -1LL));
    auto dp = [&](int l, int r, auto self) -> ll {
        ...
        } else if ((r - l + 1) % 2 == pari) {
            // JOI
            res = max(self(l, r - 1, self) + a[r % n],
                      self(l + 1, r, self) + a[l % n]);
        } else {
            // IOI
            if (a[l % n] > a[r % n]) {
                res = self(l + 1, r, self);
    ...
    ll ans = 0;
    for (int i = 0; i < n; ++i) chmax(ans, dp(i, i + n - 1, dp));
    print(ans);
}
}  // namespace bys
