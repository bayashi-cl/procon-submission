...
void solve() {
    auto [a, b, x, y] = input<int, 4>();
    Dijkstra::Adj adj(201);
    for (int i = 1; i <= 99; ++i) {
        adj[i].push_back({x, i + 100});
        adj[i + 100].push_back({x, i});

        adj[i + 1].push_back({x, i + 100});
        adj[i + 100].push_back({x, i + 1});

        adj[i].push_back({y, i + 1});
        adj[i + 1].push_back({y, i});
    }
    adj[100].push_back({x, 200});
    adj[200].push_back({x, 100});
    Dijkstra dij(adj);
    dij.search(a);
    print(dij.cost[b + 100]);
}
}  // namespace bys
