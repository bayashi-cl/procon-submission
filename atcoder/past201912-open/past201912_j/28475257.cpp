...
#line 6 "past201912_j/main.cpp"

namespace bys {
void Solver::solve() {
    auto [h, w] = input<int, 2>();
    auto a = input<int>(h, w);
    ...
    for (int i : Range(h)) {
        for (int j : Range(w)) {
            for (auto&& [ni, nj] : grid.next4(i, j)) {
                graph[grid.index(i, j)].push_back({grid.index(ni, nj), a[ni][nj]});
            }
        }
    }
    Dijkstra dij_dl(graph, grid.index(h - 1, 0));
    Dijkstra dij_dr(graph, grid.index(h - 1, w - 1));
    Dijkstra dij_ur(graph, grid.index(0, w - 1));

    ll ans = LINF;
    for (int i : Range(h)) {
        for (int j : Range(w)) {
            int idx = grid.index(i, j);
            ll cost = dij_dl.cost[idx] + dij_dr.cost[idx] + dij_ur.cost[idx] - 2 * a[i][j];
            chmin(ans, cost);
        }
    }
    print(ans);
