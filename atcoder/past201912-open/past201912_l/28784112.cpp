...
#line 7 "past201912_l/main.cpp"

namespace bys {
ld dist(pair<pair<ld, ld>, int>& a, pair<pair<ld, ld>, int>& b) {
    ld dx = a.first.first - b.first.first;
    ld dy = a.first.second - b.first.second;
    ld res = std::hypot(dx, dy);
    if (a.second != b.second) res *= 10;
    return res;
}

using Edge = pair<ld, pair<int, int>>;
void Solver::solve() {
    auto [n, m] = scanner.read<int, 2>();
    auto tower = scanner.readvec<pair<pair<ld, ld>, int>>(n + m);
    vector cost(n + m, vector<ld>(n + m));
    for (auto&& i : Range(n + m)) {
        for (int j : Range(n + m)) {
            cost[i][j] = dist(tower[i], tower[j]);
        }
    }
    ld ans = 1e9;
    for (int s : Range(1 << m)) {
        vector<Edge> graph;
        for (auto&& i : Range(n + m)) {
            if (i >= n && pop(s, i - n)) continue;
            for (int j : Range(n + m)) {
                if (j >= n && pop(s, j - n)) continue;
                graph.push_back({cost[i][j], {i, j}});
            }
        }
        ...
        ld build = 0;
        for (auto&& [c, p] : graph) {
            if (uft.same(p.first, p.second)) continue;
            uft.merge(p.first, p.second);
            build += c;
        }
        chmin(ans, build);
    }
