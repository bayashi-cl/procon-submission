...
void Solver::solve() {
    ...
    vector<int> v;
    for (int i : Range(0, n, 2)) v.push_back(c[i]);
    int m = v.size();
    for (int i : Range(1, n, 2)) v.push_back(c[i]);
    auto tr = [&](int x) {
        if (x % 2 == 0) {
            return x / 2;
        } else {
