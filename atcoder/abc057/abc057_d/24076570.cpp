...
void main() {
    auto [n, a, b] = input<int, 3>();
    auto v = input<ll>(n);
    sort(std::begin(v), std::end(v), std::greater<ll>());

    map<ll, int, std::greater<ll>> shop, buy;
    ll sm = 0;
    for (auto&& vi : v) shop[vi]++;
    for (int i = 0; i < a; i++) {
        buy[v[i]]++;
        sm += v[i];
    }

    double mean = (double)sm / a;
    ll way = 1;

    if (buy.size() == 1) {
        way = 0;
        for (auto&& i : irange(a, min(b, shop[v[0]]) + 1)) {
            way += comb(shop[v[0]], i);
        }
    } else {
        for (auto [key, value] : buy) {
            way *= comb(shop[key], buy[key]);
        }
    }
