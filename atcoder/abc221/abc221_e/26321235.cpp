...
void solve() {
    ...
    Compress cp(a);
    cp.construct();
    for (auto&& e : a) e = cp.get(e);

    atcoder::fenwick_tree<mint> bit(cp.cp.size());
    mint ans = 0;
    for (int i = 0; i < n; ++i) {
        ans += bit.sum(0, a[i] + 1) * mint(2).pow(i);
        bit.add(a[i], mint(2).pow(i + 1).inv());
    }
    print(ans);
}
}  // namespace bys
