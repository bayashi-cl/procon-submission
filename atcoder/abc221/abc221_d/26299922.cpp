...
void solve() {
    ...
    std::partial_sum(std::begin(imos), std::end(imos), imos.begin());

    for (int i = 0; i < s - 1; ++i) {
        ans[imos[i]] += cp.resume(i + 1) - cp.resume(i);
    }
    Vec ans1(std::next(ans.begin()), ans.end());
    print(ans1);
}
}  // namespace bys
