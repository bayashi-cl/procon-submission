...
VecVec rotate(VecVec& mat) {
    int row = mat.size();
    int col = mat[0].size();
    VecVec res(col, Vec(row));
    for (int i = 0; i < row; ++i) {
        for (int j = 0; j < col; ++j) {
            res[j][row - i - 1] = mat[i][j];
        }
    }
    return res;
}

VecVec bounding_box(VecVec& mat) {
    int row = mat.size();
    int col = mat[0].size();
    int left = INF, up = INF;
    int right = 0, down = 0;
    ...
    if (left == INF) return VecVec();
    VecVec res(down - up + 1, Vec(right - left + 1));
...
void solve() {
    ...
    auto bb_s = bounding_box(s);
    for (int i = 0; i < 4; ++i) {
        if (bb_s == bounding_box(t)) {
            print("Yes");
            return;
        }
        t = rotate(t);
