...
void Solver::solve() {
    ...
    map<int, vector<Pa>> cond;

    for (UV : irange(m)) {
        auto [x, y, z] = scanner.read<int, Int1, int>();
    ...
    for (int s : irange(1 << n)) {
        ...
        auto flg = true;
        if (auto itr = cond.find(p); itr != cond.end()) {
            for (auto [y, z] : itr->second) {
                if (cnt[y] > z) flg = false;
            }
        }
        if (flg) {
            for (auto d : irange(n)) {
                if (pop(s, d)) dp[s] += dp[s - (1 << d)];
            }
        }
    }
    print(dp.back());
...
int main() {
    bys::Solver().solve(/* bys::scanner.read<int>() */);
    return 0;
}
