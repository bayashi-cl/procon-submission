...
void init() {cin.tie(nullptr); std::ios::sync_with_stdio(false); cout << std::fixed << std::setprecision(11); std::clog << std::boolalpha;}
// clang-format on
}  // namespace bys
...
void solve() {
    ...
    debug(std::accumulate(std::begin(ans), std::end(ans), 0LL) == ac);
}
}  // namespace bys
