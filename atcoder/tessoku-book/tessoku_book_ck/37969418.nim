static:
  when not defined(lazyCompile):
    ...
    writeModule("byslib/algo/bisect.nim"):
      import sugar
      import ../core/utils
      
      proc meguru_bisect*(ok, ng: int, check: (int) -> bool): int =
        assert check(ok)
        assert not check(ng)
      ...
      proc bisect_float*(ok, ng: float, check: (float) -> bool, rep = 100): float =
        assert check(ok)
        assert not check(ng)
...
include byslib/core
import byslib/algo/bisect

proc main =
  let n = read(float)
  print(bisect_float(0.0, n, x => x^3+x <= n))
