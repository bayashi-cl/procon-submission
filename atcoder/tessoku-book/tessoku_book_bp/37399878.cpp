...
template <class E> auto ford_fulkerson(EdgesCOO<E> const& edges, typename E::vertex_type s, typename E::vertex_type t) {
    ...
    while (true) {
        ...
        auto incl = dfs(s, get_inf<weight_type>());
        if (incl == 0) break;
        max_flow += incl;
    }
    return max_flow;
...
void Solver::solve() {
    ...
    EdgesCOO<Edge<i32, i32>> edges(n, directed);
    for (UV : irange(m)) {
        auto [a, b, c] = scanner.read<i32_1, i32_1, i32>();
        edges.add_edge(a, b, c);
    }

    print(ford_fulkerson(edges, 0, n - 1));
}
}  // namespace bys
