...
void Solver::solve() {
    ...
    auto man = scanner.readvec<array<i32_1, 3>>(m);

    auto sz = 1 << n;
    auto s = 0;
    for (auto [i, ai] : enumerate(a)) {
        if (ai == 1) s |= 1 << i;
    }
    EdgeList edges(sz, directed);
    for (auto f : irange(sz)) {
        for (auto [x, y, z] : man) {
            auto t = f ^ ((1 << x) | (1 << y) | (1 << z));
            edges.add_edge(f, t);
        }
    }
    auto ans = bfs(edges.build(), s)[sz - 1];
