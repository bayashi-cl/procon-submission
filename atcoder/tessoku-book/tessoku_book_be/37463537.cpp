...
namespace bys {
template <class QueryType, class NodeType = int> class Doubling {
    static_assert(std::is_integral_v<QueryType>, "Query type must be integral.");
    constexpr static int digits = std::numeric_limits<QueryType>::digits;

    const std::size_t n;
    std::vector<std::vector<NodeType>> db;

  public:
    Doubling(std::vector<NodeType> const& v) : n(v.size()), db(digits, std::vector<NodeType>(n)) {
        db[0] = v;
    ...
    NodeType query(NodeType from, QueryType x) {
        assert(0 <= from and from < (NodeType)n);
        NodeType res = from;
...
void Solver::solve() {
    ...
    auto a = scanner.readvec<i32_1>(n);
    Doubling<i32> db(a);
    for (UV : irange(q)) {
        auto [x, y] = scanner.read<i32_1, int>();
        print(db.query(x, y) + 1);
    }
}
}  // namespace bys
