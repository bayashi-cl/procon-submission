...
proc lisIndices*[T](s: openArray[T]): seq[int] =
  var
    dp = newSeq[T]()
    restore = newSeq[int](s.len)
  for i, si in s:
    let idx = dp.lowerBound(si)
    restore[i] = idx
    if idx == dp.len:
      dp.add(si)
    else:
  ...
  var now = dp.high
  result.setLen(dp.len)
  for i in countdown(s.high, 0, 1):
    if restore[i] == now:
      result[now] = i
      now.dec

template lis*[T](s: openArray[T]): auto = lisIndices(s).len

proc main =

  let n = read(int)
  var box = readSeq(n, tuple[x, y: int])
  let ans = box.sortedByIt((it.x, -it.y)).mapIt(it.y).lis()
  print ans
