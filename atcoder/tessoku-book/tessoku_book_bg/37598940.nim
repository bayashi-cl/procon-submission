...
type Monoid* = object of RootObj

type AddMonoid*[T] = object of Monoid
template sType*[M: AddMonoid](cls: typedesc[M]): auto = M.T
template id*[M: AddMonoid](cls: typedesc[M]): auto = (M.sType)0
template op*[M: AddMonoid](cls: typedesc[M]; a, b: M.sType): auto = a + b


type SegTree[M] = ref object
  n, leaf: int
  data: seq[int]

proc initSegTree*[M: Monoid](n, v: int): auto =
  let leaf = bit_ceil(n)
  var data = newSeqWith(leaf * 2, M.id)
  for i in leaf..<leaf + n:
    data[i] = v
  for i in countdown(n - 1, 1):
    data[i] = M.op(data[i * 2], data[i * 2 + 1])

  return SegTree[M](n: n, leaf: leaf, data: data)
...
proc update*(self: var SegTree; index, v: int) =
  ...
  while i > 1:
    i = i shr 1
    self.data[i] = self.M.op(self.data[i * 2], self.data[i * 2 + 1])
...
proc fold*(self: SegTree; l, r: var int): int =
  ...
  while l < r:
    if l.testBit(0):
      lfold = self.M.op(lfold, self.data[l])
      l.inc
    if r.testBit(0):
      r.dec
      rfold = self.M.op(self.data[r], rfold)

    l = l shr 1
    r = r shr 1

  return self.M.op(lfold, rfold)

proc main =
  let n, q = read(int)
  var seg = initSegTree[AddMonoid[int]](n, 0)
  q.times:
    let t = read(int)
    case t
    of 1:
