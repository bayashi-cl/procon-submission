...
type AddMonoid*[T] = object of Monoid
  v*: T
template id*[T](cls: typedesc[AddMonoid[T]]): auto = AddMonoid[T](v: 0)
template `+`*[T](a, b: AddMonoid[T]): auto = AddMonoid[T](v: a.v + b.v)
...
proc initSegTree*[M: Monoid](n: int; v: M.T): SegTree[M] =
  let leaf = bit_ceil(n)
...
proc fold*(self: SegTree; l, r: var int): auto =
  var
    lfold = self.M.id
    rfold = self.M.id
  l += self.leaf
  ...
  while l < r:
    if l.testBit(0):
      lfold = lfold + self.data[l]
      l.inc
    if r.testBit(0):
      r.dec
      rfold = self.data[r] + rfold

    l = l shr 1
    r = r shr 1

  return (lfold + rfold).v
