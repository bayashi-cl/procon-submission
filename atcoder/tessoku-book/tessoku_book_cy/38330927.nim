static:
  when not defined(lazyCompile):
    ...
    writeModule("byslib/ntheory/sieve.nim"):
      import sequtils, math
      
      type Sieve = ref object
        spf: seq[int]
      
      proc initSieve*(m: int): Sieve =
        assert m >= 0
      
        var spf = (0..m).toSeq
        for i in countup(2, m, 2):
          spf[i] = 2
      
        var p = 3
        while p^2 <= m:
          if spf[p] == p:
            for q in countup(p^2, m, p):
              if spf[q] == q:
                spf[q] = p
          p += 2
      
        return Sieve(spf: spf)
      
      proc isPrime*(self: Sieve, x: int): bool =
        if x <= 1: return false
        return self.spf[x] == x
      
      iterator enumPrime*(self: Sieve, x: int = -1): auto =
        let m =
          if x == -1: self.spf.high
          else: x
        for p in 0..m:
          if self.isPrime(p):
            yield p
...
include byslib/core
import byslib/ntheory/sieve

proc main =
  let sieve = read(int).initSieve
  for p in sieve.enumPrime:
    print p
