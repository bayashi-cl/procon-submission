...
proc main =
  let
    n, w = read(int)
    goods = readSeq(n, tuple[w, v: int])
    s = goods.mapIt(it.v).sum

  var dp = newSeqWith(n + 1, newSeqWith(s + 1, IInf)) # 価値jを達成する最小の重み
  dp[0][0] = 0
  for i in 0..<n:
    for j in 0..s:
      dp[i + 1][j].chmin(dp[i][j])
      if j + goods[i].v <= s:
        dp[i + 1][j + goods[i].v].chmin(dp[i][j] + goods[i].w)
