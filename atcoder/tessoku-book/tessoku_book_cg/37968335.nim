static:
  when not defined(lazyCompile):
    ...
    writeModule("byslib/ds/cumsum.nim"):
      import math
      import sequtils
      
      type CumSum*[T] = ref object
        data: seq[T]
      
      proc initCumSum*[T](v: openArray[T]): CumSum[T] =
        var s = newSeq[T](v.len + 1)
        s[1..s.high] = v
        s.cumsum()
        return CumSum[T](data: s)
      
      proc fold*[T](self: CumSum[T]; idx: Slice[int]): T =
        return self.data[idx.b+1] - self.data[idx.a]
      
      type CumSum2D*[T] = ref object
        data: seq[seq[T]]
      
      proc initCumSum2D*[T](v: seq[seq[T]]): CumSum2D[T] =
        let
          n = v.len
          m = v[0].len
        var s = newSeqWith(n + 1, newSeq[T](m + 1))
      
        for i in 1..n:
          assert v[i - 1].len == m
          for j in 1..m:
            s[i][j] = v[i - 1][j - 1] + s[i][j - 1] + s[i - 1][j] - s[i - 1][j - 1]
      
        return CumSum2D[T](data: s)
      
      proc fold*[T](self: CumSum2D[T]; i, j: Slice[int]): T =
        return self.data[i.b+1][j.b+1] - self.data[i.a][j.b+1] - self.data[i.b+1][j.a] + self.data[i.a][j.a]
...
include byslib/core
import byslib/ds/cumsum

proc main =
  let
    n = read(int)
    pos = readSeq(n, tuple[x, y: int])
    q = read(int)

  var v = newSeqWith(1501, newSeq[int](1501))
  for p in pos: v[p.x][p.y] += 1

  let cs = initCumSum2D[int](v)
  q.times:
    let a, b, c, d = read(int)
    print(cs.fold(a..c, b..d))
