...
namespace bys {
template <class... Args> auto make_optuple(Args&&... args);

template <class... Args> class operative_tuple : public std::tuple<Args...> {
  public:
    using std::tuple<Args...>::tuple;

  private:
    template <class T, std::size_t... I> void fill(T const& v, std::index_sequence<I...>) noexcept {
        ((std::get<I>(*this) = v), ...);
    }
...
#define DEFINE_BINOP(op, iop, name)                                                                           \
  private:                                                                                                    \
    template <std::size_t... I> void name(operative_tuple const& other, std::index_sequence<I...>) noexcept { \
...
#define DEFINE_MONOOP(op, name)                                                                                                  \
  private:                                                                                                                       \
    template <std::size_t... I> auto name(std::index_sequence<I...>) noexcept { return make_optuple(op std::get<I>(*this)...); } \
                                                                                                                                 \
  public:                                                                                                                        \
...
#define DEFINE_INCL(op, name)                                                                                   \
  private:                                                                                                      \
    template <std::size_t... I> void name(std::index_sequence<I...>) noexcept { (op std::get<I>(*this), ...); } \
                                                                                                                \
  public:                                                                                                       \
...
#define DEFINE_BITSHIFT(op, iop, name)                                                         \
  private:                                                                                     \
    template <std::size_t... I> void name(std::size_t x, std::index_sequence<I...>) noexcept { \
...
template <class... Args> operative_tuple(Args...) -> operative_tuple<Args...>;

template <class... Args> using optuple = operative_tuple<Args...>;

template <class... Args> auto make_optuple(Args&&... args) { return operative_tuple<Args...>(std::forward<Args>(args)...); }
}  // namespace bys

template <class... Types>
class std::tuple_size<bys::operative_tuple<Types...>> : public std::integral_constant<std::size_t, sizeof...(Types)> {};

template <std::size_t I, class... Types> struct std::tuple_element<I, bys::operative_tuple<Types...>> {
...
namespace bys {
void Solver::solve() {
    using W = operative_tuple<int, int>;
    using E = Edge<W>;
    auto [n, m] = scanner.read<i32, 2>();
    EdgesCOO<E> edges(n, undirected);

    for (UV : irange(m)) {
        auto [a, b] = scanner.read<i32_1, 2>();
        auto [c, d] = scanner.read<i32, 2>();
        edges.add_edge(a, b, W(c, -d));
    }
    auto [d, t] = dijkstra(edges.build(), 0).back();
    print(d, -t);
}
}  // namespace bys
