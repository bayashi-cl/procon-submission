...
void Solver::solve() {
    ...
    // DEBUG(base);
    i32 ans = 0;
    for (auto r : irange(min(h, k) + 1)) {
        for (auto perm : Combinations(irange(h).to_vec(), r)) {
            i32 b = 0;
            vector<i32> cnt(w);
            set perm_s(perm.begin(), perm.end());
            for (auto i : irange(h)) {
                if (perm_s.find(i) == perm_s.end()) {
                    for (auto j : irange(w)) {
                        if (c[i][j] == '#') ++cnt[j];
                    }
                } else {
            ...
            for (auto ki : irange(k - r)) cnt[ki] = h - r;
            b += std::reduce(cnt.begin(), cnt.end());
            chmax(ans, b);
        }
    }
    print(ans);
