...
namespace bys {
template <class V, class W> struct FlowEdge : EdgeBase {
    using vertex_type = V;
    using weight_type = W;
    vertex_type src, dest;
    weight_type weight;
    ...
    FlowEdge() = default;
    FlowEdge(vertex_type s, vertex_type d, weight_type w, i32 r) : src(s), dest(d), weight(w), rev_id(r) {}
};
template <class E> auto ford_fulkerson(EdgesCOO<E> const& edges, typename E::vertex_type s, typename E::vertex_type t) {
    using weight_type = typename E::weight_type;
    using vertex_type = typename E::vertex_type;
    static_assert(std::is_integral_v<weight_type>, "cap must be integral.");
    using F = FlowEdge<vertex_type, weight_type>;
    EdgesCOO<F> flow_edges(edges.size(), directed);

    for (auto&& e : edges) {
        i32 src_id = flow_edges.count(e.src);
        i32 dest_id = flow_edges.count(e.dest);

        flow_edges.add(F(e.src, e.dest, e.weight, dest_id));
        flow_edges.add(F(e.dest, e.src, 0, src_id));
    }
    auto graph = flow_edges.build();

    std::vector<bool> seen(edges.ssize());
    std::vector<i32> hist;
    FixPoint dfs([&](auto&& self, vertex_type now, weight_type flow) -> weight_type {
        seen[now] = true;
        hist.push_back(now);

        if (now == t) return flow;
        for (auto&& e : graph[now]) {
            if (e.weight == 0 or seen[e.dest]) continue;
            auto f = self(e.dest, std::min(flow, e.weight));
            if (f > 0) {
                e.weight -= f;
                graph[e.dest][e.rev_id].weight += f;
                return f;
            }
        }
        return 0;
    });
    weight_type max_flow = 0;
    while (true) {
        for (auto h : hist) seen[h] = false;
        hist.clear();
        auto inc = dfs(s, get_inf<weight_type>());
        if (inc == 0) break;
        max_flow += inc;
    }
    return max_flow;
...
void Solver::solve() {
    ...
    EdgeList edges(2 * n + 2, directed);

    i32 source = 2 * n, sink = 2 * n + 1;

    for (auto i : irange(n)) {
        edges.add_edge(source, i, 1);
        edges.add_edge(i + n, sink, 1);

        for (auto j : irange(n)) {
            if (s[i][j] == '#') {
                edges.add_edge(i, j + n, 1);
            }
        }
    }

    print(ford_fulkerson(edges, source, sink));
}
}  // namespace bys
