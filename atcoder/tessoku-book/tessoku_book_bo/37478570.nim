static:
  when not defined(lazyCompile):
    ...
    writeModule("byslib/ds/unionfind.nim"):
      ...
      proc union*(self: var UnionFindTree, a, b: int): int =
        var
...
include byslib/core
include byslib/ds/unionfind

type Edge* = object
  src, dest, weight: int

proc `<`*(a, b: Edge): bool = a.weight < b.weight
proc initEdge*(s, d: int, w: int = 1): Edge = Edge(src: s, dest: d, weight: w)
...
proc main =
  ...
  var
    ...
    ans = 0
  for e in edges:
    if not uf.same(e.src, e.dest):
      ans += e.weight
      discard uf.union(e.src, e.dest)
