...
namespace bys {
int z2n(int n) { return n * (n + 1) / 2; }
int calc(int n, int l, int r, int pos) {
    int res;
    // [l, r)
    if (pos < l) {
        res = z2n(r - pos - 1) - z2n(l - pos - 1);
    } else if (l <= pos && pos < r) {
        res = z2n(pos - l) + z2n(r - pos - 1);
    } else if (r <= pos) {
        res = z2n(pos - l) - z2n(pos - r);
    } else {
        assert(false);
    }
    return res;
...
void solve() {
    auto [r, g, b] = input<int, 3>();
    int ans = INF;
    for (int i = -400; i <= 400; ++i) {
        int cnt = 0;

        // red
        int red_r = min(i, -100 + (r + 1) / 2);
        int red_l = red_r - r;
        cnt += calc(r, red_l, red_r, -100);

        // green
        cnt += calc(g, i, i + g, 0);

        // blue
        int blue_l = max(i + g, 100 - (b - 1) / 2);
        int blue_r = blue_l + b;
        cnt += calc(b, blue_l, blue_r, 100);
