...
namespace bys {
void Solver::solve() {
    auto [n, l, r] = scanner.read<int, ll, ll>();
    auto a = scanner.readvec<ll>(n);

    auto cs = cumsum(a);
    vector<ll> mi(n);
    for (int i : irange(n)) {
        mi[i] = min(0LL, l * (i + 1) - cs.prefix_fold(i + 1));
    }
    CumulativeSum<Min<ll>> cm(mi);

    auto ans = min({l * n, r * n, std::accumulate(a.begin(), a.end(), 0LL)});
    for (int i : irange(1, n + 1)) {
        chmin(ans, cs.prefix_fold(i) + cm.prefix_fold(i) + r * (n - i));
    }
    print(ans);
}
}  // namespace bys
