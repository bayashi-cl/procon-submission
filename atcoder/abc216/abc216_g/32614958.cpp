...
void Solver::solve() {
    ...
    auto span = scanner.readvec<array<int, 3>>(m);
    sort(span.begin(), span.end(), ItemSorter<1>());

    BinaryIndexedTree<Add<int>> bit(n);
    for (auto [l, r, x] : span) {
        --l;
        x = max(x - bit.fold(l, r), 0);
        for (int i : reversed(irange(l, r))) {
            if (x == 0) break;
            // DEBUG(1, i);
            // DEBUG(2, bit.fold(i, i + 1));
            // DEBUG(3, bit[i]);
            if (bit[i] == 0) {
    ...
    // vector<int> ans(n);
    for (auto i : irange(n)) cout << bit[i] << ' ';
    cout << '\n';
    // print(ans);
}
}  // namespace bys
