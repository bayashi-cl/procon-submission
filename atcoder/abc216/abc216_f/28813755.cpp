...
void Solver::solve() {
    ...
    vector<Pa> ab(n);
    for (int i : Range(n)) ab[i] = {a[i], b[i]};
    sort(ab.begin(), ab.end());

    vector dp(n + 1, vector<mint>(mx + 1));
    dp[0][0] = 1;
    vector<mint> sm(mx + 1);
    sm[0] = 1;
    for (int i : Range(1, n + 1)) {
        auto [ai, bi] = ab[i - 1];
        for (int j : Range(mx + 1)) {
            if (j - bi >= 0) dp[i][j] = sm[j - bi];
        }
        for (int j : Range(mx + 1)) {
            sm[j] += dp[i][j];
        }
    ...
    mint ans = 0;
    for (int i : Range(1, n + 1)) {
        for (int j : Range(ab[i - 1].first + 1)) {
            ans += dp[i][j];
        }
    }
    // for (int i : Range(1, ab.back().first + 1)) ans += dp[n][i];
    print(ans);
}
}  // namespace bys
