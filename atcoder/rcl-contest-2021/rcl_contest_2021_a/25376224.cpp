...
void solve() {
    ...
    auto a = input<int>(n);
    // vector<Pa> a;
    // for (int i = 0; i < n; ++i) a.push_back({input<int>(), i});
    // sort(std::begin(a), std::end(a), std::greater<Pa>());

    auto score = [&](Vec& ans, int magi) {
        double sm = 0;
        int cnt = 0;
        double lk = log2(k);
        for (auto&& ai : ans) {
    ...
    while (std::clock() - ti < 190 * CLOCKS_PER_SEC / 100) {
        ...
        int j = std::rand() % n;
        Vec a_copy(a);
        a_copy[i] = (a_copy[i] + a_copy[j]) % k;
        if (chmax(s, score(a_copy, cnt + 1))) {
            print(i, j);
            ++cnt;
            a = a_copy;
        }
