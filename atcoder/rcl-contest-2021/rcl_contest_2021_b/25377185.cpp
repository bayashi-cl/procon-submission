...
void put(vector<vector<bool>>& s, Pa pos, int p) {
    int n = s.size();
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            if (abs(i - pos.first) + abs(j - pos.second) <= p) {
                s[i][j] = true;
            }
        }
    }
...
void solve() {
    auto n = input<int>();
    using chair = pair<int, Pa>;
    vector<chair> e;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            e.push_back({input<int>(), {i, j}});
        }
    }
    sort(std::begin(e), std::end(e), std::greater<chair>());
    int max_score = 0;
    vector<vector<int>> answer(n, vector<int>(n));
    for (int p = 0; p <= n; ++p) {
        vector<vector<bool>> sound(n, vector<bool>(n));
        vector<vector<int>> ans(n, vector<int>(n));
        int score = 0;
        for (auto [eij, ij] : e) {
            auto [i, j] = ij;
            if (sound[i][j]) continue;
            put(sound, ij, p);
            score += eij * p;
            ans[i][j] = p;
        }
        if (chmax(max_score, score)) answer = ans;
    }
    for (auto&& e : answer) print(e);
}
}  // namespace bys
