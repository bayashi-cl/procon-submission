...
def main(**kwargs) -> None:
    ...
    px = [(n - i, ai, bi, ai + bi) for i, (ai, bi) in enumerate(zip(a, b))]
    py = [(n - i, ai, bi, ai + bi) for i, (ai, bi) in enumerate(zip(a, b))]
    pz = [(n - i, ai, bi, ai + bi) for i, (ai, bi) in enumerate(zip(a, b))]
    ans = set()

    px.sort(key=itemgetter(1), reverse=True)
    for i, *_ in px:
        if x > 0 and i not in ans:
            x -= 1
            ans.add(i)
    py.sort(key=itemgetter(2), reverse=True)
    for i, *_ in py:
        if y > 0 and i not in ans:
            y -= 1
            ans.add(i)
    pz.sort(key=itemgetter(3), reverse=True)
    for i, *_ in pz:
        if z > 0 and i not in ans:
            z -= 1
            ans.add(i)

    for e in reversed(sorted(ans)):
        print(n - e + 1)
