...
def main(**kwargs) -> None:
    _, *xyz = map(int, readline().split())
    a = list(map(int, readline().split()))
    b = list(map(int, readline().split()))
    p = [(i + 1, ai, bi, ai + bi) for i, (ai, bi) in enumerate(zip(a, b))]
    ans = set()
    for e in range(3):
        for i, *_ in sorted(p, key=itemgetter(e + 1), reverse=True):
            if xyz[e] > 0 and i not in ans:
                xyz[e] -= 1
                ans.add(i)
