...
void Solver::solve() {
    ...
    auto s = scanner.readvec<ll>(n - 1);
    auto x = scanner.readvec<ll>(m);
    set xs(x.begin(), x.end());
    vector<ll> a = {0};
    for (auto si : s) a.push_back(si - a.back());
    std::unordered_map<ll, int> cnt_even, cnt_odd;
    for (int i : irange(n)) {
        if (i & 1) {
            cnt_odd[a[i]]++;
        } else {
    ...
    for (int i : irange(n)) {
        for (auto xi : x) {
            ...
            for (auto xj : x) {
                if (i & 1) {
                    // luckey += cnt_odd[xj - offset] + cnt_even[xj + offset];
                    if (auto itr = cnt_odd.find(xj - offset); itr != cnt_odd.end()) luckey += itr->second;
                    if (auto itr = cnt_even.find(xj + offset); itr != cnt_even.end()) luckey += itr->second;
                } else {
                    // luckey += cnt_odd[xj + offset] + cnt_even[xj - offset];
                    if (auto itr = cnt_odd.find(xj + offset); itr != cnt_odd.end()) luckey += itr->second;
                    if (auto itr = cnt_even.find(xj - offset); itr != cnt_even.end()) luckey += itr->second;
