...
void Solver::solve() {
    ...
    auto pre = scanner.readvec<Int1>(n);
    auto in = scanner.readvec<Int1>(n);

    if (pre[0] != 0) EXIT(-1);

    vector<int> pos_pre(n), pos_in(n);
    for (int i : irange(n)) {
        pos_pre[pre[i]] = i;
        pos_in[in[i]] = i;
    }
    ...
    vector<Pa> ans(n);
    bool flg = false;
    FixPoint rec([&](auto&& self, int pre_left, int pre_right, int in_left, int in_right) -> void {
        auto parent = pre[pre_left];
        auto in_parent_pos = pos_in[parent];
        if (in_parent_pos < in_left or in_right <= in_parent_pos or flg) {
            flg = true;
            return;
        }
        auto left_cnt = in_parent_pos - in_left;
        auto right_cnt = (pre_right - pre_left) - 1 - left_cnt;
        Pa children = {0, 0};
        auto left_child_start = pre_left + 1;
        if (left_cnt > 0) {
            children.first = pre[left_child_start] + 1;
            self(left_child_start, left_child_start + left_cnt, in_left, in_parent_pos);
        }
        auto right_child_start = pre_left + 1 + left_cnt;
        if (right_cnt > 0) {
            children.second = pre[right_child_start] + 1;
            self(right_child_start, pre_right, in_parent_pos + 1, in_right);
        }
        ans[parent] = children;
    });
    rec(0, n, 0, n);
