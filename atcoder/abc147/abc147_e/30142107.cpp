...
void Solver::solve() {
    ...
    auto a = scanner.readvec<int>(n, m);
    auto b = scanner.readvec<int>(n, m);
    constexpr int offset = 10000;
    vector dp(n + 1, vector<std::bitset<20000>>(m + 1));
    dp[0][1].set(offset);
    for (int i : irange(1, n + 1)) {
        for (int j : irange(1, m + 1)) {
            int diff = abs(a[i - 1][j - 1] - b[i - 1][j - 1]);
            dp[i][j] |= dp[i - 1][j] << diff | dp[i - 1][j] >> diff;
            dp[i][j] |= dp[i][j - 1] << diff | dp[i][j - 1] >> diff;
        }
    }
    int ans = INF;
    for (int i : irange(dp[n][m].size())) {
        if (dp[n][m].test(i)) chmin(ans, abs(i - offset));
    }
    print(ans);
}
}  // namespace bys
