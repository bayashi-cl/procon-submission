...
namespace bys {
void Solver::solve() {
    auto [n, q, x] = scanner.read<ll, 3>();
    auto w = scanner.readvec<ll>(n);
    auto sum_w = std::accumulate(w.begin(), w.end(), 0LL);
    ll loop = n * (x / sum_w);
    x %= sum_w;

    auto ww = w;
    for (auto wi : w) ww.push_back(wi);
    std::partial_sum(ww.begin(), ww.end(), ww.begin());
    vector db(65, vector<int>(n));
    vector<ll> cnt(n);
    for (int i : irange(n)) {
        auto th = i == 0 ? x : x + ww[i - 1];
        auto close_itr = std::lower_bound(ww.begin(), ww.end(), th);
        int nxt = close_itr - ww.begin() + 1;

        db[0][i] = nxt % n;
        cnt[i] = loop + nxt - i;
    }
    for (auto i : irange(64)) {
        for (auto j : irange(n)) {
