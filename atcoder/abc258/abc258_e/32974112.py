...
def main(**kwargs) -> None:
    n, q, x = map(int, readline().split())
    w = list(map(int, readline().split()))
    loop_weight = sum(w)
    n_loop, reminder = divmod(x, loop_weight)
    ww_cumsum = list(accumulate(w + w))
    ww_cumsum.insert(0, 0)
    n_potato = [0] * n

    db = [[0] * n for _ in range(65)]
    for i in range(n):
        th = ww_cumsum[i] + reminder
        index = bisect_left(ww_cumsum, th)

        db[0][i] = index % n
        n_potato[i] = n_loop * n + (index - i)
