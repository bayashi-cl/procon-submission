...
void Solver::solve() {
    ...
    vector<int> ss;
    for (auto&& si : s) ss.push_back(si);
    atcoder::segtree<int, op, e> seg(ss);
    int r = n;
    for (int l : Range(n)) {
        if (l >= r) break;
        auto mi = seg.prod(l, r);
        if (ss[l] <= mi) continue;
        while (r >= 0) {
            --r;
            if (ss[r] == mi) break;
        }
        if (r < 0) throw std::logic_error("");
        if (l >= r) break;
        std::swap(s[l], s[r]);
    }
    print(s);
