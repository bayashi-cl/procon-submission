...
void Solver::solve() {
    ...
    auto ab = scanner.readvec<pair<double, double>>(n);
    auto cd = scanner.readvec<pair<double, double>>(m);
    reverse(ab.begin(), ab.end());
    reverse(cd.begin(), cd.end());
    vector<point> sp, tp;
    polygon s, t;
    for (auto&& [ai, bi] : ab) {
        sp.push_back(point(ai, bi));
    }
    sp.push_back(point(ab[0].first, ab[0].second));
    for (auto&& [ci, di] : cd) {
        tp.push_back(point(ci, di));
    }
    tp.push_back(point(cd[0].first, cd[0].second));
    bg::assign_points(s, sp);
    bg::assign_points(t, tp);
    vector<polygon> out;
    bg::intersection(s, t, out);
    if (out.size() == 0) EXIT(0);
    print(bg::area(out[0]));
}
}  // namespace bys
