...
#line 5 "past202112_i/main.cpp"

namespace bys {
void Solver::solve() {
    auto [n, m] = input<ll, 2>();
    auto abc = input<array<ll, 3> >(m);
    Compress<ll> cp;
    cp.add(1);
    cp.add(n);
    for (auto&& [a, b, c] : abc) {
    ...
    cp.construct();
    Adj graph(cp.size());
    for (int i : Range(cp.size() - 1)) {
        ll c = cp.unzip(i + 1) - cp.unzip(i);
        graph[i].push_back({i + 1, c});
        graph[i + 1].push_back({i, c});
    }
    for (auto&& [a, b, c] : abc) {
        int ca = cp.get(a);
        int cb = cp.get(b);
        graph[ca].push_back({cb, c});
        graph[cb].push_back({ca, c});
    }
