...
void Solver::solve() {
    ...
    int sz = n.length();
    int ans = 500;
    set<int> left = {'1', '2', '3', '4', '5'};
    set<int> right = {'6', '7', '8', '9', '0'};
    for (int i : Range(1, sz)) {
        if (n[i] == n[i - 1]) {
            ans += 301;
        } else if ((left.find(n[i]) != left.end() &&
                    left.find(n[i - 1]) != left.end()) ||
                   (right.find(n[i]) != right.end() &&
                    right.find(n[i - 1]) != right.end())) {
            ans += 210;
        } else {
            ans += 100;
        }
