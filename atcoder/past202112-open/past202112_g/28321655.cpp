...
void Solver::solve() {
    ...
    set<OrderedPair<int>> edges;
    for (int i = 0; i < q; ++i) {
        auto [t, u, v] = input<int, 3>();
        --u, --v;
        if (t == 1) {
            ...
            atcoder::dsu uft(n);
            for (auto&& [a, b] : edges) uft.merge(a, b);
            print(uft.same(u, v) ? "Yes" : "No");
        }
    }
}
}  // namespace bys
