...
#line 6 "abc242_e/main.cpp"

namespace bys {
void Solver::solve() {
    auto [n, s] = scanner.read<int, string>();
    int h = ceildiv(n, 2);
    vector dp(4, vector<mint>(h + 1));
    dp[0][0] = 1;
    int cnt = 0;
    for (int i : irange(h)) {
        char l = s[i], r = s[n - i - 1];
        // char th = min(l, r);
        if (l == r && n & 1 && i != h - 1) ++cnt;
        for (char c = 'A'; c <= 'Z'; ++c) {
            dp[1][i + 1] += dp[1][i];
            if (c < l) {
                dp[1][i + 1] += dp[0][i] + dp[2][i] + dp[3][i];
            }
            if (c == l) {
                if (c < r) {
                    dp[0][i + 1] += dp[0][i];
    ...
    mint ans = dp[0].back() + dp[1].back() + dp[2].back();
    // if (n & 1) ans = ans - 1 + cnt;
    print(ans);
}
}  // namespace bys
