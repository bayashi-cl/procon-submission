...
#line 5 "panasonic2020_d/main2.cpp"

namespace bys {
void Solver::solve() {
    auto n = scanner.read<uint>();
    vector<string> ans;
    FixPoint dfs([&](auto&& self, string s) -> void {
        if (s.length() == n) {
            ans.emplace_back(s);
            return;
        }
        char max_c = s.length() == 0 ? 'b' : *std::max_element(s.begin(), s.end()) + 2;
        for (auto&& nxt : irange('a', max_c)) {
            s.push_back(nxt);
            self(s);
            s.pop_back();
        }
