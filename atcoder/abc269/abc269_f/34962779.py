...
def arithmetic_sum(start: int, diff: int, n: int) -> int:
    return (start * 2 + (n - 1) * diff) * n // 2
...
def main(**kwargs) -> None:
    ...
    def calc(i: int, j: int) -> int:
        ...
        col = (j + 1) // 2
        s = arithmetic_sum(1, 2, col)
        res += arithmetic_sum(s, 2 * m * col, (i + 1) // 2)
        if i >= 2 and j >= 2:
            col = j // 2
            s = arithmetic_sum(m + 2, 2, col)
            res += arithmetic_sum(s, 2 * m * col, i // 2)
        return res

    q = int(readline())
    for _ in range(q):
        a, b, c, d = map(int, readline().split())
        ans = calc(b, d) - calc(b, c - 1) - calc(a - 1, d) + calc(a - 1, c - 1)
        print(ans % MOD)
