...
#line 5 "joi2021_yo2_b/main.cpp"

namespace bys {
void solve() {
    auto [n, q] = input<int, 2>();
    umap<string, int> cost;
    std::queue<string> que;
    for (int a = 0; a <= n; ++a) {
        for (int b = 0; b <= n - a; ++b) {
            int c = n - a - b;
            string goal = string(a, 'A') + string(b, 'B') + string(c, 'C');
            que.emplace(goal);
            cost.emplace(goal, 0);
            while (!que.empty()) {
                ...
                for (int i = 2; i <= n; ++i) {
                    auto to = top;
                    reverse(to.begin(), to.begin() + i);
                    if (cost.find(to) == cost.end()) {
                        cost.emplace(to, from + 1);
                        que.emplace(to);
                    }
                }
            }
