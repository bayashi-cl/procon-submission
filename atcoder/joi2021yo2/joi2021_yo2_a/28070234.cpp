...
#line 2 "joi2021_yo2_a/main.cpp"

namespace bys {
void Solver::solve() {
    auto [n, a, s] = input<int, int, string>();
    --a;
    vector<int> l, r;
    for (int i : Range(n)) {
        if (i < a) {
    ...
    DEBUG(l);
    DEBUG(r);
    while (std::next(itr_l) != l.end() || std::next(itr_r) != r.end()) {
        if (i % 2 == 0) {  // right
            ans += abs(*itr_r - pos);
            pos = *itr_r;
            if (std::next(itr_r) != r.end()) itr_r++;
        } else {  // left
            ans += abs(*itr_l - pos);
            pos = *itr_l;
            if (std::next(itr_l) != l.end()) itr_l++;
        }
        ++i;
        DEBUG(i, *itr_l, *itr_r);
    }
