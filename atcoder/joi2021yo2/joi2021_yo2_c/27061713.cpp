...
#line 2 "joi2021_yo2_c/main.cpp"

namespace bys {
void solve() {
    auto [n, d, k] = input<int, ll, ll>();
    auto cost = [&](int j) -> ll { return d + k * j; };
    map<ll, int> a, b;
    vector<pair<ll, int>> s = {{0, 1}, {0, 2}};
    for (int i = 0; i < n; ++i) {
        auto [pi, si] = input<int, ll>();
        s.push_back({si, pi});
        if (pi == 1) {
            a[si] = -1;
        } else {
    ...
    a[0] = 0;
    b[0] = 0;
    // if (!a.empty()) a.begin()->second = 1;
    // if (!b.empty()) b.begin()->second = 1;
    ...
    for (auto& [si, pi] : s) {
        ...
        auto itr_a = a.lower_bound(next_a);
        if (itr_a != a.end()) {
            chmax(itr_a->second, take + 1);
        }
        auto itr_b = b.lower_bound(next_b);
        if (itr_b != b.end()) {
            chmax(itr_b->second, take + 1);
        }
    }
    print(max(a.rbegin()->second, b.rbegin()->second));
    // int ans = 0;
    // if (!a.empty()) chmax(ans, a.rbegin()->second);
    // if (!b.empty()) chmax(ans, b.rbegin()->second);
    // print(ans);
}
}  // namespace bys
