...
void Solver::solve() {
    ...
    Grid g(n, 2);
    DynamicAdjacencyList graph(g.area());
    for (int i : irange(n - 1)) {
        if (abs(a[i + 1] - a[i]) <= k) {
            graph.add_edge(g.index(i, 0), g.index(i + 1, 0));
        }
        if (abs(b[i + 1] - a[i]) <= k) {
            graph.add_edge(g.index(i, 0), g.index(i + 1, 1));
        }
        if (abs(a[i + 1] - b[i]) <= k) {
            graph.add_edge(g.index(i, 1), g.index(i + 1, 0));
        }
        if (abs(b[i + 1] - b[i]) <= k) {
            graph.add_edge(g.index(i, 1), g.index(i + 1, 1));
        }
    }
    auto res1 = breadth_first_search(graph, g.index(0, 0));
    if (res1.cost[g.index(n - 1, 0)] != LINF or res1.cost[g.index(n - 1, 1)] != LINF) EXIT("Yes");
    auto res2 = breadth_first_search(graph, g.index(0, 1));
    if (res2.cost[g.index(n - 1, 0)] != LINF or res2.cost[g.index(n - 1, 1)] != LINF) EXIT("Yes");
    print("No");
}
}  // namespace bys
