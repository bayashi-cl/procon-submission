...
void Solver::solve() {
    ...
    auto c = scanner.readvec<int>(m);
    auto d = scanner.readvec<int>(m);

    vector<Pa> ab, cd;
    for (int i : irange(n)) ab.emplace_back(a[i], b[i]);
    sort(ab.begin(), ab.end(), std::greater<Pa>());
    for (int i : irange(m)) cd.emplace_back(c[i], d[i]);
    sort(cd.begin(), cd.end());
    std::multiset<int> cs;
    for (auto [ai, bi] : ab) {
        while (!cd.empty() and cd.back().first >= ai) {
            cs.insert(cd.back().second);
            cd.pop_back();
        }

        auto itr = cs.lower_bound(bi);
        if (itr == cs.end()) EXIT("No");
        cs.erase(itr);
    }
    print("Yes");
}
