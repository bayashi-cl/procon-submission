...
namespace bys {
void Solver::solve() {
    auto n = scanner.read<string>();
    int sz = n.length();
    vector<int> dp1(sz + 1, INF), dp2(sz + 1, INF);
    dp1[0] = 0;
    dp2[0] = 1;
    for (int i : irange(sz)) {
        int d = n[i] - '0';
        chmin(dp1[i + 1], dp1[i] + d);
        chmin(dp2[i + 1], dp1[i] + d + 1);
        chmin(dp1[i + 1], dp2[i] + (10 - d));
        chmin(dp2[i + 1], dp2[i] + (10 - d - 1));
    }
