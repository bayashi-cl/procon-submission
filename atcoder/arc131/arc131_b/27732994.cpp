...
#line 3 "/home/bayashi/git/byslib/byslib/util.hpp"
...
#line 3 "arc131_b/main.cpp"

namespace bys {
void Solver::solve() {
    auto [h, w] = input<int, 2>();
    auto d = input<string>(h);
    Grid grid(h, w);
    set<char> col = {'1', '2', '3', '4', '5'};
    for (int i : Range(h)) {
        for (int j : Range(w)) {
            if (d[i][j] != '.') continue;
            auto c = col;
            for (auto& [ni, nj] : grid.next(i, j)) {
                if (c.find(d[ni][nj]) != c.end()) c.erase(d[ni][nj]);
            }
            d[i][j] = *c.begin();
        }
    }
    for (auto&& l : d) print(l);
}
}  // namespace bys
