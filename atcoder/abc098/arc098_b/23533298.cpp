...
template <class Tail> void debug(Tail&& tail) {std::cerr << tail << endl;}
template <class Head, class... Body> void debug(Head&& head, Body&&... tail) {std::cerr << head << " "; debug(std::forward<Body>(tail)...);}
