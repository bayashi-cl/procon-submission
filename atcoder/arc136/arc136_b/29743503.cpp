...
#line 3 "/home/bayashi/dev/byslib/utility/enumerate.hpp"
namespace bys {
...
void Solver::solve() {
    ...
    for (auto&& ai : a) cnt_a[ai]++;
    for (auto&& bi : b) cnt_b[bi]++;
    for (auto&& ai : a) {
        if (cnt_a[ai] != cnt_b[ai]) EXIT("No");
    }
    if ((int)cnt_a.size() != n) EXIT("Yes");

    map<int, int> order;
    for (auto [i, bi] : cenumerate(b)) order[bi] = i;
    for (auto&& ai : a) ai = order[ai];
    DEBUG(a);
    BinaryIndexedTree<int> bit(n + 1);
    ll cnt = 0;
    for (int ai : a) {
        cnt += bit.sum(ai + 1, n + 1);
        bit.add(ai, 1);
    }
