...
namespace bys {
void Solver::solve() {
    auto [h, w, a, b] = scanner.read<int, 4>();
    int ans = 0;
    int full = (1 << (h * w)) - 1;
    FixPoint rec([&](auto&& self, int state, int p, int q) -> void {
        if (state == full) {
            ++ans;
            return;
        }
        int ur = -1;
        for (int b : irange(h * w)) {
            if (not pop(state, b)) {
                ur = b;
                break;
            }
        ...
        state |= 1 << ur;
        if (p) {
            if (ur % w != w - 1 and not pop(state, ur + 1)) {
                self(state | (1 << (ur + 1)), p - 1, q);
            }
            if (ur + w < h * w and not pop(state, ur + w)) {
                self(state | (1 << (ur + w)), p - 1, q);
            }
        }
        if (q) {
            self(state, p, q - 1);
