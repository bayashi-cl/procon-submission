...
auto calc(i32 n, i32 a, i32 p) {
    auto dp = vector(n + 1, vector(n + 2, Mint(0)));
    dp[a][0] = Mint(1);
    auto inv_p = Mint(1) / p;

    for (auto i : irange(a, n)) {
        for (auto j : irange(n + 1)) {
            auto e = dp[i][j] * inv_p;
            for (auto d : irange(1, p + 1)) {
                dp[min(i + d, n)][j + 1] += e;
            }
        }
    }
...
void Solver::solve() {
    auto [n, a, b, p, q] = scanner.read<i32, 5>();
    auto ta = calc(n, a, p);
    auto ao = calc(n, b, q);
    ...
    for (auto i : irange(0, n + 1)) {
        auto p_ao = Mint(0);
        for (auto j : irange(i, (i32)ao.size())) {
            p_ao += ao[j];
        }
        ans += ta[i] * p_ao;
    }
