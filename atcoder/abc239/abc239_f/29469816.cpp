...
#line 3 "/home/bayashi/dev/byslib/data/union_find.hpp"
namespace bys {
...
void Solver::solve() {
    ...
    int sm = std::accumulate(d.begin(), d.end(), 0);
    if (sm != (n - m - 1) * 2) EXIT(-1);

    auto res = uft.groups();
    map<int, vector<int>> rem;
    for (auto&& [le, mem] : res) {
        for (auto&& mi : mem) {
            for (UV : irange(d[mi])) rem[le].push_back(mi);
        }
    }
    set<Pa> s;
    for (auto&& [k, v] : rem) {
    ...
    vector<Pa> ans;
    while (!s.empty()) {
        auto small_itr = s.begin();
        int sf = (*small_itr).first;
        if (sf > 1) EXIT(-1);
        if (sf == 0) break;
        int src = rem[small_itr->second].back();
        rem[small_itr->second].pop_back();
        s.erase(small_itr);

        auto big_itr = std::prev(s.end());
        auto big = *big_itr;
        int dest = rem[big.second].back();
        rem[big.second].pop_back();
        s.erase(big_itr);
        big.first--;
        s.insert(big);
