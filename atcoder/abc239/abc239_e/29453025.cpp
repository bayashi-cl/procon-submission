...
#line 4 "/home/bayashi/dev/byslib/graphv2/deapth_first_search.hpp"
namespace bys {
...
#line 6 "abc239_e/main.cpp"

namespace bys {
void Solver::solve() {
    auto [n, q] = scanner.read<int, 2>();
    auto x = scanner.readvec<int>(n);
    AdjacencyList graph(n, (n - 1) * 2);
    for (UV : irange(n - 1)) {
        auto [a, b] = scanner.read<Int1, 2>();
        graph.add_edge(a, b);
        graph.add_edge(b, a);
    }
    DeapthFirstSearch dfs(graph, 0);
    vector<vector<int>> dp(n);
    for (int i : irange(n)) dp[i].push_back(x[i]);
    for (int now : dfs.post_order) {
        for (auto&& nxt : graph[now]) {
            if ((int)nxt.dest == dfs.prev[now]) continue;
            for (auto e : dp[nxt.dest]) dp[now].push_back(e);
        }
        sort(dp[now].begin(), dp[now].end(), std::greater<int>());
        dp[now].erase(dp[now].begin() + min(20UL, dp[now].size()), dp[now].end());
    }
    for (UV : irange(q)) {
        auto [v, k] = scanner.read<Int1, 2>();
        print(dp[v][k]);
    }
}
}  // namespace bys
