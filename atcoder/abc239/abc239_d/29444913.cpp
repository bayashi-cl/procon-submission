...
#line 3 "/home/bayashi/dev/byslib/math/osa_k.hpp"
...
#line 5 "abc239_d/main.cpp"

namespace bys {
void Solver::solve() {
    Osa_K osa_k(1000);
    auto [a, b, c, d] = scanner.read<int, 4>();
    for (int ta : irange(a, b + 1)) {
        bool flg = true;
        for (int ao : irange(c, d + 1)) {
            if (osa_k.is_prime(ta + ao)) {
                flg = false;
                break;
            }
        }
        if (flg) EXIT("Takahashi");
    }
    print("Aoki");
}
}  // namespace bys
