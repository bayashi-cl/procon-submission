...
#line 5 "dp_w/main.cpp"
// #include "byslib/utility/itemsorter.hpp"
...
void Solver::solve() {
    ...
    vector<vector<pair<int, ll>>> interval(n + 1);
    for (UV : irange(m)) {
        auto [l, r, a] = scanner.read<int, int, ll>();
        interval[r].push_back({l, a});
    }
    atcoder::lazy_segtree<S, op, e, F, mapping, composition, id> seg(n + 1);
    seg.set(0, 0);
    for (int i : Range(1, n + 1)) {
        auto ma = seg.all_prod();
        seg.set(i, ma);
        for (auto&& [l, a] : interval[i]) {
            seg.apply(l, i + 1, a);
        }
    }
    print(seg.all_prod());
