...
void Solver::solve() {
    ...
    vector memo(n, vector<std::optional<ll>>(n + 1));
    FixPoint dp([&](auto&& self, int l, int r) -> ll {
        if (memo[l][r].has_value()) return memo[l][r].value();
        if (l + 1 == r) return 0;
        ll res = LINF;
        for (auto&& m : Range(l + 1, r)) {
            chmin(res, self(l, m) + self(m, r) + cs.sum(l, m) + cs.sum(m, r));
        }
        memo[l][r] = res;
        return res;
    });
