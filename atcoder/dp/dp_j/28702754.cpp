...
#line 5 "dp_j/main.cpp"

namespace bys {
void Solver::solve() {
    auto n = scanner.read<int>();
    auto a = scanner.readvec<int>(n);
    vector memo(n + 1, vector(n + 1, vector<std::optional<ld>>(n + 1)));

    FixPoint dp([&](auto&& self, int i, int j, int k) -> ld {
        if (i + j + k == 0) return 0.0;
        if (memo[i][j][k].has_value()) return memo[i][j][k].value();
        ld ijk = i + j + k;
        ld res = n / ijk;
        if (i > 0) res += self(i - 1, j, k) * (i / ijk);
        if (j > 0) res += self(i + 1, j - 1, k) * (j / ijk);
        if (k > 0) res += self(i, j + 1, k - 1) * (k / ijk);
        memo[i][j][k] = res;
        return res;
    });
    vector<int> cnt(4);
