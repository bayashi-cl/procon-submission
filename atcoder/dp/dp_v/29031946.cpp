...
#line 3 "/home/bayashi/dev/byslib/draft/edge.hpp"
namespace bys {
struct Edge {
    std::size_t src, dest;
    ll weight;
...
#line 4 "/home/bayashi/dev/byslib/draft/deapth_first_search.hpp"
namespace bys {
...
#line 5 "/home/bayashi/dev/byslib/draft/reader.hpp"
namespace bys {
...
#line 6 "dp_v/main.cpp"

namespace bys {
void Solver::solve() {
    auto [n, m] = scanner.read<uint, 2>();
    auto graph = read_adj_uv(n, n - 1);
    vector<ll> dp1(n), dp2(n);
    DeapthFirstSearch dfs(graph, 0);

    // dfs1
    for (auto&& now : dfs.post_order) {
        ll prod = 1;
        for (auto&& next : graph[now]) {
            if (next == dfs.prev[now]) continue;
            prod = prod * dp1[next] % m;
        }
        dp1[now] = (prod + 1) % m;
    }
    ...
    for (auto&& now : dfs.pre_order) {
        int last = dfs.prev[now];
        if (last == -1) dp2[now] = 1;
        int csz = graph[now].size();
        vector<ll> lprod(csz, 1), rprod(csz, 1);
        for (auto i : irange(csz)) {
            int to = graph[now][i];
        ...
        for (auto i : reversed(irange(csz))) {
            int to = graph[now][i];
            if (i + 1 < (int)rprod.size()) rprod[i] = rprod[i + 1];
            if (to == last) continue;
            rprod[i] *= dp1[to];
            rprod[i] %= m;
        }
        ...
        for (auto i : irange(csz)) {
            ...
            if (to == last) continue;
            ll rest_prod = 1;
            if (i - 1 >= 0) rest_prod = rest_prod * lprod[i - 1] % m;
            if (i + 1 < (int)rprod.size()) rest_prod = rest_prod * rprod[i + 1] % m;
            dp2[to] = (dp2[now] * rest_prod + 1) % m;
        }
    }
    // dfs3
    vector<ll> ans(n);
