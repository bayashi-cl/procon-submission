...
#line 5 "dp_i/main.cpp"

namespace bys {
void Solver::solve() {
    auto n = scanner.read<int>();
    auto p = scanner.readvec<ld>(n);
    // auto p = scanner.readln([](const string& s) { return (int)std::round(std::stod(s) * 100); });
    ...
    for (int i : Range(1, n + 1)) {
        ...
        for (int j : Range(n + 1)) {
            ...
            if (j - 1 >= 0) dp[i][j] += dp[i - 1][j - 1] * pi;
        }
    }
    ld ans = 0;
    for (int j : Range((int)ceildiv(n, 2), n + 1)) ans += dp[n][j];
    print(ans);
}
}  // namespace bys
