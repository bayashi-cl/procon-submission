...
#line 7 "dp_u/main.cpp"

namespace bys {
void Solver::solve() {
    auto n = scanner.read<int>();
    auto a = scanner.readvec<ll>(n, n);

    vector<std::optional<ll>> memo(1 << n);
    FixPoint dp([&](auto&& self, int s) -> ll {
        if (memo[s].has_value()) return *memo[s];
        ll res = 0;
        for (auto&& [i, j] : IndexCombinations<2>(n)) {
            if (pop(s, i) && pop(s, j)) res += a[i][j];
        }
        for (int t = s; t >= 0; t--) {
            t &= s;
            if (t == s || t == 0) continue;
            chmax(res, self(t) + self(t ^ s));
        }
        memo[s] = res;
        return res;
    });
