...
#line 5 "dp_s/main.cpp"

namespace bys {
void Solver::solve() {
    auto [k, d] = scanner.read<string, int>();
    int n = k.length();
    // vector dp(n + 1, vector(d, vector<mint7>(2)));
    // dp[0][0][0] = 1;
    // for (int i : Range(n)) {
    //     int ki = k[i] - '0';
    //     for (int j : Range(d)) {
    ...
    //     }
    // }
    // print(dp[n][0][0] + dp[n][0][1] - 1);
    vector dp(n + 1, vector(2, vector<mint7>(d)));
    dp[0][0][0] = 1;
    for (int i : Range(n)) {
        int ki = k[i] - '0';
        for (int j : Range(d)) {
            for (int t : Range(10)) {
                int nxt = (j + t) % d;
                if (t < ki) {
                    dp[i + 1][1][nxt] += dp[i][0][j];
                } else if (t == ki) {
                    dp[i + 1][0][nxt] += dp[i][0][j];
                }
                dp[i + 1][1][nxt] += dp[i][1][j];
            }
        }
    }
