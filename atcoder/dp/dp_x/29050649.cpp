...
#line 4 "dp_x/main.cpp"

namespace bys {
void Solver::solve() {
    auto n = scanner.read<int>();
    auto wsv = scanner.readvec<array<ll, 3>>(n);
    constexpr int MAX_S = 22000;
    sort(wsv.begin(), wsv.end(), [](const auto& a, const auto& b) { return a[0] + a[1] < b[0] + b[1]; });

    vector dp(n + 1, vector<ll>(MAX_S));
    for (int i : irange(1, n + 1)) {
        auto [w, s, v] = wsv[i - 1];
        for (int j : irange(MAX_S)) {
            dp[i][j] = dp[i - 1][j];
            if (j - w <= s && j - w >= 0) {
    ...
    print(*std::max_element(dp[n].begin(), dp[n].end()));
}
}  // namespace bys
