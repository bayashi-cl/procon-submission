...
void Solver::solve() {
    ...
    vector<pair<mint7, mint7>> dp(n, {1, 1});  // {白, 黒}
    for (auto&& now : dfs.post_order) {
        for (auto&& nxt : graph[now]) {
            if (nxt.to == dfs.prev[now]) continue;
            dp[now].first *= dp[nxt.to].first + dp[nxt.to].second;
            dp[now].second *= dp[nxt.to].first;
        }
    }
    print(dp[0].first + dp[0].second);
}
}  // namespace bys
