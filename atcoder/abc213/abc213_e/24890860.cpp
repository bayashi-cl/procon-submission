...
void solve() {
    ...
    vector<vector<pair<ll, int>>> adj(h * w);
    ...
    for (int i = 0; i < h; ++i) {
        for (int j = 0; j < w; ++j) {
            ...
            for (auto [di, dj] : punch) {
                ...
                int t = idx(ni, nj);
                adj[f].push_back({1LL, t});
                adj[t].push_back({1LL, f});
                if (abs(di) == 2 || abs(dj) == 2) {
                    for (auto [ddi, ddj] : delta) {
                        int nni = ni + ddi, nnj = nj + ddj;
                        if (nni < 0 || h <= nni) continue;
                        if (nnj < 0 || w <= nnj) continue;
                        if (s[nni][nnj] == '.') {
                            int tt = idx(nni, nnj);
                            adj[f].push_back({1LL, tt});
                            // adj[tt].push_back({1LL, f});
                        }
                    }
                }
            }
            ...
            for (auto [di, dj] : delta) {
                ...
                if (s[ni][nj] == '.') {
                    int t = idx(ni, nj);
                    adj[f].push_back({0LL, t});
                    adj[t].push_back({0LL, f});
                }
            }
        }
    }
