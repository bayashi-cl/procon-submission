...
#line 2 "past202109_e/main.cpp"

namespace bys {
void Solver::solve() {
    auto [n, k] = input<int, uint>();
    auto c = input<int>(n);
    auto p = input<int>(n);
    map<int, int> cost;
    for (int i : Range(n)) {
        if (cost.find(c[i]) == cost.end()) {
            cost[c[i]] = p[i];
        } else {
            chmin(cost[c[i]], p[i]);
        }
    ...
    vector<int> min_cost;
    for (auto&& [key, val] : cost) min_cost.push_back(val);
    sort(min_cost.begin(), min_cost.end());
    if (min_cost.size() < k) EXIT(-1);
    print(std::accumulate(min_cost.begin(), min_cost.begin() + k, 0LL));
}
}  // namespace bys
