...
#line 7 "abc236_d/main.cpp"

namespace bys {
void Solver::solve() {
    auto n = scanner.read<int>();
    vector b(2 * n, vector<ll>(2 * n));
    for (auto&& [i, j] : IndexCombinations<2>(2 * n)) b[i][j] = scanner.read<ll>();

    ll MX = 1 << (2 * n);
    ll ans = -1;
    FixPoint rec([&](auto&& self, ll s, ll c) -> void {
        assert(s >= 0);
        if (s == MX - 1) chmax(ans, c);
        int f = -1;
        for (int d : Range(2 * n)) {
            if (pop(s, d)) continue;
            if (f == -1) {
