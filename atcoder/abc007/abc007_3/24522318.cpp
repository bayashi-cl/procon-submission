...
void solve() {
    auto [r, c, si, sj, gi, gj] = input<int, 6>();
    int start = (si - 1) * c + (sj - 1);
    int goal = (gi - 1) * c + (gj - 1);
    auto b = input<string>(r);
    ...
    for (int i = 0; i < r; ++i) {
        for (int j = 0; j < c; ++j) {
            ...
            for (auto [di, dj] : delta) {
                if (b[i + di][j + dj] == '.') {
                    adj[i * c + j].push_back((i + di) * c + (j + dj));
                }
            }
        }
    }
    BreadthFirstSearch bfs(adj);
    auto cost = bfs.search(start);
    print(cost[goal]);
}
}  // namespace bys
