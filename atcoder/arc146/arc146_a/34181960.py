...
def main(**kwargs) -> None:
    ...
    for ai in a:
        a_dict[len(ai)].append(int(ai))

    for key in a_dict:
        a_dict[key].sort(reverse=True)

    a_sorted = []
    for d in sorted(a_dict.keys(), reverse=True):
        a_sorted.extend(a_dict[d])
    cand = [str(ai) for ai in a_sorted[:3]]
