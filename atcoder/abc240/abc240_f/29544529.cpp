...
#line 5 "abc240_f/main.cpp"

namespace bys {
ll s(ll a, ll d, ll n) { return n * (2 * a + (n - 1) * d) / 2; }
void Solver::solve() {
    auto [n, m] = scanner.read<ll, 2>();
    auto xy = scanner.readvec<pair<ll, ll>>(n);
    ll ans = max(xy[0].first, s(xy[0].first, xy[0].first, xy[0].second));
    ll cs = 0;
    ll cscs = 0;
    vector<ll> cand(n, -1);
    for (auto i : irange(n)) {
        ...
        if (cs >= 0 && cs_nxt < 0) {
            assert(x < 0);
            ll posi = ceildiv(cs, abs(x));
            ll ex = posi * (2 * cs + (posi - 1) * x) / 2;
            cand[i] = ex;
            chmax(ans, cscs + ex);
        }
        cscs += y * (2 * cs + (y - 1) * x) / 2;
