...
#line 4 "/home/bayashi/dev/byslib/graphv2/depth_first_search.hpp"
namespace bys {
...
#line 4 "/home/bayashi/dev/byslib/graphv2/rooted_tree.hpp"
namespace bys {
...
void Solver::solve() {
    ...
    auto graph = make_rooted(read_tree_uv(n), 0);
    ...
    for (auto&& now : dfs.post_order) {
        if (graph[now].empty()) {
            ...
            int l = INF, r = -1;
            for (auto&& nxt : graph[now]) {
                chmin(l, ans[nxt.dest].first);
                chmax(r, ans[nxt.dest].second);
            }
            ans[now] = {l, r};
        }
    }
