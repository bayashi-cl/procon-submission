...
#line 8 "abc240_e/main.cpp"

namespace bys {
void Solver::solve() {
    auto n = scanner.read<int>();
    // AdjacencyList graph(n, 2 * (n - 1));
    ...
    FixPoint dfs([&](auto&& self, int now, int prev) -> void {
        for (auto&& e : graph[now]) {
            if (e == prev) continue;
            graph_r[now].push_back(e);
            self(e, now);
        }
    });
    dfs(0, -1);
    ...
    vector<Pa> ans(n);
    int crr = 1;
    FixPoint dfs2([&](auto&& self, int now) -> void {
        int sz = graph_r[now].size();
        ans[now].first = crr;
        for (int i : irange(sz - 1)) {
            self(graph_r[now][i]);
            ++crr;
        }
        if (!graph_r[now].empty()) self(graph_r[now].back());
        ans[now].second = crr;
    });
    dfs2(0);
    ...
    // auto et = dfs.euler_tour;
    // vector ans(n, vector<int>());

    // for (int i : irange(et.size())) {
    //     ans[et[i]].push_back(i);
    // }
    // Compress<int> cp;
    // for (auto&& e : ans) {
    //     cp.add(e.front());
    ...
    // cp.construct();
    // for (auto&& e : ans) {
    //     print(cp.get(e.front()) + 1, cp.get(e.back()) + 1);
    // }
}
}  // namespace bys
