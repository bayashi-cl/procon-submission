...
void Solver::solve() {
    ...
    ll ans = 0;
    for (auto ai : a) {
        auto div = make_divisor(ai);
        int sz = div.size();
        for (int i : irange(sz / 2)) {
            if (cnt.find(div[i]) != cnt.end() and cnt.find(div[sz - 1 - i]) != cnt.end()) {
                ans += cnt[div[i]] * cnt[div[sz - i - 1]] * 2;
            }
        }
        if (sz & 1) {
            if (cnt.find(div[sz / 2]) != cnt.end()) {
                ans += cnt[div[sz / 2]] * cnt[div[sz / 2]];
            };
        }
    }
    print(ans);
