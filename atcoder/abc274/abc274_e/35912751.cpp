...
namespace bys {
auto dist(Pa const& p, Pa const& q) { return std::hypotl(q.first - p.first, q.second - p.second); }
void Solver::solve() {
    auto [n, m] = scanner.read<int, 2>();
    auto e = n + m + 1;
    // auto a = scanner.readvec<Pa>(e);
    vector<Pa> a = {{0, 0}};
    for (UV : irange(n + m)) {
        a.push_back(scanner.read<Pa>());
    }
    vector d(e, vector<ld>(e));
    for (int i : irange(e)) {
        for (int j : irange(e)) {
            d[i][j] = dist(a[i], a[j]);
        }
    ...
    auto dp = ndvector({1UL << e, (uint)e, (uint)(m + 1)}, (ld)1e12);
    dp[1][0][0] = 0.0;

    for (int i : irange(1, 1 << (e))) {
        for (int j : irange(e)) {
            if (not pop(i, j)) continue;
            for (int k : irange(m + 1)) {
                auto v = 1 << k;
                for (auto nxt : irange(e)) {
                    if (pop(i, nxt)) continue;
                    if (k == m or nxt <= n) {
                        chmin(dp[i | (1 << nxt)][nxt][k], dp[i][j][k] + d[j][nxt] / v);
                    } else {
                        chmin(dp[i | (1 << nxt)][nxt][k + 1], dp[i][j][k] + d[j][nxt] / v);
                    }
                }
            }
        }
    ...
    auto msk = (1 << (n + 1)) - 1;
    ld ans = 1e12;
    for (auto s : irange(1 << (e))) {
        if ((s & msk) == msk) {
            for (int j : irange(e)) {
                for (int k : irange(m + 1)) {
                    chmin(ans, dp[s][j][k] + d[j][0] / (1 << k));
                }
            }
        }
    }
    // for (auto&& e : dp) DEBUG(e);
    // DEBUG(dp);
    print(ans);
}
}  // namespace bys
