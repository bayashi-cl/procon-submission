...
void Solver::solve() {
    ...
    sort(s.begin(), s.end());
    auto t_vec = scanner.readvec<string>(m);
    set t(t_vec.begin(), t_vec.end());

    int r = 16;
    for (auto&& si : s) r -= si.size();
    vector<vector<int>> seps;
    vector<int> sep;
    FixPoint([&](auto&& self, int rem) -> void {
        if ((int)sep.size() == n - 1) seps.push_back(sep);
        sep.push_back(0);
        for (int u : irange(1, rem + 1)) {
            sep.back() = u;
            self(rem - u);
        }
        sep.pop_back();
    ...
    vector<string> sep_str(17);
    for (int i : irange(1, 17)) {
        sep_str[i] = string(i, '_');
    }
    do {
        for (auto&& sep : seps) {
            std::stringstream ss;
            for (auto i : irange(n - 1)) {
                ss << s[i] << sep_str[sep[i]];
            }
            ss << s.back();
            auto x = ss.str();
            DEBUG(x);
            if (x.size() < 3 or 16 < x.size()) continue;
