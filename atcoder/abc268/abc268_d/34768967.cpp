...
void Solver::solve() {
    ...
    if (n == 1) {
        auto x = s[0];
        if (x.size() < 3 or 16 < x.size() or t.find(x) != t.end()) {
            print(-1);
        } else {
            print(x);
        }
    ...
    auto r = 16;
    for (auto&& si : s) r -= si.length();

    vector<vector<int>> seps;
    for (auto u : irange(n - 1, r + 1)) {
        for (auto b : irange(1 << (u - 1))) {
            if (__builtin_popcount(b) != n - 2) continue;
            std::vector<int> sep;
            int prev = -1;
            for (int d : irange(u - 1)) {
                if (pop(b, d)) {
                    sep.push_back(d - prev);
                    prev = d;
                }
            }
            sep.push_back((u - 1) - prev);
            assert((int)sep.size() == n - 1);
            seps.push_back(sep);
        }
    }
    // DEBUG(seps);
