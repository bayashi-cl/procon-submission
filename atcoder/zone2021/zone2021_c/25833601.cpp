...
void solve() {
    using Para = std::array<int, 5>;
    auto n = input<int>();
    auto member = input<Para>(n);
    auto ok = [&](int x) {
        set<Para> cand;
        for (auto&& m : member) {
            Para p;
            for (int i = 0; i < 5; ++i) p[i] = m[i] >= x;
            cand.insert(p);
        }
        vector<Para> cand_v(std::begin(cand), std::end(cand));
            ...
            for (int j = 0; j < s; ++j) {
                for (int k = 0; k < s; ++k) {
                    ...
                    for (int idx = 0; idx < 5; ++idx) {
                        if (cand_v[i][idx] == 0 && cand_v[j][idx] == 0 &&
                            cand_v[k][idx] == 0) {
                            flg = false;
                        }
                    }
