...
#line 4 "arc133_b/main.cpp"

namespace bys {
void Solver::solve() {
    auto n = scanner.read<int>();
    auto p = scanner.readvec<int>(n);
    auto q = scanner.readvec<int>(n);

    vector<int> pos(n + 1);
    for (int i : Range(n)) pos[q[i]] = i;
    vector<int> dp(n + 1, INF);
    dp[0] = -1;

    for (auto&& pi : p) {
        vector<Pa> index;
        for (auto&& t : Range(pi, n + 1, pi)) {
            int id = std::lower_bound(dp.begin(), dp.end(), pos[t]) - dp.begin();
            index.push_back({id, pos[t]});
        }
        for (auto [id, pt] : index) chmin(dp[id], pt);
    }
    int ans = std::lower_bound(dp.begin(), dp.end(), INF) - dp.begin() - 1;
    print(ans);
}
}  // namespace bys
