...
#line 4 "abc184_e/main.cpp"

namespace bys {
void Solver::solve() {
    auto [h, w] = input<int, 2>();
    auto a = input<string>(h);
    Grid grid(h, w);
    int s, g;
    int sz = grid.area();
    Adj graph(sz + 26);
    for (int i : Range(h)) {
        for (int j : Range(w)) {
            ...
            for (auto& [ni, nj] : grid.next(i, j)) {
                if (a[ni][nj] != '#') {
                    graph[now].push_back(Edge(grid.index(ni, nj), 1));
                }
            }

            if (!(a[i][j] == '.' || sg)) {
                int c = a[i][j] - 'a';
                graph[now].push_back(Edge(sz + c, 1));
                graph[sz + c].push_back(Edge(now, 0));
            }
        }
    }
    ZeroOneBFS bfs(graph, s);
    print(bfs.cost[g]);
}
}  // namespace bys
