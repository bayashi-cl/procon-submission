...
def main(**kwargs) -> None:
    ...
    a = [bytearray(readline()) for _ in range(h)]
    ...
    tele: List[List[Tuple[int, int]]] = [[] for _ in range(26)]
    ...
    for i in range(h):
        for j in range(w):
            ...
            if cell == oS:
                ...
                a[i][j] = odot
                gi, gj = i, j
            elif cell == odot or cell == ohash:
                pass
            else:
                tele[cell - oa].append((i, j))

    que: Deque[Tuple[int, int]] = deque()
    que.append((si, sj))
    cost_grid = [[IINF] * w for _ in range(h)]
    cost_grid[si][sj] = 0
    cost_tele = [IINF] * 26
    ...
    while que:
        i, j = que.popleft()
        if j == -1:
            nxt_cost = cost_tele[i] + 1
            for ni, nj in tele[i]:
                if nxt_cost < cost_grid[ni][nj]:
                    cost_grid[ni][nj] = nxt_cost
                    que.append((ni, nj))
        else:
            now = a[i][j]
            now_cost = cost_grid[i][j]
            for di, dj in delta:
                ni, nj = i + di, j + dj
                if 0 <= ni < h and 0 <= nj < w and a[ni][nj] != ohash:
                    if now_cost + 1 < cost_grid[ni][nj]:
                        cost_grid[ni][nj] = now_cost + 1
                        que.append((ni, nj))

            if now != odot:
                t = now - oa
                if now_cost < cost_tele[t]:
                    cost_tele[t] = now_cost
                    que.appendleft((t, -1))
