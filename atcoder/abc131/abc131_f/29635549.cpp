...
void Solver::solve() {
    ...
    for (UV : irange(n)) {
        auto [x, y] = scanner.read<int, 2>();
        row[x].push_back(y);
        col[y].push_back(x);
    }
    UnionFindTree uft(MAX);
    for (auto&& c : col) {
        if (c.size() < 2) continue;
        for (int i : irange(c.size() - 1)) uft.unite(c[i], c[i + 1]);
    }
    auto groups = uft.groups();
    ll ans = 0;
    for (auto&& [key, rows] : groups) {
        set<int> pos;
        for (auto&& rowidx : rows) {
            for (auto&& c : row[rowidx]) pos.insert(c);
        }
        int sz = pos.size();
        for (auto&& rowidx : rows) {
            ans += sz - row[rowidx].size();
        }
    }
    print(ans);
}
