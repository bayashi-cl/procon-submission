...
namespace bys {
void Solver::solve() {
    auto [n, k] = scanner.read<i32, 2>();
    auto a = scanner.readvec<i64>(n);
    auto q = scanner.read<i32>();
    vector aa(k, vector<i64>(n));

    for (auto i : irange(n)) {
        aa[i % k][i] = a[i];
    }

    vector<CumulativeSum<Add<i64>>> a_cs_modk;

    for (auto i : irange(k)) {
        a_cs_modk.push_back(cumsum(aa[i]));
    }

    for (UV : irange(q)) {
        bool ans = true;
        auto [l, r] = scanner.read<i32_1, i32>();
        auto minus = a_cs_modk[(r - k) % k].fold(l, r);
        for (auto i : irange(r - k + 1, r)) {
            auto s = a_cs_modk[i % k].fold(l, r) - minus;
            if (s != 0) {
                ans = false;
                break;
            }
