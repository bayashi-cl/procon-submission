...
void Solver::solve() {
    ...
    auto q = scanner.readvec<Int1>(n);
    atcoder::scc_graph scc(n);
    for (int i : irange(n)) {
        scc.add_edge(p[i], q[i]);
    }
    auto res = scc.scc();
    Mint ans = 1;
    vector<Mint> lucas = {2, 1};
    for (int i : irange(1, n)) lucas.push_back(lucas[i] + lucas[i - 1]);

    for (auto e : res) ans *= lucas[e.size()];
    print(ans);
}
}  // namespace bys
