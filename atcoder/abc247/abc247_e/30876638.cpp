...
ll calc(const vector<int>& v, int x, int y) {
    ll sz = v.size();
    ...
    ll res = 0;
    for (int i : irange(sz)) {
        auto itrx = std::lower_bound(xpos.begin(), xpos.end(), i);
        if (itrx == xpos.end()) continue;
        auto itry = std::lower_bound(ypos.begin(), ypos.end(), i);
        if (itry == ypos.end()) continue;
        int r = max(*itrx, *itry);
        res += sz - r;
    }
    return res;
    ...
    for (auto ai : a) {
        if (ai < y or x < ai) {
            ...
            if (ex) {
                ...
                sep.back().push_back(ai);
            }
        }
    }

    ll ans = 0;
    for (auto&& e : sep) ans += calc(e, x, y);
    print(ans);
}
}  // namespace bys
