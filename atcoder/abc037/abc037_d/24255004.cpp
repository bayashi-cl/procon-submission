...
int idx(int i, int j, int w) { return w * i + j; }

void bys::solve() {
    auto [h, w] = input<int, 2>();
    VecVec a;
    for (int i = 0; i < h; i++) a.push_back(input<int>(w));
        ...
        for (int j = 0; j < w; j++) {
            for (auto [di, dj] : delta) {
                ...
                if (a[i][j] < a[i + di][j + dj]) {
                    adj[idx(i, j, w)].push_back(idx(i + di, j + dj, w));
                }
            }
        }
    }
    ...
    for (int i = 0; i < h * w; i++) {
        if (!dfs.seen[i]) dfs.search(i);
    }
    vector<mint> dp(h * w);
    for (auto&& i : dfs.post_order) {
        dp[i] = 1;
        for (auto&& next : adj[i]) dp[i] += dp[next];
    }
    mint ans = 0;
    for (auto&& di : dp) ans += di;
    print(ans);
}
