...
void main() {
    ...
    using mani = pair<Pa, Pa>;  // {{year, type}, {v|a, idx|b}}
    vector<mani> que;
    for (int i = 0; i < m; i++) {
        auto [a, b, year] = input<int, 3>();
        que.push_back({{year, 0}, {a - 1, b - 1}});
    }
    auto q = input<int>();
    for (int i = 0; i < q; i++) {
        auto [v, year] = input<int, 2>();
        que.push_back({{year, 1}, {v - 1, i}});
    }
    sort(std::begin(que), std::end(que), std::greater<mani>());

    atcoder::dsu uft(n);
    Vec ans(q);
    for (auto [yt, data] : que) {
        if (yt.second == 0) {
            uft.merge(data.first, data.second);
        } else {
            ans[data.second] = uft.size(data.first);
        }
    }
