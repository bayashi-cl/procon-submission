...
void main() {
    ...
    using Edge = pair<int, Pa>;
    vector<Edge> graph;
    for (int i = 0; i < m; i++) {
        auto [a, b, y] = input<int, 3>();
        graph.push_back({y, {a - 1, b - 1}});
    }
    sort(std::begin(graph), std::end(graph), std::greater<Edge>());

    auto q = input<int>();
    using Person = pair<int, Pa>;
    vector<Person> go;
    for (int i = 0; i < q; i++) {
        auto [v, w] = input<int, 2>();
        go.push_back({w, {v - 1, i}});
    }
    sort(std::begin(go), std::end(go), std::greater<Person>());

    atcoder::dsu uft(n);
    vector<int> ans(q);
    int i = 0;
    for (auto [year, person] : go) {
        while (i < m && graph[i].first > year) {
            auto [a, b] = graph[i].second;
            uft.merge(a, b);
            ++i;
        }
        ans[person.second] = uft.size(person.first);
    }
