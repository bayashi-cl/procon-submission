...
void Solver::solve() {
    ...
    set<u8> seen;
    int ans = 0;
    FixPoint rec([&](auto&& self, u8 state) -> void {
        if (seen.find(state) != seen.end()) return;
        seen.insert(state);
        if (__builtin_popcountll(state) == k) {
            ++ans;
            return;
        }
        for (int b : irange(nn)) {
            if (pop(state, b)) {
                for (auto [di, dj] : delta) {
                    ...
                    if (0 <= ni and ni < n and 0 <= nj and nj < n) {
                        auto nb = n * ni + nj;
                        if (not pop(state, nb) and s[ni][nj] == '.') {
                            self(state | (1LL << nb));
                        }
                    }
                }
