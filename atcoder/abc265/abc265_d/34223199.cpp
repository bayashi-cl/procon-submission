...
void Solver::solve() {
    ...
    const auto [p, q, r] = scanner.read<ll, 3>();
    auto a = scanner.readvec<ll>(n);

    auto bit = fenwick_tree(a);

    for (int x : irange(n)) {
        auto pp = p + bit.prefix_fold(x);
        auto y = bit.bisect(pp);
        if (y == -1 or bit.prefix_fold(y) != pp) continue;

        auto qq = q + pp;
        auto z = bit.bisect(qq);
        if (z == -1 or bit.prefix_fold(z) != qq) continue;

        auto rr = r + qq;
        auto w = bit.bisect(rr);
        if (y == -1 or bit.prefix_fold(w) != rr) continue;
