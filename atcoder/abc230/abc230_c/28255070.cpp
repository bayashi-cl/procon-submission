...
#line 3 "abc230_c/main.cpp"

namespace bys {
void Solver::solve() {
    auto [n, a, b, p, q, r, s] = input<ll, 7>();
    vector ans(q - p + 1, string(s - r + 1, '.'));
    for (ll i : Range(p, q + 1)) {
        for (ll j : Range(r, s + 1)) {
            if (abs(i - a) == abs(j - b)) ans[i - p][j - r] = '#';
        }
    }
