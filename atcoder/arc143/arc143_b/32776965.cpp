...
void Solver::solve() {
    ...
    vector<Mint> factorial(n * n + 1);
    factorial[0] = 1;
    for (int i : irange(1, n * n + 1)) {
        factorial[i] = factorial[i - 1] * i;
    }
    MultiComb mc(n * n);

    auto ans = factorial[n * n];
    auto o = factorial[(n - 1) * (n - 1)] * n * n;
    for (int i : irange(1, n * n + 1)) {
        if (n <= i and i <= n * n - n + 1) {
            ans -= mc.perm(n * n - i, n - 1) * mc.perm(i - 1, n - 1) * o;
        }
    }
    print(ans);
}
