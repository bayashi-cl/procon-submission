...
struct Perm : Magma<std::array<int, N>> {
    using typename Magma<std::array<int, N>>::set_type;
    static set_type operation(const set_type& a, const set_type& b) {
        set_type res;
        for (auto i = 0UL; i < N; ++i) res[i] = a[b[i]];
        return res;
    }
    static set_type inverse(const set_type& a) {
        set_type res;
        for (auto i = 0UL; i < N; ++i) res[a[i]] = i;
        return res;
    }
    static inline set_type identity = []() {
        set_type res;
...
void Solver::solve() {
    ...
    auto f = [&](ll right) {
        ...
        for (int i : irange(n)) {
            ll t = s + (n - i - 1);
            if (right <= t) {
                std::rotate(p.begin(), p.begin() + (n - i), p.begin() + n);
                std::reverse(p.begin(), p.begin() + i);
                for (auto j : irange(s, right)) {
                    std::swap(p[i], p[i + 1 + j - s]);
                }
                break;
            }
            s = t;
    ...
    auto p = P::operation(P::inverse(f(l - 1)), f(r));
    vector<int> ans(n);
    for (int i : irange(n)) ans[i] = p[i] + 1;
    print(ans);
}
}  // namespace bys
