...
namespace bys {
void Solver::solve() {
    auto [n, m, q] = scanner.read<int, 3>();
    vector<std::array<ll, 4>> query;
    vector<vector<std::tuple<ll, ll, ll>>> resp(n);
    vector<ll> ans(q, -1);
    for (UV : irange(q)) {
        auto t = scanner.read<int>();
        if (t == 1) {
            auto [l, r, x] = scanner.read<ll, 3>();
            query.push_back({t, l - 1, r, x});
        } else if (t == 2) {
            auto [i, x] = scanner.read<ll, 2>();
            query.push_back({t, i - 1, x, -1});
        } else if (t == 3) {
            auto [i, j] = scanner.read<ll, 2>();
            query.push_back({t, i - 1, j - 1, -1});
        }
    }

    DualSegmentTree<ll, Add<ll>> seg(m, 0LL);
    for (int qi : reversed(irange(q))) {
        int t = query[qi][0];
        if (t == 1) {
            int l = query[qi][1];
            int r = query[qi][2];
            ...
            int i = query[qi][1];
            ll x = query[qi][2];
            for (auto [j, idx, offset] : resp[i]) {
                ans[idx] = x + seg[j] - offset;
            }
            resp[i].clear();
        } else if (t == 3) {
            int i = query[qi][1];
            int j = query[qi][2];
            resp[i].push_back({j, qi, seg[j]});
        }
    }
    for (int i : irange(n)) {
        for (auto [j, idx, offset] : resp[i]) {
            ans[idx] = seg[j] - offset;
        }
    }
