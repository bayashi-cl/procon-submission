...
void Solver::solve() {
    ...
    for (int j : irange(1, m + 1)) dp[0][j] = 1;
    for (int i : irange(1, n)) {
        for (int j : irange(1, m + 1)) dp[i - 1][j] += dp[i - 1][j - 1];

        for (int j : irange(1, m + 1)) {
            dp[i][j] += dp[i - 1][max(j - k, 0)];
            dp[i][j] += dp[i - 1][m] - dp[i - 1][min(m, j + k - 1)];
            if (k == 0) dp[i][j] -= dp[i - 1][j] - dp[i - 1][j - 1];
        }
    }
    DEBUG(dp);
    print(std::accumulate(dp.back().begin(), dp.back().end(), Mint(0)));
}
}  // namespace bys
