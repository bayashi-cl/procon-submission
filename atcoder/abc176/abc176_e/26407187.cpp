...
namespace bys {
void solve() {
    auto [h, w, m] = input<int, 3>();
    vector<set<int>> row(h);
    vector<Pa> cnt(w);
    for (int i = 0; i < w; ++i) cnt[i].second = i;

    for (int i = 0; i < m; ++i) {
        auto [hi, wi] = input<int, 2>();
        --hi, --wi;
        row[hi].insert(wi);
        cnt[wi].first++;
    }
    sort(std::begin(cnt), std::end(cnt), std::greater<Pa>());
    int ans = 0;
    for (int i = 0; i < h; ++i) {
        int n_row = row[i].size();
        for (auto [n_col, col] : cnt) {
            if (row[i].find(col) == row[i].end()) {
                chmax(ans, n_row + n_col);
                break;
            } else {
                chmax(ans, n_row + n_col - 1);
            }
        }
    }
    print(ans);
