...
void Solver::solve() {
    ...
    std::uniform_int_distribution<> dist(0, SIZE * 4 - 1);
    string dir = "RLUD";
    ...
    string init_ans;
    {
        ...
        for (int i : irange(path.size() - 1)) {
            ...
            if (diff == 1) {
                init_ans.push_back('R');
            } else if (diff == -1) {
                init_ans.push_back('L');
            } else if (diff == M) {
                init_ans.push_back('D');
            } else if (diff == -M) {
                init_ans.push_back('U');
            } else {
                throw RE;
            }
        }
    ...
    string ans;
    {
        int times = ceildiv(SIZE, (int)init_ans.length());
        for (auto ci : init_ans) {
            com += string(times, ci);
        }
        com = com.substr(0, SIZE);
        assert(com.length() == 200UL);
        ans = com;
    }
