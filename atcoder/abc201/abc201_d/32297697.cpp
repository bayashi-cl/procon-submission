...
void Solver::solve() {
    ...
    auto a = scanner.readvec<string>(h);
    ...
    for (int i : reversed(irange(h))) {
        for (int j : reversed(irange(w))) {
            if (i == h - 1 and j == w - 1) continue;
            if ((i + j) & 1) {  // takahashi
                int score = INF;
                if (i + 1 != h) chmin(score, dp[i + 1][j] - aa[i + 1][j]);
                if (j + 1 != w) chmin(score, dp[i][j + 1] - aa[i][j + 1]);
                dp[i][j] = score;
            } else {  // aoki
                int score = -INF;
                if (i + 1 != h) chmax(score, dp[i + 1][j] + aa[i + 1][j]);
                if (j + 1 != w) chmax(score, dp[i][j + 1] + aa[i][j + 1]);
                dp[i][j] = score;
            }
        }
    }
