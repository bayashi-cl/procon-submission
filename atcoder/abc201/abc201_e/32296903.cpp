...
void Solver::solve() {
    ...
    // COOMatrix<ll> es(n, n);
    // for (UV : irange(n - 1)) {
    //     auto [u, v, w] = scanner.read<Int1, Int1, ll>();
    //     es.append(u, v, w);
    //     es.append(v, u, w);
    // }
    ...
    while (not que.empty()) {
        auto top = que.front();
        que.pop();
        for (auto&& [dest, weight] : graph[top]) {
            if (cost[dest] == -1) {
                cost[dest] = cost[top] ^ weight;
                que.emplace(dest);
            }
        }
    }

    vector<int> bit_cnt(62);
    Mint7 ans = 0;
    // for (auto c : cost) {
    for (int i : irange(n)) {
        auto c = cost[i];
        for (int d : irange(62)) {
            if (pop(c, d)) {
                ans += Mint7(1LL << d) * (i - bit_cnt[d]);
                ++bit_cnt[d];
            } else {
                ans += Mint7(1LL << d) * bit_cnt[d];
            }
        }
    }
    print(ans);
