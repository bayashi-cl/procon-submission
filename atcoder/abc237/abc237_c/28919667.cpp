...
#line 4 "abc237_c/main.cpp"

namespace bys {
void Solver::solve() {
    auto s = scanner.read<string>();
    int la = std::find_if(s.begin(), s.end(), [](char c) { return c != 'a'; }) - s.begin();
    int ra = std::find_if(s.rbegin(), s.rend(), [](char c) { return c != 'a'; }) - s.rbegin();
    if (la > ra) EXIT("No");
    int n = s.length();
    string mid = s.substr(la, n - la - ra);

    auto mid_rev = mid;
    reverse(mid.begin(), mid.end());
    print(mid == mid_rev ? "Yes" : "No");
}
}  // namespace bys
