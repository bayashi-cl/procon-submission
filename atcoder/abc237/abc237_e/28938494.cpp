...
#line 5 "abc237_e/main.cpp"

namespace bys {
void Solver::solve() {
    auto [n, m] = scanner.read<int, 2>();
    auto h = scanner.readvec<ll>(n);
    Adj graph(n);
    for (UV : Range(m)) {
        auto [u, v] = scanner.read<Int1, 2>();
        if (h[u] < h[v]) std::swap(u, v);
        graph[u].push_back({v, -(h[u] - h[v])});
        graph[v].push_back({u, 2 * (h[u] - h[v])});
    }
    Dijkstra bf(graph, 0);
    print(-(*std::min_element(bf.cost.begin(), bf.cost.end())));
}
}  // namespace bys
