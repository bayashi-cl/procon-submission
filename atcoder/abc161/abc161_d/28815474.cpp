...
#line 4 "abc161_d/main.cpp"

namespace bys {
void Solver::solve() {
    auto k = scanner.read<int>();
    set<ll> lunlun = {1, 2, 3, 4, 5, 6, 7, 8, 9};
    while ((int)lunlun.size() < k) {
        vector<ll> add;
        for (auto&& li : lunlun) {
            ll t = li % 10;
            for (auto&& nxt : Range(max(t - 1, 0LL), min(t + 1, 9LL) + 1)) {
                add.push_back(li * 10 + (ll)nxt);
            }
        }
        for (auto&& ai : add) lunlun.insert(ai);
    }
    vector ans(lunlun.begin(), lunlun.end());
    print(ans[k - 1]);
}
}  // namespace bys
