...
void bys::solve() {
    auto [n, m, t] = input<int, 3>();
    auto a = input<int>(n);
    Dijkstra::Adj adj(n), adj_rev(n);
    ...
    Dijkstra dij(adj), dij_rev(adj_rev);
    dij.search(0);
    dij_rev.search(0);
    ll ans = 0;
    for (int i = 0; i < n; i++) {
        if (dij.cost[i] == LINF || dij_rev.cost[i] == LINF) continue;
        ll time = t - (dij.cost[i] + dij_rev.cost[i]);
        chmax(ans, time * a[i]);
    }
    print(ans);
}
