...
void Solver::solve() {
    ...
    for (auto d : div) {
        ...
        CumulativeSum<int> modcs(n), modcs_inv(n);
        for (auto [i, mi] : enumerate(mod)) {
            modcs.add(i, mi);
            modcs_inv.add(i, d - mi);
        }
        modcs.construct();
        modcs_inv.construct();
        for (int i : irange(n)) {
            if (modcs.sum(0, i) == modcs_inv.sum(i, n)) {
                if (modcs.sum(0, i) <= k) ans = d;
                break;
            }
        }
    }
