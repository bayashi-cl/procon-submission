...
void Solver::solve() {
    ...
    for (int i : irange(h)) {
        for (int j : irange(w)) {
            if (s[i][j] == 's') snuke = {i, j};
            if (s[i][j] == 'a') cargo = {i, j};
            if (s[i][j] == 'g') goal = {i, j};
        }
    }

    vector cost(g.area(), vector<int>(g.area(), INF));  // [cargo][snuke]
    cost[g.index(cargo)][g.index(snuke)] = 0;
    std::queue<pair<Pa, Pa>> que;
    que.push({cargo, snuke});

    while (not que.empty()) {
        auto [ca, su] = que.front();
        que.pop();

        auto now = cost[g.index(ca)][g.index(su)];
        auto [i, j] = su;

        for (auto [ni, nj] : g.next4(i, j)) {
            if (s[ni][nj] == '#') continue;
            if (Pa(ni, nj) == ca) {
                auto toi = ni + (ni - i);
                auto toj = nj + (nj - j);

                if (g.contain(toi, toj) and s[toi][toj] != '#') {
                    if (cost[g.index(toi, toj)][g.index(ca)] == INF) {
                        cost[g.index(toi, toj)][g.index(ca)] = now + 1;
                        que.push({{toi, toj}, ca});
                    }
                }
            } else {
                if (cost[g.index(ca)][g.index(ni, nj)] == INF) {
                    cost[g.index(ca)][g.index(ni, nj)] = now + 1;
                    que.push({ca, {ni, nj}});
                }
            }
        }
    ...
    auto ans = *std::min_element(cost[g.index(goal)].begin(), cost[g.index(goal)].end());
    print(ans == INF ? -1 : ans);
}
}  // namespace bys
