...
def main(**kwargs) -> None:
    ...
    wv = [tuple(map(int, readline().split())) for _ in range(n)]

    dp = [[[-IINF] * (b + 1) for _ in range(a + 1)] for _ in range(n + 1)]
    dp[0][0][0] = 0
    for i in range(n):
        w, v = wv[i]
        for j in range(a + 1):
            for k in range(b + 1):
                ...
                if j + w <= a:
                    dp[i + 1][j + w][k] = max(dp[i + 1][j + w][k], now + v)
                if k + w <= b:
                    dp[i + 1][j][k + w] = max(dp[i + 1][j][k + w], now + v)

    print(max(max(e) for e in dp[n]))
