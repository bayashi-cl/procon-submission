...
void Solver::solve() {
    ...
    using Node = pair<ll, Pos>;
    vector<Pa> delta = {{-1, 0}, {0, 1}, {1, 0}, {0, -1}};

    auto [h, w, k] = scanner.read<int, int, ll>();
    auto [x1, y1, x2, y2] = scanner.read<Int1, 4>();
    auto c = scanner.readvec<string>(h);

    Grid grid(h, w);
    std::priority_queue<Node, vector<Node>, std::greater<Node>> que;
    vector cost(h, vector(w, vector<ll>(4, LINF)));
    for (int i : irange(4)) {
        que.push({0, {x1, y1, i}});
        cost[x1][y1][i] = 0;
    }
    while (not que.empty()) {
        ...
        auto [i, j, dir] = top;
        que.pop();
        if (cost[i][j][dir] < top_cost) continue;
        for (auto ddir : irange(4)) {
            auto nxt_cost = cost[i][j][dir];
            auto ni = i, nj = j;
            auto ndir = (dir + ddir) % 4;
            if (ndir == dir) {
                ndir = (ndir + 2) % 4;
                ...
                nxt_cost = ceildiv(nxt_cost, k) * k;
            }

            if (grid.contain(ni, nj) and c[ni][nj] == '.' and nxt_cost < cost[ni][nj][ndir]) {
                cost[ni][nj][ndir] = nxt_cost;
                que.push({nxt_cost, {ni, nj, ndir}});
            }
        }
    }
    auto ans = LINF;
    for (auto i : irange(4)) chmin(ans, cost[x2][y2][i]);
    print(ans == LINF ? -1 : ceildiv(ans, k));
}
}  // namespace bys
