...
void Solver::solve() {
    ...
    auto infant = input<Pa>(n);
    const int m = 200000;

    vector<set<Pa>> kinder(m);
    for (int i = 0; i < n; ++i) {
        auto& [a, b] = infant[i];
        b--;
        kinder[b].insert({a, i});
    ...
    for (int i = 0; i < m; ++i) {
        if (!kinder[i].empty()) seg.set(i, kinder[i].rbegin()->first);
    }
    for (int i = 0; i < q; ++i) {
        auto [c, d] = input<int, 2>();
        c--, d--;
        int now = std::exchange(infant[c].second, d);
        Pa t = {infant[c].first, c};
        kinder[now].erase(t);
        if (kinder[now].empty()) {
            seg.set(now, INF);
        } else {
            seg.set(now, kinder[now].rbegin()->first);
        }
        kinder[d].insert(t);
        seg.set(d, kinder[d].rbegin()->first);
        print(seg.all_prod());
    }
}
}  // namespace bys
