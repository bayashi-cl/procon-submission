...
namespace bys {
void Solver::solve() {
    auto [n, m] = scanner.read<ll, 2>();
    auto bm = bit_width(m);
    if (n > bm) EXIT(0);

    vector dp(n + 1, vector<Mint>(bm + 1));
    for (int j : irange(bm + 1)) dp[0][j] = 1;
    vector<Mint> cnt(bm + 1);
    for (auto k : irange(1, bm + 1)) {
        cnt[k] = min(1LL << k, m + 1) - (1LL << (k - 1));
    }
    for (int i : irange(1, int(n) + 1)) {
        for (int j : irange(1, bm + 1)) {
            dp[i][j] = dp[i][j - 1] + dp[i - 1][j - 1] * cnt[j];
        }
    }
    print(dp[n][bm]);
}
