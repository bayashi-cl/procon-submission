...
proc tipArea(r, theta: float): float =
  let
    c = r^2 * PI * (theta / (2*PI))
    t = (r^2) * sin(theta) / 2
...
proc main =
  ...
  let
    circle = (d^2).toFloat * PI
    rec = (h * w).toFloat
  var ans: float

  if h < d * 2:
    let l = sqrt((d^2).toFloat - (h / 2)^2) * 2
    if w.toFloat < l:
      ans = 1.0
    else:
      var remCircle = circle
      let thetaH = ((h/2) / d.toFloat).arccos * 2
      remCircle -= tipArea(d.toFloat, thetaH) * 2

      if w < d * 2:
        let thetaW = ((w/2) / d.toFloat).arccos * 2
        remCircle -= tipArea(d.toFloat, thetaW) * 2
  ...
  print(ans.formatFloat(ffDecimal))
