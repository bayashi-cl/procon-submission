...
type Coin = tuple[g, s, c: int]

proc main =
  let
    n, x = read(int)
    bags = readSeq(n, tuple[a, b, c: int])

  var dp = newSeqWith(n+1, newSeq[Coin](x+1))
  dp[0][0] = (0, 10^9, x)

  for i in 1..n:
    for j in 0..x:
      let (ai, bi, ci) = bags[i - 1]
      dp[i][j].chmax(dp[i - 1][j])
      if j - bi >= 0:
        let p = dp[i - 1][j - bi]
        dp[i][j].chmax((p.g + ci, p.s - ai, p.c - bi))

  let ans = dp[^1].max()
  print(ans.g, ans.s, ans.c)
