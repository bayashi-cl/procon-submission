...
void solve() {
    ...
    for (int se = 0; se < (1 << (h - 1)); ++se) {
        ...
        int cut = __builtin_popcount(se);
        int hol = cut + 1;
        vector<int> white(hol);
        bool var_flg = false;
        ...
        for (int j = 0; j < w; ++j) {
            ...
            bool flg = false;
            for (int hi = 0; hi < hol; ++hi) {
                if (cnt[hi] > k) {
                    var_flg = true;
                    break;
                }
                if (white[hi] + cnt[hi] > k) flg = true;
            }
            if (var_flg) break;
            if (flg) {
                white = cnt;
                ...
                for (int hi = 0; hi < hol; ++hi) white[hi] += cnt[hi];
            }
        }
