...
namespace bys {
void Solver::solve() {
    auto [n, s] = scanner.read<int, 2>();
    auto a = scanner.readvec<int>(n);
    vector dp(n + 1, vector(s + 1, vector<Mint>(3)));
    dp[0][0][0] = 1;
    for (int i : irange(n)) {
        for (int j : irange(s + 1)) {
            for (int now : irange(3)) {
                for (int to : irange(now, 3)) {
                    dp[i + 1][j][to] += dp[i][j][now];
                    if (int nxt = j + a[i]; nxt <= s and now != 2 and to != 0) dp[i + 1][nxt][to] += dp[i][j][now];
                }
            }
        }
    }
