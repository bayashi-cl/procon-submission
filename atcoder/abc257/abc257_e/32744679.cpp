...
void Solver::solve() {
    ...
    vector<int> c(10);
    c[0] = INF;
    for (int i : irange(1, 10)) cin >> c[i];
    auto itr = std::min_element(c.begin(), c.end());

    int base = itr - c.begin();
    int sz = n / *itr;
    int rem = n % *itr;

    vector<int> ans(sz, base);
    for (int i : irange(sz)) {
        for (int d = 9; d > ans[i]; --d) {
            auto cost = -c[ans[i]] + c[d];
            if (cost <= rem) {
                ans[i] = d;
                rem -= cost;
                break;
