...
void Solver::solve() {
    ...
    auto res_inv = breadth_first_search(graph, n - 1);

    auto tele_start = LINF / 2, tele_goal = LINF / 2;
    for (auto si : s) {
        chmin(tele_start, res.cost[si] + 1);
        chmin(tele_goal, res_inv.cost[si] + 1);
    }

    vector<ll> ans;
    for (int i : irange(n)) {
        ll a = min(res.cost[n - 1], tele_start + tele_goal);
        if (auto fwd = res.cost[i]; fwd != LINF) chmin(a, fwd + tele_goal);
        if (auto inv = res_inv.cost[i]; inv != LINF) chmin(a, inv + tele_start);

        ans.push_back(a < INF ? a : -1LL);
    }
    print(ans);
}
}  // namespace bys
