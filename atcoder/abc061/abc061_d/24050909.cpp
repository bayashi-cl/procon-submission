...
template <class T, size_t N> inline std::array<T, N> iinput() {std::array<T, N> res; for (auto&& r : res) cin >> r; return res;}
template <class T> inline int ninput() {T n; cin >> n; return n;}
template <class T> inline vector<T> vinput(int n) {vector<T> res(n); for (auto&& r : res) cin >> r; return res;}
...
struct Edge {
    ...
    int to;
    int cost;
    Edge(int from, int to, int cost) : from(from - 1), to(to - 1), cost(cost) {}
};

void main() {
    auto [n, m] = iinput<int, 2>();
    vector<Edge> graph;
    for (int i = 0; i < m; i++) {
        auto [a, b, c] = iinput<int, 3>();
