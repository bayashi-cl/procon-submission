...
void Solver::solve() {
    ...
    COOMatrix<Pall> es(n, n);
    for (UV : irange(m)) {
        auto [a, b] = scanner.read<Int1, 2>();
        auto cd = scanner.read<Pall>();
        es.append(a, b, cd);
        es.append(b, a, cd);
    }
    ...
    while (!que.empty()) {
        ...
        for (auto&& [dest, cd] : graph[top]) {
            auto [c, d] = cd;
            ll t_star = (ll)std::round(std::sqrt(d)) - 1;
            ll next_cost = LINF;
            for (ll t : irange(t_star - 3, t_star + 3)) {
                chmax(t, top_cost);
                chmin(next_cost, t + c + d / (t + 1));
            }
            if (next_cost < cost[dest]) {
                cost[dest] = next_cost;
                que.push({next_cost, dest});
            }
        }
    }
    print(cost[n - 1] != LINF ? cost[n - 1] : -1);
}
}  // namespace bys
