...
#line 4 "/home/bayashi/dev/byslib/graphv2/result.hpp"
namespace bys {
/**
...
#line 7 "/home/bayashi/dev/byslib/graphv2/breadth_first_search.hpp"
namespace bys {
...
#line 6 "abc132_e/main.cpp"

namespace bys {
void Solver::solve() {
    auto [n, m] = scanner.read<int, 2>();
    Grid g(n, 3);
    AdjacencyList graph(n * 3, m * 3);
    for (UV : irange(m)) {
        auto [u, v] = scanner.read<Int1, 2>();
        graph.add_edge(g.index(u, 0), g.index(v, 1));
        graph.add_edge(g.index(u, 1), g.index(v, 2));
        graph.add_edge(g.index(u, 2), g.index(v, 0));
    }
    assert(graph.build_flg());
    auto [s, t] = scanner.read<Int1, 2>();
    auto res = breadth_first_search(graph, g.index(s, 0));
    auto ans = res.cost[g.index(t, 0)];
    print(ans == LINF ? -1 : ans / 3);
}
