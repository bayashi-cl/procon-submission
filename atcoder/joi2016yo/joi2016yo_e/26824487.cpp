...
struct Dijkstra {
    ...
    void search(const Adj& graph, int start) {
        ...
        while (!que.empty()) {
            Edge top = que.top();
            que.pop();
            if (cost[top.to] < top.cost) continue;
            for (auto&& next : graph[top.to]) {
                ll next_cost = cost[top.to] + next.cost;
                if (next_cost < cost[next.to]) {
                    cost[next.to] = next_cost;
                    prev[next.to] = top.to;
                    que.push(Edge(next.to, next_cost));
                }
            }
        }
    }
...
void solve() {
    ...
    while (!que.empty()) {
        ...
        for (auto&& next : graph[now]) {
            if (zdist[next.to] == INF) {
                zdist[next.to] = zdist[now] + 1;
                que.push(next.to);
            }
        }
    }
    set<int> zombi(std::begin(c), std::end(c));
    Adj graph2(n);

    for (int from = 0; from < n; ++from) {
        for (auto&& e : graph[from]) {
            if (zombi.find(from) != zombi.end() ||
                zombi.find(e.to) != zombi.end()) {
                continue;
            }
            int cost = zdist[e.to] > s ? p : q;
            if (e.to == n - 1) cost = 0;
            graph2[from].push_back(Edge(e.to, cost));
        }
    }

    Dijkstra dij(graph2, 0);
    print(dij.cost[n - 1]);
}
}  // namespace bys
