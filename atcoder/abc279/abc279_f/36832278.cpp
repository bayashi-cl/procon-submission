...
void Solver::solve() {
    ...
    for (UV : irange(q)) {
        ...
        if (t == 1) {
            ...
            if (leader[x] == -1 and leader[y] == -1) continue;
            auto l = [&]() -> int {
                if (leader[x] != -1 and leader[y] != -1) {
                    uf.unite(leader[x], leader[y]);
                    return uf.find(leader[x]);
                } else if (leader[x] == -1) {
                    return leader[y];
                } else {
                    return leader[x];
            ...
            auto x = scanner.read<int1>();
            auto l = [&]() -> int {
                if (leader[x] == -1) {
                    return cnt;
                } else {
                    uf.unite(leader[x], cnt);
                    return uf.find(cnt);
                }
            }();
            leader[x] = l;
            ...
            auto x = scanner.read<int1>();
            print(box[uf.find(x)] + 1);
        }
#ifdef LOCAL
        vector<vector<int>> b(n);
        for (int i : irange(cnt)) {
            b[box[uf.find(i)]].push_back(i + 1);
        }
        for (int i : irange(n)) {
            DEBUG(i + 1, ": ", b[i]);
        }
