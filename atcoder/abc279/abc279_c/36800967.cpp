...
void Solver::solve() {
    ...
    vector<string> s_rot, t_rot;
    for (int j : irange(w)) {
        string s_row, t_row;
        for (int i : irange(h)) {
            s_row.push_back(s[i][j]);
            t_row.push_back(t[i][j]);
        }
        s_rot.push_back(s_row);
        t_rot.push_back(t_row);
    }

    sort(s_rot.begin(), s_rot.end());
    sort(t_rot.begin(), t_rot.end());

    print(s_rot == t_rot ? "Yes" : "No");
}
}  // namespace bys
