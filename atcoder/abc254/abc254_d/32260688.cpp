...
struct Osa_KConstexpr {
    std::array<int, N + 1> spf = {0};
    constexpr Osa_KConstexpr() {
        for (int i = 1; i <= N; ++i) spf[i] = i & 1 ? i : 2;
...
void Solver::solve() {
    ...
    vector<ll> sq;
    // Osa_K o(n + 1);
    Osa_KConstexpr<200000> o;
    for (ll i = 1; i <= n; ++i) {
        sq.push_back(i * i);
    }
    for (ll i = 1; i <= n; ++i) {
        auto fac = o.factorize(i);
        ll p = 1;
        for (auto [f, cnt] : fac) {
            if (cnt & 1) p *= f;
        }
        ll th = (n / p);
        ans += std::upper_bound(sq.begin(), sq.end(), th) - sq.begin();
    }
    print(ans);
}
}  // namespace bys
