...
struct Gcd : Magma<T> {
    using typename Magma<T>::set_type;
    static constexpr set_type operation(set_type a, set_type b) { return std::gcd(a, b); }
    static inline constexpr set_type identity{0};
};
void Solver::solve() {
    auto [n, q] = scanner.read<int, 2>();
    auto a = scanner.readvec<int>(n);
    auto b = scanner.readvec<int>(n);
    vector<int> a_diff, b_diff;
    for (int i : irange(n - 1)) {
        a_diff.push_back(a[i + 1] - a[i]);
        b_diff.push_back(b[i + 1] - b[i]);
    }
    SparseTable<Gcd<int>> st_a(a_diff), st_b(b_diff);
    for (UV : irange(q)) {
        auto [h1, h2, w1, w2] = scanner.read<Int1, 4>();
        auto gcd_diff = std::gcd(st_a.query(h1, h2), st_b.query(w1, w2));
        print(std::gcd(a[h1] + b[w1], gcd_diff));
    }
}
}  // namespace bys
