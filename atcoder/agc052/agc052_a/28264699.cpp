...
#line 3 "agc052_a/main.cpp"

namespace bys {
set<int> opos(const string& s) {
    int o = 0;
    set<int> res;
    if (s[0] == '0') res.insert(-1);
    for (int i : Range(s.length() - 1)) {
        if (s[i] == '1') {
            if (s[i + 1] == '0') res.insert(o);
            ++o;
        }
    }
    return res;
...
void Solver::solve() {
    ...
    // auto o3 = opos(s3 + s3);
    // for (int i : Range(2 * n)) {
    //     if (o1.find(i) != o1.end() && o2.find(i) != o2.end() &&
    //         o3.find(i) != o3.end()) {
    //         string ans(i + 1, '1');
    //         ans.push_back('0');
    //         ans += string(2 * n - (i + 1), '1');
    //         print(ans);
    //         break;
    //     }
    // }
