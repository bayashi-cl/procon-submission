static:
  when not defined(lazyCompile):
    ...
    writeModule("byslib/algo/compress.nim"):
      import sequtils, algorithm, sets
      
      type Compress* = ref object
        s: seq[int]
      
      proc initCompress*(v: seq[int]): Compress =
        return Compress(s: v.toHashSet.toSeq.sorted)
      
      proc zip*(self: Compress, x: int): int = self.s.binarySearch(x)
      proc unziup*(self: Compress, idx: int): int = self.s[idx]
      proc len*(self: Compress): int = self.s.len
...
proc main =
  ...
  var right = newSeqWith(cp.len, -1)
  for i, ai in a[0..<k]: right[cp.zip(ai)] = i
  for i in 1..right.high: right[i].chmax(right[i - 1])
