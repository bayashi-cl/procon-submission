...
void Solver::solve() {
    ...
    auto head = scanner.readvec<int>(k);
    auto tail = scanner.readvec<int>(n - k);

    vector<Pa> h, t;
    for (auto [i, hi] : cenumerate(head)) h.emplace_back(hi, i);
    for (auto [i, ti] : cenumerate(tail)) t.emplace_back(ti, i + k);
    sort(h.begin(), h.end(), std::greater<Pa>());
    sort(t.begin(), t.end());
    int ans = INF;
    int right = INF;
    for (auto [hi, i] : h) {
        while (!t.empty() and t.back().first > hi) {
            chmin(right, t.back().second);
            t.pop_back();
        }
        if (right == INF) continue;
        int score = (k - i - 1) + (right - k) + 1;
        chmin(ans, score);
    }
    print(ans > 2 * n ? -1 : ans);
}
