...
void Solver::solve() {
    ...
    vector<Pa> ap;
    for (auto [i, ai] : cenumerate(a)) ap.emplace_back(ai, i);
    sort(ap.begin(), ap.end(), std::greater<Pa>());

    vector<int> pick(n + 1, 1);
    ll score = 0;
    for (int i : irange(n / 2)) {
        score += ap[i].first;
        pick[ap[i].second + 1] = -1;
    }
    pick[0] = 0;
    std::partial_sum(pick.begin(), pick.end(), pick.begin());
    auto top = std::min_element(pick.begin(), pick.end()) - pick.begin();
