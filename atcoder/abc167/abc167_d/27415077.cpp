...
#line 2 "abc167_d/main.cpp"

namespace bys {
pair<vector<int>, vector<int>> calc(Vec& dist, int start) {
    int n = dist.size();
    Vec pos(n, -1);
    Vec path, loop;
    int now = start;
    [[maybe_unused]] bool flg;
    for (int i = 0; i < n; ++i) {
        pos[now] = i;
        path.push_back(now);
        int to = dist[now];
        if (pos[to] != -1) {
            auto entry_itr = path.begin() + pos[to];
            std::copy(entry_itr, path.end(), std::back_inserter(loop));
            path.erase(entry_itr, path.end());
            flg = true;
            break;
        }
        now = to;
...
void solve() {
    auto [n, k] = input<int, ll>();
    auto a = input<int>(n);
    for (auto&& ai : a) --ai;
    auto [path, loop] = calc(a, 0);
    ll path_len = path.size();
    if (k < path_len) {
        print(path[k] + 1);
    } else {
        k -= path_len;
