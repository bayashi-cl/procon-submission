...
namespace bys {
void Solver::solve() {
    auto h = scanner.readvec<int>(3);
    auto w = scanner.readvec<int>(3);
    int ans = 0;
    for (int a : irange(1, h[0] + 1)) {
        for (int b : irange(1, h[0] + 1)) {
            int c = h[0] - (a + b);
            if (c <= 0) continue;

            for (int d : irange(1, h[1] + 1)) {
                for (int e : irange(1, h[1] + 1)) {
                    int f = h[1] - (d + e);
                    if (f <= 0) continue;
                    ...
                    int q = w[1] - (b + e);
                    int r = w[2] - (c + f);
                    if (p > 0 and q > 0 and r > 0 and p + q + r == h[2]) ++ans;
                }
            }
        }
    }
