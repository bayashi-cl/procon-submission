...
void main() {
    ...
    auto ma_itr = std::max_element(std::begin(a), std::end(a));
    int ma = *ma_itr;
    int ma_idx = std::distance(a.begin(), ma_itr);

    auto mi_itr = std::min_element(std::begin(a), std::end(a));
    int mi = *mi_itr;
    int mi_idx = std::distance(a.begin(), mi_itr);

    vector<Pa> ans;
    if (abs(ma) >= abs(mi)) {
        for (auto&& i : irange(n)) {
            if (a[i] < 0) ans.push_back({ma_idx + 1, i + 1});
        }
        ...
        for (auto&& i : irange(n)) {
            if (a[i] > 0) ans.push_back({mi_idx + 1, i + 1});
        }
        for (int i = n; i > 1; --i) {
            ans.push_back({i, i - 1});
        }
