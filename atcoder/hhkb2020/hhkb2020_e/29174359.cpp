...
#line 6 "hhkb2020_e/main.cpp"

namespace bys {
void Solver::solve() {
    auto [h, w] = scanner.read<int, 2>();
    auto s = scanner.readvec<string>(h);
    vector row(h, UnionFindTree(w)), col(w, UnionFindTree(h));
    vector<mint7> table(h * w + 1);
    table[0] = 1;
    for (int i : irange(1, h * w + 1)) table[i] = table[i - 1] * 2;
    int space = 0;
    for (int i : irange(h)) {
        for (int j : irange(w)) {
            if (s[i][j] == '#') continue;
            ++space;
            if (i + 1 != h && s[i + 1][j] == '.') col[j].unite(i, i + 1);
            if (j + 1 != w && s[i][j + 1] == '.') row[i].unite(j, j + 1);
        }
    }
    mint7 ans = 0;
    for (int i : irange(h)) {
        for (int j : irange(w)) {
            if (s[i][j] == '#') continue;
            int sz = row[i].size(j) + col[j].size(i) - 1;
            ans += (table[sz] - 1) * table[space - sz];
        }
    }
    print(ans);
}
