...
int edist(const Pa& a, const Pa& b) {
    int dx = b.first - a.first;
    int dy = b.second - a.second;
    double d = std::round(std::hypot(dx, dy));
    return std::round(d * 1.5);
}
pair<int, vector<bool>> kruskal(vector<Edge>& graph) {
    atcoder::dsu uft(N);
    vector<bool> ans(M);
    // int sz = N;
    int cost = 0;
    for (auto&& edge : graph) {
        if (edge.sel != 1 && uft.same(edge.u, edge.v)) continue;
        if (edge.sel == 0) return {INF, ans};
        uft.merge(edge.u, edge.v);
        ans[edge.id] = true;
        cost += edge.cost;
        // --sz;
        // if (sz == 1) break;
...
void Solver::solve() {
    ...
    for (int i : Range(M)) {
        auto [u, v] = uv[i];
        int d = edist(xy[u], xy[v]);
        edge_list.push_back({i, u, v, d, -1});
    }
    vector<Edge> edge_list_sorted(edge_list);
    sort(edge_list_sorted.begin(), edge_list_sorted.end());
    auto [cost, ans] = kruskal(edge_list_sorted);
    for (int i : Range(M)) {
        auto li = input<int>();
        edge_list[i].cost = li;
        edge_list_sorted = edge_list;
        if (!std::is_sorted(edge_list_sorted.begin(), edge_list_sorted.end())) {
            sort(edge_list_sorted.begin(), edge_list_sorted.end());
        }
        auto [cost_i, ans_i] = kruskal(edge_list_sorted);
        if (chmin(cost, cost_i)) ans = ans_i;
        if (ans[i]) {
            cout << 1 << endl;
            edge_list[i].sel = 1;
        } else {
