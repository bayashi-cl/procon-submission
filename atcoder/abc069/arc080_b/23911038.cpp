...
Pa paint(Pa now, int color, VecVec& ans, const int w) {
    ans[now.first][now.second] = color;
    if (now.second == w - 1) {
        return {now.first + 1, 0};
    } else {
...
void main() {
    ...
    for (auto&& i : irange(1, n + 1)) {
        int ai = a[i - 1];
        for (int j = 0; j < ai; j++) {
            pos = paint(pos, i, ans, w);
        }
    }
    for (auto&& i : irange(h)) {
        if (i % 2) {
            std::reverse(std::begin(ans[i]), std::end(ans[i]));
            print(ans[i]);
        } else {
            print(ans[i]);
        }
