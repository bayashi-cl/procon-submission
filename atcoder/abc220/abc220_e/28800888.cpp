...
#line 5 "abc220_e/main.cpp"

namespace bys {
void Solver::solve() {
    auto [n, D] = scanner.read<int, 2>();
    mint ans = 0;
    vector<mint> p(max(D + 1, n));
    p[0] = 1;
    for (auto&& i : Range(1, (int)p.size())) p[i] = p[i - 1] * 2;

    for (int d : Range(n - 1)) {
        mint cnt = 0;
        if (d + D < n) cnt += p[D];

        int mi_a = max(D + d - (n - 1), 1);
        int ma_a = min((n - 1) - d, D - 1);
        int a = max(ma_a - mi_a + 1, 0);
        cnt += p[D - 2] * a;
        cnt *= 2;
