...
void Solver::solve() {
    ...
    i64 biper_vertex = 0;
    map<int, bool> is_bipar;
    for (auto&& [leader, members] : groups) {
        bool bipar = true;
        FixPoint rec([&](auto&& self, i32 now, i32 c) -> void {
            color[now] = c;
    ...
    for (auto&& [leader, members] : groups) {
        ...
        for (auto m : members) {
            if (color[m] == 0) {
                ans1 += black - graph[m].size();
            }
        }
        ans2 += (biper_vertex - members.size()) * members.size();
    }
    if (ans2 % 2 != 0) throw RE;
    print(ans1 + ans2 / 2);
}
}  // namespace bys
