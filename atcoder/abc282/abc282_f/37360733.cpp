...
template <class Band> class SparseTable {
  ...
  public:
    ...
    void build(const std::vector<T>& v) {
        ...
        for (int i : irange(n)) ans1.push_back({i, i + 1});
        ...
        int cnt = table[0].back();
        for (i32 i = 1; i <= max_k; ++i) {
            for (i32 j = 0; j <= n - (1 << i); ++j) {
                ++cnt;
                // print(i, i + (1 << j));
                ans1.push_back({j, j + (1 << i)});
                table[i][j] = cnt;
    ...
    T fold(i32 l, i32 r) {
        ...
        i32 w = r - l;
        // print(id[{lookup[w], l}], id[{lookup[w], r - (1 << lookup[w])}]);
        print(table[lookup[w]][l], table[lookup[w]][r - (1 << lookup[w])]);
...
void Solver::solve() {
    ...
    vector<int> dummy(n);
    std::iota(dummy.begin(), dummy.end(), 1);
    SparseTable<Max<i32>> st(dummy);
    print(st.ans1.size());
    for (auto e : st.ans1) print(e.first + 1, e.second);
