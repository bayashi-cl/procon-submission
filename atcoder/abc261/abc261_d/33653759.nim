static:
  when not defined(lazyCompile):
    ...
    writeModule("byslib/core/intops.nim"):
      template `|`*[T: SomeInteger](a, b: T): untyped = a or b
      template `&`*[T: SomeInteger](a, b: T): untyped = a and b
      template `%`*[T: SomeInteger](a, b: T): untyped = a mod b
      template `//`*[T: SomeInteger](a, b: T): untyped = a div b
      template `<<`*[T: SomeInteger](a, b: T): untyped = a shl b
      template `>>`*[T: SomeInteger](a, b: T): untyped = a shr b
      
      template `|=`*[T: SomeInteger](a: T, b: T): untyped = a = a or b
      template `&=`*[T: SomeInteger](a: T, b: T): untyped = a = a and b
      template `%=`*[T: SomeInteger](a: T, b: T): untyped = a = a mod b
      template `//=`*[T: SomeInteger](a: T, b: T): untyped = a = a div b
      template `<<=`*[T: SomeInteger](a: T, b: T): untyped = a = a shl b
      template `>>=`*[T: SomeInteger](a: T, b: T): untyped = a = a shr b
...
proc main =
  ...
  var dp = newSeqWith(n + 1, newSeqWith(n + 1, -LInf))
  dp[0][0] = 0
  for i in 0..<n:
    for j in 0..<n:
      chmax(dp[i + 1][0], dp[i][j] + b[0])
      chmax(dp[i + 1][j + 1], dp[i][j] + b[j+1] + x[i])

  print(max(dp[dp.high]))
