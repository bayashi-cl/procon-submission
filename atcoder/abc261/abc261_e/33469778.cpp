...
namespace bys {
void Solver::solve() {
    auto [n, c] = scanner.read<int, 2>();
    auto op = scanner.readvec<Pa>(n);

    vector<vector<Pa>> dp(n + 1, vector<Pa>(31, {0, 1}));
    for (int i : irange(n)) {
        auto [t, a] = op[i];
        for (int j : irange(31)) {
            int b = pop(a, j) ? 1 : 0;
            if (t == 1) {
                dp[i + 1][j] = {dp[i][j].first & b, dp[i][j].second & b};
            } else if (t == 2) {
                dp[i + 1][j] = {dp[i][j].first | b, dp[i][j].second | b};
            } else {
                dp[i + 1][j] = {dp[i][j].first ^ b, dp[i][j].second ^ b};
    ...
    for (int i : irange(1, n + 1)) {
        int xx = 0;
        for (int d : irange(31)) {
            xx |= (pop(x, d) ? dp[i][d].second : dp[i][d].first) << d;
        }
