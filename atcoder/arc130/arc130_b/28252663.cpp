...
#line 2 "arc130_b/main.cpp"

namespace bys {
void Solver::solve() {
    auto [h, w, c, q] = input<int, 4>();
    auto que = input<array<int, 3>>(q);
    reverse(que.begin(), que.end());
    set<int> painted_row, painted_col;
    vector<ll> ans(c);
    for (auto&& [t, n, ci] : que) {
        --ci;
        if (t == 1) {
            if (painted_row.find(n) == painted_row.end()) {
                ans[ci] += w - (int)painted_col.size();
                painted_row.insert(n);
            }
        } else {
            if (painted_col.find(n) == painted_col.end()) {
                ans[ci] += h - (int)painted_row.size();
                painted_col.insert(n);
            }
        }
    }
    print(ans);
