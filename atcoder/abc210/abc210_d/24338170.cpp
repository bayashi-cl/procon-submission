...
namespace bys {
ll calc(vector<vector<ll>>& a, int h, int w, int c) {
    ...
    for (int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {
            ll x = LINF;
            if (i > 0) chmin(x, dp[i - 1][j] + c + a[i][j]);
            if (j > 0) chmin(x, dp[i][j - 1] + c + a[i][j]);
            chmin(res, x);
        }
    }
    return res;
...
void solve() {
    auto [h, w, c] = input<ll, 3>();
    vector<vector<ll>> a(h);
    for (auto&& ai : a) ai = input<ll>(w);

    ll ans1 = calc(a, h, w, c);
    for (auto&& ai : a) std::reverse(std::begin(ai), std::end(ai));
    ll ans2 = calc(a, h, w, c);
