...
namespace bys {
void Solver::solve() {
    auto [h, w, c] = scanner.read<int, int, ll>();
    auto a = scanner.readvec<ll>(h, w);

    auto ans = LINF;
    for (UV : irange(2)) {
        vector dp(h, vector(w, LINF));  // {start, stop, rail}
        for (int i : irange(h)) {
            for (int j : irange(w)) {
                if (i != 0) {
                    auto prev = min(a[i - 1][j], dp[i - 1][j]);
                    chmin(ans, prev + c + a[i][j]);
                    chmin(dp[i][j], prev + c);
                }
                if (j != 0) {
                    auto prev = min(a[i][j - 1], dp[i][j - 1]);
                    chmin(ans, prev + c + a[i][j]);
                    chmin(dp[i][j], prev + c);
                }
            }
