...
namespace bys {
double dist(pair<double, double> s, pair<double, double> t) {
...
void solve() {
    ...
    auto cn = input<pair<pair<double, double>, double>>(n);
    auto cm = input<pair<double, double>>(m);

    double ans = 1e9;
    for (int i = 0; i < n; ++i) chmin(ans, cn[i].second);

    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            double d = dist(cn[i].first, cm[j]);
            chmin(ans, d - cn[i].second);
        }
    }

    for (int i = 0; i < m; ++i) {
        for (int j = i + 1; j < m; ++j) {
            double d = dist(cm[i], cm[j]);
            chmin(ans, d / 2.0);
        }
    }
    print(ans);
