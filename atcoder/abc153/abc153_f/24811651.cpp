...
void solve() {
    auto [n, d, a] = input<ll, 3>();
    auto xh = input<pair<ll, ll>>(n);
    sort(std::begin(xh), std::end(xh));
    vector<ll> cs(n + 1);
    ll ans = 0;
    for (int i = 0; i < n; ++i) {
        auto [x, h] = xh[i];
        pair<ll, ll> th = {x + 2 * d, LINF};
        auto itr = std::upper_bound(std::begin(xh), std::end(xh), th);
        int dist = std::distance(xh.begin(), itr);
        ll bomb = iceil(max(h - cs[i], 0LL), a);
        ans += bomb;
        cs[i] += bomb * a;
        cs[dist] -= bomb * a;
        cs[i + 1] += cs[i];
