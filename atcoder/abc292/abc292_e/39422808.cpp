...
namespace bys {
template <class E, class V = typename E::vertex_type> std::vector<bool> reachable(EdgesCSR<E> const& graph, V source) {
...
void Solver::solve() {
    ...
    auto edges = EdgeList(n, directed);
    for (UV : irange(m)) {
    ...
    for (auto i : irange(n)) {
        ...
        ans += std::count(res.begin(), res.end(), true) - 1;
    }
    print(ans - m);
}
}  // namespace bys
