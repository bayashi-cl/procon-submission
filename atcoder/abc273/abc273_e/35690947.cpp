...
void Solver::solve() {
    ...
    for (UV : irange(q)) {
        ...
        if (t == "ADD") {
            ...
            nodes.push_back({now, x});
            now = new_index;
        } else if (t == "DELETE") {
            if (now != 0) now = nodes[now].first;
        } else if (t == "SAVE") {
            auto y = scanner.read<int>();
            note[y] = now;
        } else if (t == "LOAD") {
            auto z = scanner.read<int>();
        ...
        ans.push_back(nodes[now].second);
    }
    print(ans);
}
}  // namespace bys
