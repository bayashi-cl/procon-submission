...
void Solver::solve() {
    ...
    auto [h, w, r, c] = scanner.read<int, 4>();
    auto n = scanner.read<int>();
    for (UV : irange(n)) {
        auto [ri, ci] = scanner.read<int, 2>();
        row[ri].push_back(ci);
        col[ci].push_back(ri);
    }
    for (auto&& e : row) sort(e.second.begin(), e.second.end());
    for (auto&& e : col) sort(e.second.begin(), e.second.end());

    auto q = scanner.read<int>();
    for (UV : irange(q)) {
        auto [d, l] = scanner.read<char, int>();
        if (d == 'L') {
            int lim;
            if (auto v_itr = row.find(r); v_itr == row.end()) {
                lim = 1;
            } else {
                auto itr = std::lower_bound(v_itr->second.begin(), v_itr->second.end(), c);
                if (itr == v_itr->second.begin()) {
                    lim = 1;
                } else {
                    lim = *std::prev(itr) + 1;
                }
            ...
            int lim;
            if (auto v_itr = col.find(c); v_itr == col.end()) {
                lim = 1;
            } else {
                auto itr = std::lower_bound(v_itr->second.begin(), v_itr->second.end(), r);
                if (itr == v_itr->second.begin()) {
                    lim = 1;
                } else {
                    lim = *std::prev(itr) + 1;
