...
void Solver::solve() {
    ...
    vector connect(n, vector<bool>(n));
    for (UV : irange(m)) {
    ...
    for (int s : irange(1 << n)) {
        vector<int> pops;
        for (int d : irange(n)) {
            if (pop(s, d)) pops.push_back(d);
        }

        bool flg = true;
        int sz = pops.size();
        for (auto i : irange(sz)) {
            for (auto j : irange(i + 1, sz)) {
                if (not connect[pops[i]][pops[j]]) flg = false;
            }
        }
        if (flg) clique[s] = true;
    }
    ...
    for (int s : irange(1 << n)) {
        ...
        int t = (1 << n) - 1 - s;
        for (int b = t; b != 0; b = (b - 1) & t) {
            if (clique[b]) chmin(dp[s | b], dp[s] + 1);
        }
    }
    print(dp.back());
}
