...
struct Grid {
    ...
    //! @brief 上下左右
    auto next4(i32 row, i32 col) const { return next(row, col, {{1, 0}, {-1, 0}, {0, 1}, {0, -1}}); }
    //! @brief 8方向
...
void Solver::solve() {
    ...
    atcoder::mf_graph<i32> graph(g.area() + 2);
    i32 source = n * m, sink = n * m + 1;
    for (auto i : irange(n)) {
        for (auto j : irange(m)) {
            if (s[i][j] == '#') continue;
            if ((i + j) % 2 == 0) {
                graph.add_edge(source, g.index(i, j), 1);
                for (auto [ni, nj] : g.next4(i, j)) {
                    if (s[ni][nj] == '.') {
                        graph.add_edge(g.index(i, j), g.index(ni, nj), 1);
                    }
                }
            } else {
                graph.add_edge(g.index(i, j), sink, 1);
            }
        }
    }
    ...
    for (auto&& e : graph.edges()) {
        ...
        if (e.flow == 1) {
            auto [fi, fj] = g.coord(e.from);
            auto [ti, tj] = g.coord(e.to);
            if (fi == ti) {
                if (fj > tj) std::swap(fj, tj);
                s[fi][fj] = '>';
                s[ti][tj] = '<';
            } else if (fj == tj) {
