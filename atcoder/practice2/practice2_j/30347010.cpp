...
class SegmentTree {
   ...
   public:
    ...
    // f(query(l, r)) == false となる最小のr
    int bisect_from_left(int l, std::function<bool(value_type)> f) const {
        assert(0 <= l && l < _n);
        ...
        do {
            l /= l & -l;
            while (l % 2 == 0) l >>= 1;
            if (!f(Monoid::operation(sm, data[l]))) {
                while (l < n_leaf) {
                    l *= 2;
                    if (value_type op = Monoid::operation(sm, data[l]); f(op)) {
                        sm = op;
                        l++;
                    }
                }
    ...
    // int bisect_from_right(int r, std::function<bool(value_type)> f) const {}
};
}  // namespace bys
#include <optional>
namespace bys {
...
void Solver::solve() {
    ...
    SegmentTree<Max<int>> seg(a);
    for (UV : irange(q)) {
        auto t = scanner.read<int>();
        if (t == 1) {
            auto [x, v] = scanner.read<Int1, int>();
            seg.update(x, v);
        } else if (t == 2) {
            ...
            auto [x, v] = scanner.read<Int1, int>();
            print(seg.bisect_from_left(x, [&](int p) { return p < v; }) + 1);
        }
    }
}
}  // namespace bys
