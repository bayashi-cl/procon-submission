...
namespace bys {
template <class W> class WeightedUnionFindTree {
    using weight_type = W;
    i32 _n_tree;
    //! Size if it is root else parnet.
    std::vector<i32> parent_or_size;
    std::vector<weight_type> diff_weight;

  public:
    WeightedUnionFindTree(i32 n) : _n_tree(n), parent_or_size(n, -1), diff_weight(n) {}

    bool unite(i32 a, i32 b, weight_type w) {
        w = w + weight(a) - weight(b);
        a = root(a);
        b = root(b);

        if (a == b) {
            if (w != 0) diff_weight[a] = get_inf<weight_type>();
            return false;
        }
    ...
    bool same(i32 a, i32 b) { return root(a) == root(b); }
    ...
    i32 root(i32 x) {
        if (parent_or_size[x] >= 0) {
            ...
            std::for_each(path.rbegin(), path.rend(), [&](i32 p) {
                if (is_inf(diff_weight[parent_or_size[p]])) {
                    diff_weight[p] = get_inf<weight_type>();
                } else {
                    diff_weight[p] += diff_weight[parent_or_size[p]];
                }
                parent_or_size[p] = x;
            });
        }
    ...
    i32 size(i32 x) { return -parent_or_size[root(x)]; }
    i32 n_tree() const { return _n_tree; }

    weight_type weight(i32 x) {
        root(x);
        return diff_weight[x];
    }
    weight_type diff(i32 a, i32 b) {
        auto wa = weight(a), wb = weight(b);
        if (is_inf(wa) or is_inf(wb)) {
            return get_inf<weight_type>();
        } else {
            return wb - wa;
        }
    }
...
void Solver::solve() {
    auto [n, m, q] = scanner.read<i32, 3>();
    WeightedUnionFindTree<i64> uf(n);
    for (UV : irange(m)) {
        auto [a, b, c] = scanner.read<i32_1, i32_1, i64>();
        uf.unite(a, b, c);
    }

    for (UV : irange(q)) {
        auto [x, y] = scanner.read<i32_1, 2>();
        if (not uf.same(x, y)) {
            print("nan");
        } else {
            auto ans = uf.diff(x, y);
            is_inf(ans) ? print("inf") : print(ans);
        }
    }
}
