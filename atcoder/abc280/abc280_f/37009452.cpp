...
void Solver::solve() {
    ...
    for (UV : irange(m)) {
        ...
        edges.add_edge(b, a, -c);
    }
    auto graph = edges.build();

    map<i32, bool> cycle;
    vector<i64> cost(n, LINF);
    for (auto&& [l, ch] : uf.groups()) {
        std::queue<i32> que;
        que.push(l);
        cost[l] = 0;
        bool cy = false;
        ...
        while (not que.empty()) {
            ...
            for (auto&& e : graph[top]) {
                if (is_inf(cost[e.dest])) {
                    cost[e.dest] = cost[top] + e.weight;
                    que.push(e.dest);
                } else if (cost[e.dest] != cost[top] + e.weight) {
                    cy = true;
                    break;
                }
            }
