...
void Solver::solve() {
    struct Edge {
        int dest, weight;
        ll k;
        Edge(int dest, int weight, ll k) : dest(dest), weight(weight), k(k) {}
    };
    auto [n, m, x, y] = scanner.read<int, int, Int1, Int1>();
    vector graph(n, vector<Edge>());
    vector<ll> k(m);
    for (UV : irange(m)) {
        auto [a, b, t, k] = scanner.read<Int1, Int1, int, int>();
    ...
    vector<ll> cost(n, LINF);
    {
        ...
        while (!que.empty()) {
            ...
            for (auto&& next : graph[top]) {
                auto wait = ceildiv(cost[top], next.k) * next.k - cost[top];
                ll next_cost = cost[top] + next.weight + wait;
