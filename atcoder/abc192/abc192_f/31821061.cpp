...
namespace bys {
void Solver::solve() {
    auto [n, x] = scanner.read<int, ll>();
    auto a = scanner.readvec<ll>(n);

    ll ans = LINF;
    for (int k : irange(1, n + 1)) {
        vector dp(n + 1, vector(k + 1, vector<ll>(k, -1)));
        dp[0][0][0] = 0;
        for (int i : irange(n)) {
            for (int j : irange(k + 1)) {
                for (int m : irange(k)) {
                    if (dp[i][j][m] == -1) continue;
                    auto nxt = dp[i][j][m] + a[i];
                    if (j != k) chmax(dp[i + 1][j + 1][nxt % k], nxt);
                    chmax(dp[i + 1][j][m], dp[i][j][m]);
                }
            }
        }
        if (auto sum = dp[n][k][x % k]; sum != -1) {
            chmin(ans, (x - sum) / k);
        }
    }
    print(ans);
