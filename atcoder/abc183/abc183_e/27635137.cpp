#line 2 "/home/bayashi/git/byslib/byslib/template/acl.hpp"
...
#line 2 "/home/bayashi/git/byslib/byslib/template/bys.hpp"
#ifndef LOCAL
#define NDEBUG
#endif
...
#include <complex>
#include <iomanip>
#line 13 "/home/bayashi/git/byslib/byslib/template/bys.hpp"
#include <iterator>
#include <limits>
#include <map>
#include <numeric>
...
#line 3 "abc183_e/main.cpp"

namespace bys {
void Solver::solve() {
    auto [h, w] = input<int, 2>();
    auto s = input<string>(h);
    vector dp(h + 1, vector<mint7>(w + 1)), d(h + 1, vector<mint7>(w + 1)),
        r(h + 1, vector<mint7>(w + 1)), dr(h + 1, vector<mint7>(w + 1));
    dp[1][1] = 1;
    for (int i : Range(1, h + 1)) {
        for (int j : Range(1, w + 1)) {
            if (s[i - 1][j - 1] == '#') continue;
            dp[i][j] += d[i - 1][j] + r[i][j - 1] + dr[i - 1][j - 1];
            d[i][j] = dp[i][j] + d[i - 1][j];
            r[i][j] = dp[i][j] + r[i][j - 1];
            dr[i][j] = dp[i][j] + dr[i - 1][j - 1];
        }
    }
    print(dp[h][w]);
}
