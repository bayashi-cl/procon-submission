...
void Solver::solve() {
    ...
    vector row(h + 2, vector<Mint7>(w + 2)), col(h + 2, vector<Mint7>(w + 2));
    vector diag(h + 2, vector<Mint7>(w + 2)), dp(h + 2, vector<Mint7>(w + 2));
    dp[1][1] = 1;
    for (int i : irange(1, h + 1)) {
        for (int j : irange(1, w + 1)) {
            if (s[i - 1][j - 1] == '#') {
                row[i][j] = 0;
                col[i][j] = 0;
                diag[i][j] = 0;
                ...
                dp[i][j] += row[i][j] + col[i][j] + diag[i][j];
                row[i + 1][j] += dp[i][j];
                col[i][j + 1] += dp[i][j];
                diag[i + 1][j + 1] += dp[i][j];
            }
