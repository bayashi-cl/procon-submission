...
void Solver::solve() {
    ...
    auto c = input<int>(n);
    atcoder::dsu uft(n);
    vector<map<int, int>> member(n);
    for (int i : Range(n)) member[i][c[i] - 1]++;
    for ([[maybe_unused]] int qi : Range(q)) {
        auto [t, x, y] = input<int, 3>();
        --x, --y;
        if (t == 1) {
            if (uft.same(x, y)) continue;
            int lx = uft.leader(x);
            int ly = uft.leader(y);
            uft.merge(x, y);
            if (uft.leader(x) == lx) {
                for (auto& [k, v] : member[ly]) member[lx][k] += v;
            } else {
                for (auto& [k, v] : member[lx]) member[ly][k] += v;
            }
        } else {
            print(member[uft.leader(x)][y]);
        }
    }
}
}  // namespace bys
