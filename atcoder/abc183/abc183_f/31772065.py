...
def main(**kwargs) -> None:
    ...
    cnt = [Counter([c[i]]) for i in range(n)]
    uft = UnionFindTree(n)
    for _ in range(q):
        t, *body = map(int1, readline().split())
        if t == 0:
            a, b = body
            leader_a = uft.find(a)
            leader_b = uft.find(b)
            if leader_a != leader_b:
                uft.union(a, b)
                new_leader = uft.find(a)

                old_leader = leader_a if new_leader != leader_a else leader_b
                for key, val in cnt[old_leader].items():
                    cnt[new_leader][key] += val
