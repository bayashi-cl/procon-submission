...
void Solver::solve() {
    ...
    auto [s, t, x] = scanner.read<Int1, 3>();
    auto graph = read_adj_uv(n, m);
    vector dp(k + 1, vector<pair<mint, mint>>(n));  // {even, odd}
    dp[0][s].first = 1;
    for (int i : irange(k)) {
        for (int j : irange(n)) {
            for (auto nxt : graph[j]) {
                if ((int)nxt.dest == x) {
                    dp[i + 1][nxt.dest].first += dp[i][j].second;
                    dp[i + 1][nxt.dest].second += dp[i][j].first;
                } else {
                    dp[i + 1][nxt.dest].first += dp[i][j].first;
                    dp[i + 1][nxt.dest].second += dp[i][j].second;
                }
            }
        }
    }
