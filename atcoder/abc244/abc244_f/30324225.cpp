...
void Solver::solve() {
    ...
    for (int i : irange(n)) graph.add_edge(0, grid.index(1 << i, i));
    for (int i : irange(1, 1 << n)) {
        for (auto&& e : elist) {
            graph.add_edge(grid.index(i, e.src), grid.index(i ^ (1 << e.dest), e.dest));
        }
    }
    ...
    for (int s : irange(1 << n)) {
        ll cost = INF;
        for (int t : irange(n)) {
            chmin(cost, res.cost[grid.index(s, t)]);
        }
        ans += cost;
    }
    print(ans);
