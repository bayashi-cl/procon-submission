...
void Solver::solve() {
    ...
    Grid gr((1 << n), n);
    DynamicAdjacencyList g((1 << n) * n);
    for (int i : irange(n)) {
        g.add_edge(0, gr.index(1 << i, i));
    }
    for (int i : irange(1, 1 << n)) {
        for (int f : irange(n)) {
            for (auto&& nxt : graph[f]) {
                g.add_edge(gr.index(i, f), gr.index(i ^ (1 << nxt.dest), nxt.dest));
            }
        }
    }

    auto res = breadth_first_search(g, 0);
    int ans = 0;
    for (int i : irange(1 << n)) {
        ll co = INF;
        for (int to : irange(n)) {
            chmin(co, res.cost[gr.index(i, to)]);
        }
        ans += co;
    }
    print(ans);
