...
namespace bys {
ll dist2(const Pall& a, const Pall& b) {
    auto dx = b.first - a.first;
    auto dy = b.second - a.second;
    return dx * dx + dy * dy;
}
void Solver::solve() {
    auto n = scanner.read<int>();
    auto [s, t] = scanner.read<Pall, 2>();
    AdjacencyList graph(n + 2);
    vector<ll> r(n);
    vector<Pall> c(n);
    for (int i : irange(n)) {
        c[i] = scanner.read<Pall>();
        r[i] = scanner.read<ll>();
    }
    for (int i : irange(n)) {
        for (int j : irange(n)) {
            auto d = dist2(c[i], c[j]);
            auto a = r[i];
            auto b = r[j];
        ...
        if (dist2(c[i], s) == r[i] * r[i]) {
            graph.add_edge(i, n);
            graph.add_edge(n, i);
        }
        if (dist2(c[i], t) == r[i] * r[i]) {
            graph.add_edge(i, n + 1);
            graph.add_edge(n + 1, i);
        }
    }
    graph.build();
    print(reachable(graph, n)[n + 1] ? "Yes" : "No");
}
}  // namespace bys
