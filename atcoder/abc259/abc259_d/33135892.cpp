...
template <class T>
bool is_cross(const Circle<T>& a, const Circle<T>& b) {
    auto d = distance2(a.c, b.c);
    return (a.r - b.r) * (a.r - b.r) <= d and d <= (a.r + b.r) * (a.r + b.r);
}
}  // namespace bys::geo
/**
 * @file change.hpp
...
void Solver::solve() {
    ...
    auto [s, t] = scanner.read<P, 2>();
    vector<geo::Circle<ll>> c;
    for (UV : irange(n)) {
        auto [p, r] = scanner.read<P, ll>();
        c.emplace_back(p, r);
    }
    UnionFindTree uf(n + 2);
    for (int i : irange(n)) {
        for (int j : irange(n)) {
            if (geo::is_cross(c[i], c[j])) uf.unite(i, j);
        }
        if (c[i].r * c[i].r == geo::distance2(c[i].c, s)) uf.unite(i, n);
        if (c[i].r * c[i].r == geo::distance2(c[i].c, t)) uf.unite(i, n + 1);
    }
    print(uf.same(n, n + 1) ? "Yes" : "No");
}
}  // namespace bys
