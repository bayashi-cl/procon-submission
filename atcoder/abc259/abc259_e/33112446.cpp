...
void Solver::solve() {
    ...
    map<int, vector<Pa>> p_cnt;
    vector<vector<Pa>> a(n);
    for (int i : irange(n)) {
        auto m = scanner.read<int>();
        for (UV : irange(m)) {
            auto [p, e] = scanner.read<int, 2>();
            p_cnt[p].push_back({e, i});
            a[i].push_back({p, e});
        }
    }
    for (auto&& [p, es] : p_cnt) sort(es.begin(), es.end(), std::greater<Pa>());

    int ans = 1;
    for (int i : irange(n)) {
        for (auto [p, e] : a[i]) {
            if (p_cnt[p][0].second == i) {
                if (p_cnt[p].size() == 1 or p_cnt[p][1].first != e) {
                    ++ans;
                    break;
                }
            }
