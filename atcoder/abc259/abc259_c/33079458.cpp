...
void Solver::solve() {
    ...
    auto t = scanner.read<string>();
    auto rs = run_length_encode(s);
    auto rt = run_length_encode(t);
    if (rs.size() != rt.size()) EXIT("No");
    for (int i : irange(rs.size())) {
        if (rs[i].first != rt[i].first) EXIT("No");
        if (not(rs[i].second == rt[i].second or (rs[i].second >= 2 and rs[i].second <= rt[i].second))) {
            EXIT("No");
        }
    }
    print("Yes");
