...
#line 4 "typical90_bh/main.cpp"

namespace bys {
void Solver::solve() {
    auto n = scanner.read<int>();
    auto a = scanner.readvec<int>(n);
    vector<int> dp_l(n + 1, INF);
    dp_l[0] = 0;
    vector<int> lis(n);
    for (int i : Range(n)) {
        auto itr = std::lower_bound(dp_l.begin(), dp_l.end(), a[i]);
        chmin(*itr, a[i]);
        auto itr2 = std::lower_bound(dp_l.begin(), dp_l.end(), INF);
        lis[i] = itr2 - dp_l.begin() - 1;
    }
    reverse(a.begin(), a.end());
    vector<int> dp_r(n + 1, INF);
    dp_r[0] = 0;
    vector<int> lds(n);
    for (int i : Range(n)) {
        auto itr = std::lower_bound(dp_r.begin(), dp_r.end(), a[i]);
        chmin(*itr, a[i]);
        auto itr2 = std::lower_bound(dp_r.begin(), dp_r.end(), INF);
        lds[i] = itr2 - dp_r.begin() - 1;
    }
    std::reverse(lds.begin(), lds.end());
    int ans = 0;
    for (int i : Range(lis.size())) {
        chmax(ans, lis[i] + lds[i] - 1);
    }
