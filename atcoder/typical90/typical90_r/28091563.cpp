...
#line 3 "typical90_r/main.cpp"

namespace bys {
void Solver::solve() {
    using geo::PI;
    auto [t, l, x, y] = input<ld, 4>();

    auto q = input<int>();
    for (int i = 0; i < q; ++i) {
        auto e = input<ld>();
        ld theta_d = e / t * 2 * PI;
        ld theta = PI * 1.5 - theta_d;
        // theta_d <= PI * 1.5 ? PI * 1.5 - theta_d : PI * 3.5 - theta_d;
        ld ye = l / 2 * std::cos(theta);
        ld ze = l / 2 * std::sin(theta) + l / 2;
        ld d = std::hypot(x - 0, y - ye);
        print(geo::degree(std::atan2(ze, d)));
    }
}
}  // namespace bys
