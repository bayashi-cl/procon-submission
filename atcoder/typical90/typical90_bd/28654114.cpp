...
#line 4 "typical90_bd/main.cpp"

namespace bys {
void Solver::solve() {
    auto [n, s] = scanner.read<int, 2>();
    auto ab = scanner.readvec<Pa>(n);
    ...
    if (!dp[n][s]) EXIT("Impossible");

    string ans;
    for (int i : reversed(Range(n))) {
        auto&& [a, b] = ab[i];
        if (s - a >= 0 && dp[i][s - a]) {
            s -= a;
            ans.push_back('A');
        } else if (s - b >= 0 && dp[i][s - b]) {
            s -= b;
            ans.push_back('B');
        } else {
            throw std::logic_error("dp");
        }
    }
