...
#line 7 "typical90_bu/main.cpp"

namespace bys {
void Solver::solve() {
    auto n = scanner.read<int>();
    auto c = scanner.readln([](const string& s) { return s == "a" ? 0 : 1; });
    auto graph = read_adj_uv(n, n - 1);

    DepthFirstSearch dfs(graph, 0);
    vector dp(n, vector<mint7>(3, 0));

    for (auto&& now : dfs.post_order) {
        dp[now][c[now]] = 1;
        dp[now][2] = 1;
        for (auto&& nxt : graph[now]) {
            if (nxt.to == dfs.prev[now]) continue;
            dp[now][0] *= dp[nxt.to][0] + dp[nxt.to][2];
            dp[now][1] *= dp[nxt.to][1] + dp[nxt.to][2];
            dp[now][2] *= dp[nxt.to][0] + dp[nxt.to][1] + dp[nxt.to][2] * 2;
        }
