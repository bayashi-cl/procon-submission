...
void Solver::solve() {
    ...
    vector<ll> s(n), ss(n);
    for (UV : Range(q)) {
        auto [t, x, y, v] = scanner.read<int, Int1, Int1, ll>();
        que.push_back({t, x, y, v});
        if (t == 0) {
            s[x] = v;
            ss[y] = v;
        }
    ...
    vector<ll> p(n);
    for (int i : Range(n - 1)) p[i + 1] = s[i] - p[i];
    for (int i : Range(1, n)) ss[i] -= ss[i - 1];
    atcoder::dsu uft(n);
    for (auto&& [t, x, y, v] : que) {
        if (t == 0) {
            uft.merge(x, y);
        } else {
            if (!uft.same(x, y)) {
