...
#line 5 "typical90_ay/main.cpp"

namespace bys {
void Solver::solve() {
    auto [n, k, p] = scanner.read<int, int, ll>();
    auto a = scanner.readvec<ll>(n);
    int nl = n / 2;
    int nr = n - nl;

    vector<vector<ll>> left(nl + 1);
    for (int s : Range(1 << nl)) {
        ll pr = 0;
        for (auto&& d : Range(nl)) {
            if (pop(s, d)) pr += a[d];
        }
        left[__builtin_popcount(s)].push_back(pr);
    }

    vector<vector<ll>> right(nr + 1);
    for (int s : Range(1 << nr)) {
        ll pr = 0;
        for (auto&& d : Range(nr)) {
            if (pop(s, d)) pr += a[d + nl];
        }
        right[__builtin_popcount(s)].push_back(pr);
    }

    ll ans = 0;
    for (int l : Range(nl + 1)) {
        int r = k - l;
        ...
        for (auto&& li : left[l]) {
            ll rem = p - li;
            if (rem < 0) continue;
            auto itr = std::upper_bound(right[r].begin(), right[r].end(), rem);
            ans += itr - right[r].begin();
        }
    }
    print(ans);
}
