...
#line 2 "typical90_bo/main.cpp"

namespace bys {
vector<ll> base_conversion(vector<ll>& x, ll f, ll t) {
    ll deci = 0;
    ll p = 1;
    for (auto&& xi : x) {
        deci += p * xi;
    ...
    vector<ll> res;
    while (deci > 0) {
        res.push_back(deci % t);
        deci /= t;
    }
    return res;
}
void Solver::solve() {
    auto [n, k] = input<string, int>();
    if (n == "0") EXIT(0);
    vector<ll> num;
    std::for_each(n.rbegin(), n.rend(),
                  [&](auto d) { num.push_back(d - '0'); });
    for (ll i = 0; i < k; ++i) {
        num = base_conversion(num, 8, 9);
        std::replace(num.begin(), num.end(), 8, 5);
    }
    std::for_each(num.rbegin(), num.rend(), [](auto x) { cout << x; });
    cout << endl;
}
}  // namespace bys
