...
#line 3 "/home/bayashi/dev/byslib/data/cumulative_sum.hpp"
...
#line 5 "typical90_cc/main.cpp"

namespace bys {
void Solver::solve() {
    constexpr int MX = 5000;
    auto [n, k] = scanner.read<int, 2>();
    CumulativeSum2D<int> cs(MX + 1, MX + 1);
    for (UV : Range(n)) {
    ...
    cs.construct();
    int ans = 0;
    for (int si : Range(MX + 1)) {
        for (int sj : Range(MX + 1)) {
            int gi = min(MX + 1, si + k + 1);
            int gj = min(MX + 1, sj + k + 1);
            chmax(ans, cs.sum(si, sj, gi, gj));
        }
    }
    print(ans);
}
