...
#line 6 "typical90_ci/main.cpp"

namespace bys {
void warshall_floyd(vector<vector<ll>>& graph) {
    int n = graph.size();
    for (auto&& k : Range(n)) {
        for (auto&& i : Range(n)) {
            for (auto&& j : Range(n)) {
...
void Solver::solve() {
    auto [n, p, k] = scanner.read<int, 3>();
    const auto a = scanner.readvec<ll>(n, n);

    auto ok = [&](int x, int eq) {
        auto graph = a;
        for (auto&& g : graph) std::replace(g.begin(), g.end(), -1, x);
    ...
    if (ok(INF, -1)) EXIT("Infinity");
    if (ok(INF, 0)) EXIT(0);
    if (!ok(0, 1)) EXIT(0);

    auto mi = meguru_bisect(0, INF, ok, 0);
    auto ma = meguru_bisect(0, INF, ok, 1);
    print(ma - mi);
}
}  // namespace bys
