...
#line 3 "/home/bayashi/dev/byslib/math/divisor.hpp"
...
#line 5 "typical90_cg/main.cpp"

namespace bys {
void Solver::solve() {
    auto k = scanner.read<ll>();
    auto divs = make_divisor(k);
    ll ans = 0;
    for (auto&& c : divs) {
        ll ab = k / c;
        auto abdiv = make_divisor(ab);
        auto itr = std::upper_bound(abdiv.begin(), abdiv.end(), c);
        int l = abdiv.size();
        int n = std::distance(abdiv.begin(), itr);
        ans += max(0, n - l / 2);
    }
    print(ans);
}
