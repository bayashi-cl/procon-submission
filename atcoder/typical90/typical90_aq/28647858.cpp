...
namespace bys {
int to(int i, int j, int ni, int nj) {
    if (ni == i - 1) {
        return 3;
    } else if (ni == i + 1) {
        return 1;
    ...
    for (auto&& i : Range(h)) {
        for (auto&& j : Range(w)) {
            ...
            for (auto&& [ni, nj] : grid.next4(i, j)) {
                ...
                int t = to(i, j, ni, nj);
                for (auto&& f : Range(4)) {
                    int cost = (f + t) % 2 == 0 ? 0 : 1;
                    graph[index(i, j, f)].push_back({index(ni, nj, t), cost});
                }
            }
        }
    }
    for (auto&& [i, j] : vector<Pa>{{rs, cs}, {rt, ct}}) {
        for (auto&& k : Range(4)) {
            graph[index(i, j, k)].push_back({index(i, j, (k + 1) % 4), 0});
        }
    }
