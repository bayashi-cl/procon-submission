...
#line 4 "typical90_aq/main.cpp"
// #include "byslib/utility/grid.hpp"
...
#line 6 "typical90_aq/main.cpp"

namespace bys {
void Solver::solve() {
    auto [h, w] = scanner.read<int, 2>();
    auto [rs, cs] = scanner.read<Int1, 2>();
    auto [rt, ct] = scanner.read<Int1, 2>();
    auto s = scanner.readvec<string>(h);
    Adj graph(h * w * 4);
    auto index = [&](int i, int j, int k) { return i * w + j + k * h * w; };
    ...
    for (auto&& i : Range(h)) {
        for (auto&& j : Range(w)) {
            ...
            for (auto&& k : Range(4)) {
                for (auto&& l : Range(4)) {
                    if (k == l) continue;
                    int cost = (k + l) % 2 == 0 ? 0 : 1;
                    if ((i == rs && j == cs) || (i == rt && j == ct)) cost = 0;
                    graph[index(i, j, k)].push_back({index(i, j, l), cost});
                }
            }
            if (j - 1 >= 0 && s[i][j - 1] == '.') graph[index(i, j, 0)].push_back({index(i, j - 1, 2), 0});
            if (i - 1 >= 0 && s[i - 1][j] == '.') graph[index(i, j, 1)].push_back({index(i - 1, j, 3), 0});
            if (j + 1 < w && s[i][j + 1] == '.') graph[index(i, j, 2)].push_back({index(i, j + 1, 0), 0});
            if (i + 1 < h && s[i + 1][j] == '.') graph[index(i, j, 3)].push_back({index(i + 1, j, 1), 0});
        }
    }
    ZeroOneBFS bfs(graph, index(rs, cs, 0));
    print(bfs.cost[index(rt, ct, 0)]);
}
}  // namespace bys
