...
#line 4 "abc241_c/main.cpp"

namespace bys {
bool judge(vector<char>& v) {
    if (v.size() < 6UL) return false;
    vector<pair<char, int>> rle;
    rle.push_back({v[0], 0});
    for (auto&& vi : v) {
        if (vi == rle.back().first) {
            rle.back().second++;
        } else {
            rle.push_back({vi, 1});
        }
    ...
    for (int i : irange(sz)) {
        if (rle[i].first == '.') {
            if (i + 1 < sz) {
                if (min(2, rle[i].second) + rle[i + 1].second >= 6) {
                    return true;
                }
            }
            if (i + 2 < sz) {
            ...
            if (i + 3 < sz) {
                if (rle[i + 2].second == 1 && rle[i + 1].second + rle[i + 3].second >= 4) {
                    return true;
                }
            }

        } else {
            if (rle[i].second >= 6) return true;
            if (i + 1 < sz) {
                if (rle[i].second + min(2, rle[i + 1].second) >= 6) {
                    return true;
                }
            }
            if (i + 2 < sz) {
                if (rle[i + 1].second <= 2 && rle[i].second + rle[i + 1].second + rle[i + 2].second >= 6) {
                    return true;
                }
            }
            if (i + 3 < sz) {
                if (rle[i + 1].second == 1 && rle[i].second + rle[i + 1].second + rle[i + 2].second >= 5) {
                    return true;
                }
            }
            if (i + 4 < sz) {
                if (rle[i + 1].second + rle[i + 3].second <= 2 && rle[i].second + rle[i + 2].second + rle[i + 4].second >= 4) {
                    return true;
                }
            }
        }
...
void Solver::solve() {
    ...
    for (auto i : irange(n)) {
        ...
        if (judge(row) || judge(col)) EXIT("Yes");
    }
    for (int j : irange(n)) {
        vector<char> v, w;
        for (int i : irange(n)) {
        ...
        if (judge(v) || judge(w)) EXIT("Yes");
    }
    for (auto&& si : s) reverse(si.begin(), si.end());
    for (int j : irange(n)) {
        vector<char> v, w;
        for (int i : irange(n)) {
            if (i + j < n) {
