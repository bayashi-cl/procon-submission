...
void Solver::solve() {
    ...
    auto w = bit_width(k);
    vector db(w + 1, vector<pair<int, ll>>(n));
    for (int i : irange(n)) {
        auto after = i + a[i];
        db[0][i] = {after % n, after / n};
    }
    for (int i : irange(w)) {
        for (int j : irange(n)) {
            db[i + 1][j] = {db[i][db[i][j].first].first, db[i][j].second + db[i][db[i][j].first].second};
        }
    }
    ll mod = 0;
    ll div = 0;
