...
namespace bys {
void Solver::solve() {
    auto [n, m, k] = scanner.read<int, 3>();
    EList elist(n * 2);
    for (UV : irange(m)) {
        auto [u, v, a] = scanner.read<int1, int1, int>();
        if (a == 1) {
            elist.add_undirected_edge(u, v, 1);
        } else {
            elist.add_undirected_edge(u + n, v + n, 1);
        }
    }

    auto s = scanner.readvec<int1>(k);
    for (auto si : s) {
        elist.add_undirected_edge(si, si + n, 0);
    }

    auto res = zero_one_bfs(elist.adj(), 0);
    auto ans = min(res[n - 1], res[2 * n - 1]);
