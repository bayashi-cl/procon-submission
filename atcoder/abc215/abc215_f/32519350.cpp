...
void Solver::solve() {
    ...
    auto point = scanner.readvec<Pa>(n);
    ...
    CumulativeSum<Max<int>> cs_max(ys);
    CumulativeSum<Min<int>> cs_min(ys);

    auto ok = [&](int th) -> bool {  // exists dist(a, b) >= x ?
        for (int i : irange(n)) {
            auto r = std::upper_bound(xs.begin(), xs.end(), xs[i] - th) - xs.begin();
            if (r == 0) continue;
            auto maxi = cs_max.prefix_fold(r);
            auto mini = cs_min.prefix_fold(r);
            if (maxi - ys[i] >= th or ys[i] - mini >= th) {
                return true;
            }
        }
        return false;
