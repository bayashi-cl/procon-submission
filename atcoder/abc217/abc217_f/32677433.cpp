...
void Solver::solve() {
    ...
    vector fri(2 * n, vector<bool>(2 * n));
    for (UV : irange(m)) {
    ...
    MultiComb mc(n);
    vector memo(2 * n, vector<std::optional<Mint>>(2 * n + 1));
    FixPoint dp([&](auto&& self, int l, int r) -> Mint {
        if (l == r) return 1;
        if (memo[l][r].has_value()) return memo[l][r].value();
        int p = (r - l) / 2;
        Mint res = 0;
        for (auto m : irange(l + 1, r, 2)) {
            if (fri[l][m]) {
                res += self(l + 1, m) * self(m + 1, r) * mc.comb(p, (m - l + 1) / 2);
            }
        }
        memo[l][r] = res;
        return res;
