...
void Solver::solve() {
    ...
    vector dp(n + 1, vector(2, vector<ll>(3, -LINF)));
    dp[0][1][1] = 0;
    dp[0][0][1] = 0;
    for (int i : irange(1, n + 1)) {
        if (i & 1) {
            ...
            dp[i][0][2] = max(dp[i - 1][0][2], dp[i - 1][1][2]);

            dp[i][1][1] = dp[i - 1][0][0] + a[i - 1];
            dp[i][1][2] = dp[i - 1][0][1] + a[i - 1];
        } else {
            dp[i][0][0] = max(dp[i - 1][0][1], dp[i - 1][1][1]);
            dp[i][0][1] = max(dp[i - 1][0][2], dp[i - 1][1][2]);

            dp[i][1][0] = dp[i - 1][0][0] + a[i - 1];
            dp[i][1][1] = dp[i - 1][0][1] + a[i - 1];
            dp[i][1][2] = dp[i - 1][0][2] + a[i - 1];
        }
    }
