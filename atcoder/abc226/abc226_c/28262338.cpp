...
void Solver::solve() {
    ...
    Adj graph(n);
    for (int i = 0; i < n; ++i) {
        auto [ti, k] = input<int, 2>();
        t[i] = ti;
        for (auto&& ai : input<int>(k)) graph[i].push_back({ai - 1});
    }
    DepthFirstSearch dfs(graph, n - 1);
    ll ans = 0;
    for (int i : dfs.post_order) ans += t[i];
    print(ans);
}
}  // namespace bys
