...
proc powMod*(p, q, m: int): int =
  if q == 0: return 1
  result = 1
  var base = p
  for d in 0..fastLog2(q):
    if q.testBit(d):
...
proc main =
  ...
  let
    s = read(string)
    t = s.mapIt(it.int - zero)
    n = t.len

  var
    twos = newSeq[int](n + 1)
    fives = newSeq[int](n + 1)
    tens = newSeq[int](n + 1)
    sigma = newSeq[int](n + 1)
  ...
  for i in 1..n:
    ...
    fives[i] = fives[i - 1] * 5 % Mod
    tens[i] = tens[i - 1] * 10 % Mod
    sigma[i] = (sigma[i - 1] + fives[i]) % Mod

  var ans = t[^1] * twos[n - 1]
  for i in 0..n-2:
    let r = n - i - 2
    ans += (twos[r] * sigma[r] + tens[r + 1]) % Mod * (twos[i] * t[i]) % Mod
  print(ans % Mod)
