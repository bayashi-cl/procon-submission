...
struct Piece {
    int index, r, c, a;
    bool operator<(const Piece& other) const { return a < other.a; }
};
void Solver::solve() {
    auto [h, w, n] = scanner.read<int, 3>();
    vector<Piece> piece;
    for (int i : irange(n)) {
        auto [r, c, a] = scanner.read<Int1, Int1, int>();
        Piece p = {i, r, c, a};
        piece.push_back(p);
    }
    sort(piece.rbegin(), piece.rend());

    vector<int> row(h), col(w), ans(n);
    map<int, int> lazy_row, lazy_col;
    int last = -1;
    for (auto [i, r, c, a] : piece) {
        if (a != last) {
            for (auto [k, v] : lazy_row) chmax(row[k], v);
            lazy_row.clear();
            for (auto [k, v] : lazy_col) chmax(col[k], v);
            lazy_col.clear();
            last = a;
        }
        ans[i] = max(row[r], col[c]);
        chmax(lazy_row[r], ans[i] + 1);
        chmax(lazy_col[c], ans[i] + 1);
    }
    print.set("\n")(ans);
}
}  // namespace bys
