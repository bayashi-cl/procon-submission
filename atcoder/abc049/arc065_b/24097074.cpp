...
void main() {
    auto [n, k, l] = input<int, 3>();
    atcoder::dsu road(n), train(n), uni(n);
    auto pq = input<Pa>(k);
    auto rs = input<Pa>(l);

    for (auto [p, q] : pq) road.merge(p - 1, q - 1);
    for (auto [r, s] : rs) train.merge(r - 1, s - 1);

    vector<Pa> root(n);
    map<Pa, ll> cnt;
    for (auto&& i : irange(n)) {
        int root_r = road.leader(i);
        int root_t = train.leader(i);
        root[i] = {root_r, root_t};
        cnt[{root_r, root_t}]++;
    }
    Vec ans(n);
    for (auto&& i : irange(n)) ans[i] = cnt[root[i]];
    print(ans);
}
