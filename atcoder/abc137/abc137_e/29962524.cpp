...
namespace bys {
std::vector<ll> bellman_ford(const AdjacencyList& graph, int n, int source) {
    vector<ll> res(n, LINF);
    res[source] = 0;
    for (UV : irange(n - 1)) {
        for (int now : irange(n)) {
            if (res[now] >= LINF) continue;
            for (auto&& nxt : graph[now]) {
                chmin(res[nxt.dest], res[now] + nxt.weight);
            }
        }
    }
    vector<bool> pos(n);
    for (UV : irange(n)) {
        for (int now : irange(n)) {
            if (res[now] >= LINF) continue;
            for (auto&& nxt : graph[now]) {
                if (chmin(res[nxt.dest], res[now] + nxt.weight) || pos[now]) pos[nxt.dest] = true;
            }
        }
    }
    for (int i : irange(n)) {
...
void Solver::solve() {
    ...
    auto res = bellman_ford(graph, n, 0);
    if (res[n - 1] == -LINF) {
        print(-1);
    } else {
        print(max(-res[n - 1], 0LL));
    }
}
}  // namespace bys
