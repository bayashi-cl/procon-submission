...
std::vector<typename E::weight_type> bellman_ford(EdgeList<E> const& edges, typename E::vertex_type source) {
    ...
    std::vector<W> cost(edges.size(), get_inf<W>());
    int n = edges.size();
    int m = edges.nonzero();
    cost[source] = 0;
    for (int i = 0; i < n - 1; ++i) {
...
void Solver::solve() {
    auto [n, m, p] = scanner.read<int, int, ll>();
    EList edges(n);

    for (UV : irange(m)) {
        auto [a, b, c] = scanner.read<int1, int1, ll>();
        edges.add_edge(a, b, -c + p);
    }
    auto res = bellman_ford(edges, 0);
    if (is_inf(abs(res.back()))) {
        print(-1);
    } else {
        print(max(-res.back(), (std::int64_t)0));
    }
}
}  // namespace bys
