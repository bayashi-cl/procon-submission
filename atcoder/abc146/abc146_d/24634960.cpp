...
namespace bys {
Pa e(int a, int b) { return {min(a, b), max(a, b)}; }
void solve() {
    auto n = input<int>();
    VecVec adj(n);
    vector<Pa> edge;
    ...
    map<Pa, int> col;
    col[{-1, 0}] = -1;
    auto dfs = [&](auto&& dfs, int now, int from = -1) -> void {
        int from_c = col[e(from, now)];
        int c = 0;
        for (auto&& to : adj[now]) {
            if (to == from) continue;
            if (c == from_c) c++;
    ...
    dfs(dfs, 0);
    Vec ans(n - 1);
    for (int i = 0; i < n - 1; ++i) ans[i] = col[edge[i]] + 1;
    print(*std::max_element(std::begin(ans), std::end(ans)));
