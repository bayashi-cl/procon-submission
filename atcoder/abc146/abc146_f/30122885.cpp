...
struct MinPair : Min<pair<int, int>> {
    static constexpr pair<int, int> id{INF, INF};
};
...
void Solver::solve() {
    ...
    prev[0] = 0;
    SegmentTree<MinPair> seg(n + 1);
    seg.update(0, {0, 0});
    for (int i : irange(1, n + 1)) {
        if (s[i] == '1') continue;
        auto mi = seg.query(max(i - m, 0), i);
        if (mi.first == INF) continue;
        seg.update(i, {mi.first + 1, i});
        prev[i] = i - mi.second;
    }
    if (seg[n].first == INF) EXIT(-1);
