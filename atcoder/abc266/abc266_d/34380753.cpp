...
void Solver::solve() {
    ...
    int last = -1;
    for (UV : irange(n)) {
        auto [t, x, a] = scanner.read<int, ll, ll>();
        out[t] = {x, a};
        chmax(last, t);
    }
    vector dp(last + 1, vector<ll>(5, -LINF));
    dp[0][0] = 0;
    for (int t : irange(1, last + 1)) {
        for (int j : irange(5)) {
            if (j != 0) chmax(dp[t][j], dp[t - 1][j - 1]);
            chmax(dp[t][j], dp[t - 1][j]);
            if (j != 4) chmax(dp[t][j], dp[t - 1][j + 1]);
        }
    ...
    print(*std::max_element(dp.back().begin(), dp.back().end()));
}
}  // namespace bys
