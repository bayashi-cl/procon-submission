...
void Solver::solve() {
    ...
    auto m = scanner.read<int>();
    vector<int> rest(n);  // edge id

    for (int i : irange(m)) {
        auto [u, v] = scanner.read<Int1, 2>();
        auto res = breadth_first_search(graph, u);
        auto path = res.path(v);

        for (auto j : irange(path.size() - 1)) {
            rest[mp[OrderedPair((int)path[j], (int)path[j + 1])]] |= 1 << i;
        }
    }

    vector dp(n + 1, vector<ll>(1 << m, 0));
    dp[0][0] = 1;
    for (int i : irange(n)) {
        int s = rest[i];
        for (int j : irange(1 << m)) {
            dp[i + 1][j] += dp[i][j];
            dp[i + 1][j | s] += dp[i][j];
        }
    }
