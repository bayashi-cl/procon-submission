...
void Solver::solve() {
    ...
    auto x = scanner.readvec<ll>(n);
    auto y = scanner.readvec<ll>(n);
    auto road = scanner.readvec<std::tuple<int1, int1, ll>>(m);

    ll ans = LINF;
    for (auto s : irange(1 << 2)) {
        ll cost = 0;
        vector<Edge> edges;
        if (pop(s, 0)) {
            for (int i : irange(n)) {
                edges.push_back(Edge(i, n, x[i]));
            }
        }
        if (pop(s, 1)) {
            for (int i : irange(n)) {
                edges.push_back(Edge(i, n + 1, y[i]));
            }
        }
        for (auto [u, v, w] : road) {
