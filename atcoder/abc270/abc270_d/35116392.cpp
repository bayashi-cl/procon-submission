...
void Solver::solve() {
    ...
    auto a = scanner.readvec<int>(k);
    vector memo(n + 1, vector<std::optional<int>>(2));
    FixPoint rec([&](auto&& self, int stone, int turn) -> int {
        if (memo[stone][turn].has_value()) return memo[stone][turn].value();
        if (stone == 0) return 0;
        int res = 0;
        if (turn == 0) {
            int r = 0;
            for (auto ai : a) {
                if (stone - ai >= 0) {
                    chmax(r, self(stone - ai, 1 - turn) + ai);
                } else {
                    break;
                }
            }
