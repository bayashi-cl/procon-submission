...
#line 3 "abc091_b/main.cpp"

namespace bys {
void Solver::solve() {
    auto n = input<int>();
    auto s = input<string>(n);
    auto m = input<int>();
    auto t = input<string>(m);

    map<string, int> cnt_s, cnt_t;
    for (auto&& si : s) cnt_s[si]++;
    for (auto&& ti : t) cnt_t[ti]++;
    int ans = 0;
    for (auto&& si : s) chmax(ans, cnt_s[si] - cnt_t[si]);
    print(ans);
}
}  // namespace bys
