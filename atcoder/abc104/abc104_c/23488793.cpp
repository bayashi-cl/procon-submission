...
void main() {
    ...
    for (auto&& s : irange(1 << d)) {
        ...
        for (auto&& i : irange(d)) {
            ...
            score += (i + 1) * 100 * diff[i].first + diff[i].second;
            prob += diff[i].first;
        }
        if (score < g) {
            std::reverse(std::begin(unused), std::end(unused));
            int remain = g - score;
            bool flag = false;

            for (auto u : unused) {
                if ((u + 1) * 100 * (diff[u].first - 1) < remain) {
                    remain -= (u + 1) * 100 * (diff[u].first - 1);
                    prob += diff[u].first - 1;
                    continue;
                } else {
                    prob += (remain + ((u + 1) * 100) - 1) / ((u + 1) * 100);
                    flag = true;
                    break;
                }
            }
