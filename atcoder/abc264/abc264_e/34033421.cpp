...
void Solver::solve() {
    ...
    auto es = scanner.readvec<pair<int1, int1>>(e);
    auto q = scanner.read<int>();
    auto x = scanner.readvec<int1>(q);

    set xs(x.begin(), x.end());
    UnionFindTree uf(n + 1);
    for (auto [i, ei] : enumerate(es)) {
        if (xs.find(i) == xs.end()) {
            auto [u, v] = ei;
            chmin(u, n);
            chmin(v, n);
    ...
    vector<int> ans;
    for (auto xi : SubRange(x.rbegin(), x.rend())) {
