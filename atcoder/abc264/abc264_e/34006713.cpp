...
namespace bys {
void Solver::solve() {
    auto [n, m, e] = scanner.read<int, 3>();
    auto uv = scanner.readvec<pair<inti, inti>>(e);

    auto q = scanner.read<int>();
    auto x = scanner.readvec<inti>(q);
    set xs(x.begin(), x.end());

    vector<bool> d(n + m);
    for (int i : irange(n, n + m)) d[i] = true;
    UnionFindTreeWithData uf(n + m, d, [](bool a, bool b) { return a | b; });
    for (auto [i, uvi] : enumerate(uv)) {
        if (xs.find(i) == xs.end()) {
            auto [u, v] = uvi;
            uf.unite(u, v);
        }
    ...
    for (auto [l, g] : uf.groups()) {
        if (uf.get(l)) cnt += g.size();
    }
    vector<int> ans;
    ans.push_back(cnt - m);

    for (int i : irange(q - 1)) {
        auto xi = x[q - i - 1];
        auto [u, v] = uv[xi];
