...
using inti = Indexed<int>;
using lli = Indexed<long long int>;
...
namespace bys {
void Solver::solve() {
    auto [h1, w1] = scanner.read<int, 2>();
    auto a = scanner.readvec<int>(h1, w1);
    auto [h2, w2] = scanner.read<int, 2>();
    auto b = scanner.readvec<int>(h2, w2);

    for (int h : irange(1 << h1)) {
        if (__builtin_popcount(h) != h2) continue;
        vector<int> hs;
        for (int i : irange(h1)) {
            if (pop(h, i)) hs.push_back(i);
        }
        for (int w : irange(1 << w1)) {
            if (__builtin_popcount(w) != w2) continue;
            vector<int> ws;
            for (int i : irange(w1)) {
                if (pop(w, i)) ws.push_back(i);
            }

            bool flg = true;
            for (int i : irange(hs.size())) {
                for (int j : irange(ws.size())) {
