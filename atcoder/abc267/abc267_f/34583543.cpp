...
class TreeDoubling {
   ...
   public:
    TreeDoubling(CSRMatrix<E> const& graph, vertex_type source) : _n(graph.size()), db(bit_width(_n) + 1, std::vector(_n, -1)) {
        FixPoint dfs([&](auto&& self, int prev, int now) -> void {
            db[0][now] = prev;
            for (auto&& e : graph[now]) {
                if ((int)e.dest != prev) self(now, e.dest);
            }
        });
        dfs(-1, source);

        auto log_n = bit_width(_n);
    ...
    int prev(vertex_type from, int n) {
        if (n > _n) return -1;
        for (int i : irange(bit_width(n))) {
            if ((int)from == -1) return -1;
            if (pop(n, i)) from = db[i][from];
        }
        return from;
    }
};
...
void Solver::solve() {
    ...
    for (UV : irange(n - 1)) {
        ...
        elist.add_undirected_edge(u, v);
    }
    auto graph = elist.adj();

    auto bfs1 = breadth_first_search(graph, 0);
    auto r1 = std::max_element(bfs1.begin(), bfs1.end()) - bfs1.begin();
    TreeDoubling dbl1(graph, r1);

    auto bfs2 = breadth_first_search(graph, r1);
    auto r2 = std::max_element(bfs2.begin(), bfs2.end()) - bfs2.begin();
    TreeDoubling dbl2(graph, r2);

    auto q = scanner.read<int>();
    for (UV : irange(q)) {
        auto [u, k] = scanner.read<int1, int>();
        auto ans = max(dbl1.prev(u, k), dbl2.prev(u, k));
        print(ans == -1 ? -1 : ans + 1);
    }
}
}  // namespace bys
