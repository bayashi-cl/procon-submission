...
void Solver::solve() {
    ...
    map<Pall, int> pos, neg;
    int x_axis = 0, y_axis = 0, origin = 0;
    for (UV : irange(n)) {
        auto [x, y] = scanner.read<ll, 2>();
        if (x == 0 and y == 0) {
            origin++;
        } else if (x == 0) {
            y_axis++;
    ...
    Mint7 ans(1);
    for (auto [p, vp] : pos) {
        auto vn = neg[{-p.second, p.first}];
        ans *= Mint7(2).pow(vp) + Mint7(2).pow(vn) - 1;
    }
    ans *= Mint7(2).pow(x_axis) + Mint7(2).pow(y_axis) - 1;
    ans += origin;
    print(ans - 1);
}
}  // namespace bys
