...
void Solver::solve() {
    ...
    auto c = scanner.readvec<i32>(n);
    auto edges = EdgeList(n, undirected);
    for (UV : irange(m)) {
    ...
    auto g = Grid(n, n);
    auto edges2 = EdgeList(g.area(), directed);
    for (auto i : irange(n)) {
        for (auto j : irange(n)) {
            for (auto ei : graph[i]) {
                auto di = ei.dest;
                for (auto ej : graph[j]) {
                    auto dj = ej.dest;
                    if (c[di] != c[dj]) edges2.add_edge(g.index(i, j), g.index(di, dj));
                }
            }
        }
    }
    auto res = bfs(edges2.build(), g.index(0, n - 1));
    auto ans = res[g.index(n - 1, 0)];
    print(is_inf(ans) ? -1 : ans);
}
}  // namespace bys
