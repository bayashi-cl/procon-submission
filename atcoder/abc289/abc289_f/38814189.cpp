...
namespace bys {
i32 tele(i32 a, i32 b) { return a + (b - a) * 2; }

void Solver::solve() {
    auto [sx, sy] = scanner.read<i32, 2>();
    auto [tx, ty] = scanner.read<i32, 2>();
    auto [a, b, c, d] = scanner.read<i32, 4>();

    if (sx == tx and sy == ty) EXIT("Yes");
    if (sx % 2 != tx % 2 or sy % 2 != ty % 2) EXIT("No");
    if (a == b and c == d) {
        if (tele(sx, a) == tx and tele(sy, c) == ty) {
            print("Yes");
            print(a, c);
            return;
        } else {
    ...
    auto x1 = a, x2 = a + 1;
    if (sx > tx) std::swap(x1, x2);
    for (UV : irange(abs(sx - tx) / 2)) {
        ans_x.push_back(x1);
        ans_x.push_back(x2);
    }

    auto y1 = c, y2 = c + 1;
    if (sy > ty) std::swap(y1, y2);
    for (UV : irange(abs(sy - ty) / 2)) {
        ans_y.push_back(y1);
        ans_y.push_back(y2);
    }

    auto sz = max(ans_x.size(), ans_y.size());
    while (ans_x.size() < sz) {
        ans_x.push_back(x1);
        ans_x.push_back(x1);
    }
