...
void Solver::solve() {
    ...
    for (UV : irange(m)) {
        ...
        auto si = scanner.readvec<i32>(c);
        s.push_back(set(si.begin(), si.end()));
    }

    i32 ans = 0;
    for (auto i : irange(1, 1 << m)) {
        bool ok = true;
        for (auto x : irange(1, n + 1)) {
            bool exist = false;
            for (auto e : irange(m)) {
                if (not pop(i, e)) continue;
                if (s[e].find(x) != s[e].end()) exist = true;
            }
            if (not exist) {
                ok = false;
                break;
