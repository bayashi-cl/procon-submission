...
void Solver::solve() {
    ...
    vector prev(n + 1, vector<int>(n + 1, -1)), memo(n + 1, vector<int>(n + 1, -1));
    for (int i : irange(n + 1)) {
        auto acc = cs.prefix_fold(i);
        for (int m : irange(1, n + 1)) {
            prev[i][m] = memo[m][acc % m];
            memo[m][acc % m] = i;
        }
    }
    vector dp(n + 1, vector<Mint7>(n + 1));
    dp[0][0] = 1;
    for (int i : irange(1, n + 1)) {
        for (int j : irange(1, n + 1)) {
            if (prev[i][j] != -1) dp[i][j] = dp[prev[i][j]][j - 1] + dp[prev[i][j]][j];
        }
    }
    print(std::accumulate(dp.back().begin(), dp.back().end(), Mint7(0)));
}
}  // namespace bys
