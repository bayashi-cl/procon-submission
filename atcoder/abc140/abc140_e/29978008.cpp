...
void Solver::solve() {
    ...
    vector<Pa> pos;
    for (auto [i, pi] : enumerate(p)) pos.push_back({pi, i});
    sort(pos.begin(), pos.end(), std::greater<Pa>());
    set<int> s;
    s.insert(-1);
    s.insert(n);
    ll ans = 0;
    for (auto [pi, i] : pos) {
        auto [itr, b] = s.insert(i);
        if (int l1 = *std::prev(itr); l1 != -1) {
            int l2 = *std::prev(itr, 2);
            int left = l1 - l2;
            int right = *std::next(itr) - i;
            ans += ll(pi) * left * right;
        }
        if (int r1 = *std::next(itr); r1 != n) {
            int r2 = *std::next(itr, 2);
            int right = r2 - r1;
            int left = i - *std::prev(itr);
            ans += ll(pi) * left * right;
        }
    }
    print(ans);
