...
void solve() {
    auto [m, n, k] = input<int, 3>();
    auto biome = input<string>(m);
    CumulativeSum2D<int> jungle(m, n), ocean(m, n), ice(m, n);
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            if (biome[i][j] == 'J') {
                jungle.set(i, j, 1);
    ...
    for (int i = 0; i < k; ++i) {
        auto [a, b, c, d] = input<int, 4>();
        --a, --b;
        print(jungle.sum(a, b, c, d), ocean.sum(a, b, c, d),
              ice.sum(a, b, c, d));
    }
}
}  // namespace bys
