...
void Solver::solve() {
    ...
    vector<Edge> elist_01;
    for (int i : irange(n)) {
        for (int j : irange(n)) {
            if (res.cost[i][j] <= l) elist_01.push_back(Edge(i, j, 1));
        }
    }
    auto res_01 = warshall_floyd(elist_01, n);

    auto q = scanner.read<int>();
    for (UV : irange(q)) {
        auto [s, t] = scanner.read<Int1, 2>();
        auto ans = res_01.cost[s][t];
        print(ans == LINF ? -1 : ans - 1);
    }
}
}  // namespace bys
