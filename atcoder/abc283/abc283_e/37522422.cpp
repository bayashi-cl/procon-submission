...
void Solver::solve() {
    ...
    vector iso(h, vector<bool>(w, true));
    for (auto i : irange(h)) {
        for (auto j : irange(w)) {
    ...
    dp[0] = {{0, 0}, {1, 1}};
    for (auto i : irange(1, h + 1)) {
        for (auto prev : irange(2)) {
            for (auto now : irange(2)) {
                for (auto next : irange(2)) {
                    bool enable = true;
                    for (auto j : irange(w)) {
                        if (iso[i - 1][j]) {
                            if (i == 1) {
                                auto a_now = a[i - 1][j];
                                auto a_next = a[i][j];
                                if (now == 1) a_now = 1 - a_now;
                                if (next == 1) a_next = 1 - a_next;
                                ...
                                if (a_prev != a_now and a_now != a_next) {
                                    enable = false;
                                    break;
                                }
                            }
                    ...
                    if (enable) {
                        chmin(dp[i][next][now], dp[i - 1][now][prev] + next);
                    }
                }
            }
        }
