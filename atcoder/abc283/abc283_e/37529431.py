...
def main(**kwargs) -> None:
    ...
    def flip(i: int, j: int, f: bool) -> int:
        res = a[i][j]
        if f:
            res = 1 - res
        return res

    def iso(i: int, j: int, x: bool, y: bool, z: bool) -> bool:
        c = flip(i, j, y)
        l = -1
        r = -1
        u = -1
    ...
    dp = [[[IINF, IINF], [IINF, IINF]] for _ in range(h + 1)]
    dp[0] = [[0, 0], [1, 1]]
        ...
        for prv in range(2):
            for now in range(2):
                for nxt in range(2):
                    able = True
                    for j in range(w):
                        if iso(i - 1, j, prv == 1, now == 1, nxt == 1):
                            able = False
                            break
                    if able:
                        dp[i][nxt][now] = min(dp[i][nxt][now], dp[i - 1][now][prv] + nxt)
