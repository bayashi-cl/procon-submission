...
void main() {
    auto [h, w, n] = input<ll, 3>();
    map<Pa, int> cnt;
    for (int i = 0; i < n; i++) {
        auto [a, b] = input<int, 2>();
        for (int di = a - 1; di < a + 2; di++) {
            for (int dj = b - 1; dj < b + 2; dj++) {
                if (di < 2 || h - 1 < di) continue;
                if (dj < 2 || w - 1 < dj) continue;
                cnt[{di, dj}]++;
            }
        }
    }
    vector<ll> ans(10, 0);
    for (auto [k, v] : cnt) ans[v]++;
    ans[0] = (h - 2) * (w - 2) - (ll)(cnt.size());
