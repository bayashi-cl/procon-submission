...
void Solver::solve() {
    ...
    auto ok = [&](ll x) -> bool {
        ...
        ll cnt = 0;
        for (int i : irange(n)) {
            auto itr = std::lower_bound(a.begin(), a.end(), x - a[i]);
            cnt += a.end() - itr;
        }
        return cnt <= m;
    };
    auto th = meguru_bisect(LINF, -1LL, ok);
    ...
    cs.construct();
    for (int i : irange(n)) {
        auto itr = std::lower_bound(a.begin(), a.end(), th - a[i]);
        ll k = a.end() - itr;
        cnt += k;
        ans += cs.sum(itr - a.begin(), n) + a[i] * k;
    }
    ans += (th - 1) * (m - cnt);
    print(ans);
}
