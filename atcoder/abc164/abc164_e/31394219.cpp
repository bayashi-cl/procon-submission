...
void Solver::solve() {
    ...
    auto [n, m, s] = scanner.read<int, 3>();

    Grid grid(n, MX + 1);
    AdjacencyList graph(grid.area());
    for (UV : irange(m)) {
        auto [u, v] = scanner.read<Int1, 2>();
        auto [a, b] = scanner.read<int, 2>();
        for (int i : irange(a, MX + 1)) {
            graph.add_edge(grid.index(u, i), grid.index(v, i - a), b);
            graph.add_edge(grid.index(v, i), grid.index(u, i - a), b);
        }
    }
    for (int i : irange(n)) {
        auto [c, d] = scanner.read<int, 2>();
        for (int j : irange(MX)) {
            graph.add_edge(grid.index(i, j), grid.index(i, min(j + c, MX)), d);
        }
    }
    graph.build();
    auto res = dijkstra(graph, grid.index(0, min(s, MX)));

    for (auto t : irange(1, n)) {
        ll ans = LINF;
        for (int i : irange(MX + 1)) {
            chmin(ans, res.cost[grid.index(t, i)]);
        }
        print(ans);
    }
}
