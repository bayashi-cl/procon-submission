...
namespace bys {
void Solver::solve() {
    vector p = {0, 4, 5, 7, 9, 11, 13, 17, 19, 23};
    auto sp = p.size() - 1;
    auto p_cs = p;
    std::partial_sum(p_cs.begin(), p_cs.end(), p_cs.begin());
    vector<Pa> rng(sp);
    for (auto i : irange(sp)) {
        rng[i] = {p_cs[i], p_cs[i + 1]};
    }
    auto m = std::reduce(p.begin(), p.end());
    vector<int> a(m);
    std::iota(a.begin(), a.end(), 1);
    for (auto [l, r] : rng) {
        std::rotate(a.begin() + l, a.begin() + (l + 1), a.begin() + r);
    }
    ...
    auto b = scanner.readvec<i32_1>(m);
    ...
    for (auto i : irange(sp)) {
        ...
        rem.push_back(b[l] - l);
        mod.push_back(pi);
        if (not(0 <= b[l] - l and b[l] - l < pi)) throw RE;
    }

    auto [ans, z] = atcoder::crt(rem, mod);
    print.send(ans);
}
}  // namespace bys
