...
proc main =
  const
    p = [4, 5, 7, 9, 11, 13, 17, 19, 23]
  var
    prev = 0
    rng = newSeq[tuple[l, r: int]]()

  for pi in p:
    rng.add((prev, prev + pi))
    prev += pi
  let m = p.sum
  var a = (1..m).toSeq
  for r in rng: a.rotateLeft(r.l..<r.r, 1)

  print(m)
  print(a)
  stdout.flushFile()

  let b = newSeqWith(m, read(int) - 1)
  var
    rem = newSeq[int]()
    modulo = newSeq[int]()
