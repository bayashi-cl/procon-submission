...
void solve() {
    ...
    auto abx = input<array<int, 3>>(m);
    vector cnt_l(n + 2, vector<int>(n + 3));
    vector cnt_r(n + 2, vector<int>(n + 3));
    vector cnt(n + 2, vector<int>(n + 3));
    ...
    for (int i = 1; i < n + 2; ++i) {
        for (int j = 1; j < n + 3; ++j) {
            cnt_l[i][j] += cnt_l[i - 1][j];
            cnt_r[i][j] += cnt_r[i - 1][j - 1];
            cnt[i][j] = cnt_l[i][j] + cnt_r[i][j] + cnt[i][j - 1];
        }
    }
    int ans = 0;
    for (auto&& c : cnt) {
        ans += std::count_if(std::begin(c), std::end(c),
                             [](int x) { return (bool)x; });
    }
    print(ans);
}
