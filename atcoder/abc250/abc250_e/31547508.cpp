...
void Solver::solve() {
    ...
    vector<array<int, 3>> ua, ub;  // {val, l, r}
    vector<int> pos(n);
    set<int> sa, sb;
    for (int i : irange(n)) {
        ...
        if (sb.find(b[i]) == sb.end()) {
            sb.insert(b[i]);
            ub.push_back({b[i], i, i + 1});
        } else {
            ub.back()[2]++;
        }
    }
    int sz = min(ua.size(), ub.size());
    vector<bool> eq(sz);
    set<int> diff_a, diff_b;
    for (int i : irange(sz)) {
        auto va = ua[i][0];
        auto vb = ub[i][0];

        if (va != vb) {
            if (diff_b.find(va) == diff_b.end()) {
                diff_a.insert(va);
            } else {
                diff_b.erase(va);
            }
            if (diff_a.find(vb) == diff_a.end()) {
                diff_b.insert(vb);
            } else {
                diff_a.erase(vb);
            }
        }
        if (diff_a.empty() and diff_b.empty()) eq[i] = true;
    }
    DEBUG(pos);
    for (UV : irange(q)) {
