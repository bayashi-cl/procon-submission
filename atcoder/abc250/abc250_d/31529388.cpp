...
void Solver::solve() {
    ...
    Osa_K osa_k(MX + 1);
    vector<int> p_cnt(MX + 1);
    for (int i : irange(2, MX + 1)) {
        if (osa_k.is_prime(i)) p_cnt[i] = 1;
    }
    std::partial_sum(p_cnt.begin(), p_cnt.end(), p_cnt.begin());
    ll ans = 0;
    for (ll q = 1; q * q * q <= n; ++q) {
        if (not osa_k.is_prime(q)) continue;
        auto mp = min(n / (q * q * q), q - 1);
        ans += p_cnt[mp];
    }
    print(ans);
}
