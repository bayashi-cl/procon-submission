...
namespace bys {
void Solver::solve() {
    auto [h, w] = scanner.read<i32, 2>();
    auto a = scanner.readvec<i32>(h, w);

    Grid g(h, w);
    UnionFindTree uf(g.area());

    for (auto i : irange(h)) {
        for (auto j : irange(w)) {
            for (auto [ni, nj] : g.next2(i, j)) {
                if (a[i][j] == a[ni][nj]) {
                    uf.unite(g.index(i, j), g.index(ni, nj));
                }
            }
        }
    }
    ...
    i32 ans = 0;
    for (auto&& [l, member] : group) {
        set leader{l};
        map<i32, i32> cnt;
        for (auto m : member) {
            auto [i, j] = g.coord(m);
            for (auto [ni, nj] : g.next4(i, j)) {
                auto r = uf.find(g.index(ni, nj));
                if (leader.insert(r).second) {
                    cnt[a[ni][nj]] += uf.size(r);
                }
            }
        }
        auto s = uf.size(l);
