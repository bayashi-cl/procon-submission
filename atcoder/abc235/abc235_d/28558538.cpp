...
void Solver::solve() {
    ...
    while (!que.empty()) {
        int top = que.front();
        que.pop();
        if (top * a == n) EXIT(cost[top] + 1);
        if (top * a < MX && cost.find(top * a) == cost.end()) {
            cost[top * a] = cost[top] + 1;
            que.push(top * a);
        }
        if (top >= 10 && top % 10 != 0) {
            int nxt = ror(top);
            if (nxt == n) EXIT(cost[top] + 1);
            if (cost.find(nxt) == cost.end()) {
                cost[nxt] = cost[top] + 1;
                que.push(nxt);
            }
        }
