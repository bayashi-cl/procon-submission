...
#line 3 "/home/bayashi/dev/byslib/graph/kruskal.hpp"
...
#line 6 "/home/bayashi/dev/byslib/graph/kruskal.hpp"
...
void Solver::solve() {
    ...
    map<int, vector<pair<Edge, int>>> pos;  // n番目: [{Edge,id},...]
    for (int i : Range(q)) {
        auto [u, v, w] = scanner.read<Int1, Int1, int>();
        auto itr = std::lower_bound(cost.begin(), cost.end(), w);
        pos[std::distance(cost.begin(), itr)].push_back({Edge(u, v, w), i});
    }
    atcoder::dsu uft(n);
    vector<string> ans(q);
    for (int i : Range(m)) {
        for (auto&& [e, id] : pos[i]) {
            ans[id] = uft.same(e.from, e.to) ? "No" : "Yes";
        }
        auto ee = g[i];
        uft.merge(ee.from, ee.to);
    }
    for (auto&& [e, id] : pos[m]) {
        ans[id] = uft.same(e.from, e.to) ? "No" : "Yes";
    }
