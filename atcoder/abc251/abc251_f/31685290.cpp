...
void Solver::solve() {
    ...
    vector<bool> seen(n);
    FixPoint dfs1([&](auto&& self, int now) -> void {
        seen[now] = true;
        for (auto&& e : graph[now]) {
            if (seen[e.dest]) continue;
            t1.emplace_back(now + 1, e.dest + 1);
            self(e.dest);
        }
    });
    dfs1(0);
    assert((int)t1.size() == n - 1);
    for (auto&& e : t1) print(e);
    ...
    while (not que.empty()) {
        ...
        que.pop();
        for (auto&& e : graph[now]) {
            if (uft.unite(now, e.dest)) {
                t2.emplace_back(now + 1, e.dest + 1);
                que.push(e.dest);
            }
        }
    }
    assert((int)t2.size() == n - 1);
    for (auto&& e : t2) print(e);
}
}  // namespace bys
