...
void main() {
    int n, h;
    cin >> n >> h;
    vector<int> a(n);  // slash
    vector<int> b(n);  // throw
    for (auto&& i : irange(n)) cin >> a[i] >> b[i];

    int ma = *std::max_element(std::begin(a), std::end(a));
    sort(std::begin(b), std::end(b), std::greater<int>());
    int ans = 0;
    for (int bi : b) {
        if (bi >= ma) {
            ans++;
