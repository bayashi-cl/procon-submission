...
void Solver::solve() {
    ...
    Grid g(h, w);
    unsigned long cnt = 0;
    for (auto&& si : s) cnt += std::count(si.begin(), si.end(), '#');
    vector<int> snake, ans;
    FixPoint dfs([&](auto&& self, int i, int j, int seen) -> void {
        int idx = g.index(i, j);
        if (pop(seen, idx)) return;
        seen |= 1 << idx;
        snake.push_back(idx);
        if (snake.size() == cnt) ans = snake;
