...
void Solver::solve() {
    ...
    auto [r, c] = scanner.read<Int1, 2>();
    auto s = scanner.readvec<string>(h);
    Grid g(h, w);
    AdjacencyList graph(g.area());
    for (int i : irange(h)) {
        for (int j : irange(w)) {
            if (s[i][j] == '#') continue;
            if (g.contain(i + 1, j) and (s[i + 1][j] == '.' or s[i + 1][j] == '^')) {
                graph.add_edge(g.index(i, j), g.index(i + 1, j));
            }
            if (g.contain(i - 1, j) and (s[i - 1][j] == '.' or s[i - 1][j] == 'v')) {
                graph.add_edge(g.index(i, j), g.index(i - 1, j));
            }
            if (g.contain(i, j + 1) and (s[i][j + 1] == '.' or s[i][j + 1] == '<')) {
                graph.add_edge(g.index(i, j), g.index(i, j + 1));
            }
            if (g.contain(i, j - 1) and (s[i][j - 1] == '.' or s[i][j - 1] == '>')) {
                graph.add_edge(g.index(i, j), g.index(i, j - 1));
            }
    ...
    graph.build();
    auto res = reachable(graph, g.index(r, c));
    for (int i : irange(h)) {
        string ans;
        for (int j : irange(w)) {
            if (s[i][j] == '#') {
                ...
                ans.push_back(res[g.index(i, j)] ? 'o' : 'x');
            }
        }
        print(ans);
    }
