...
void Solver::solve() {
    ...
    auto t = scanner.readvec<string>(h);
    int hs = h, ws = w;
    Grid grid(h, w);
    vector base(h, vector<bool>(w)), stamp(h, vector<bool>(w));
    // int st_cnt = 0;
    for (int i : irange(h)) {
        for (int j : irange(w)) {
            if (s[i][j] == '#') base[i][j] = true;
            if (t[i][j] == '#') stamp[i][j] = true;
        }
    }
    for (UV : irange(4)) {
        for (int ib : irange(-25, 25)) {
            for (int jb : irange(-25, 25)) {
                bool flg = true;
                for (int is : irange(hs)) {
                    for (int js : irange(ws)) {
                        if (stamp[is][js]) {
                            if (not grid.contain(ib + is, jb + js) or base[ib + is][jb + js]) flg = false;
                        }
                    }
                }
                if (flg) EXIT("Yes");
        ...
        vector stamp_rot(ws, vector<bool>(hs));
        for (int i : irange(hs)) {
            for (int j : irange(ws)) {
                stamp_rot[j][hs - i - 1] = stamp[i][j];
            }
        }
        stamp = stamp_rot;
        std::swap(hs, ws);
