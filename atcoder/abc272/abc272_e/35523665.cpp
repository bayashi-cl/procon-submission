...
int mex(Iterable const& v) {
    static_assert(std::is_integral_v<typename Iterable::value_type>, "value_type is not integral.");

    int sz = v.size();
    std::vector<bool> exist(sz + 1);
    for (auto&& e : v) {
        if (e <= sz) exist[e] = true;
    }
    auto i = 0;
    for (; i <= sz; ++i) {
        if (not exist[i]) break;
    }
    return i;
}
