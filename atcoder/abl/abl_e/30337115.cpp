...
namespace bys {
struct DigSum : Magma<pair<ll, ll>> {
    using Magma<pair<ll, ll>>::set_type;
    static inline vector<ll> tens = {1};
    static set_type operation(set_type a, set_type b) {
        return {(a.first * tens[b.second] + b.first) % MOD, a.second + b.second};
    }
    static constexpr set_type identity{0, 0};
};

template <>
struct Mapping<DigSum, Update<int>> {
    static inline vector<ll> ones = {0};
    static constexpr void mapping(DigSum::set_type& t, Update<int>::set_type s, int w) {
        if (s.has_value()) t.first = ones[w] * s.value();
    }
};

void Solver::solve() {
    using Mp = Mapping<DigSum, Update<int>>;
    auto [n, q] = scanner.read<int, 2>();
    for (UV : irange(n)) {
        DigSum::tens.push_back(DigSum::tens.back() * 10 % MOD);
        Mp::ones.push_back((Mp::ones.back() * 10 + 1) % MOD);
    }
    vector<pair<ll, ll>> init(n, {1, 1});
    LazySegmentTree<DigSum, Update<int>> seg(init);
    for (UV : irange(q)) {
        auto [l, r, d] = scanner.read<Int1, int, int>();
        seg.apply(l, r, d);
        print(seg.query_all().first);
    }
}
}  // namespace bys
