...
void solve() {
    ...
    auto pos = input<pair<Pa, Pa>>(1000);
    ...
    while (std::clock() - ti < 190 * CLOCKS_PER_SEC / 100) {
        Vec choice;
        std::sample(idx.begin(), idx.end(), std::back_inserter(choice), 50,
                    engine);
        vector<Pa> root;
        root.push_back({400, 400});
        for (auto&& ci : choice) {
            root.push_back(pos[ci].first);
            root.push_back(pos[ci].second);
        }
        root.push_back({400, 400});
    ...
    for (int i = 0; i < 50; ++i) ans_choice[i]++;
    print(ans_choice.size(), ans_choice);
    print(ans_root.size(), ans_root);
    DEBUG(judge_score(ans_root));
}
}  // namespace bys
