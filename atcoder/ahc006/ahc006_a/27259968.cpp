...
int calc_score(vector<Pos>& root) {
    ...
    for (int i = 0; i < n - 1; ++i) {
        if (root[i].second.second == 0) {
            ...
            if (recieve.find(root[i].second.first) == recieve.end()) return INF;
        }
...
void solve() {
    ...
    while (std::clock() - ti < 195 * CLOCKS_PER_SEC / 100) {
        ...
        int r = distr(eng);
        if (l == r) continue;
        // auto [id_l, type_l] = root[l].second;
        // auto [id_r, type_r] = root[r].second;
        // if (id_l == id_r) continue;
        // // l
        // if (type_l == 0) {
        //     if (hand[id_l] < r) continue;
        // } else {
        //     if (r < recive[id_l]) continue;
        // }
        // // r
        // if (type_r == 0) {
        //     if (hand[id_r] < l) continue;
        // } else {
        //     if (l < recive[id_r]) continue;
        // }
        std::swap(root[l], root[r]);
        if (!chmin(score, calc_score(root))) std::swap(root[l], root[r]);
        // if (chmin(score, calc_score(root))) {
        //     // l
        //     if (type_l == 0) {
        //         recive[id_l] = r;
        //     } else {
