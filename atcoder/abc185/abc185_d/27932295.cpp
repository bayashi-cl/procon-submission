...
void Solver::solve() {
    ...
    vector<int> white;
    for (int i : Range(m + 1)) {
        if (int l = a[i + 1] - a[i] - 1; l != 0) white.push_back(l);
    }
    if (white.empty()) EXIT(0);
    int w = *std::min_element(white.begin(), white.end());
    int ans = 0;
    for (auto&& wi : white) ans += iceil(wi, w);
    print(ans);
}
}  // namespace bys
