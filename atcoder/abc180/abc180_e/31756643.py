...
def main(**kwargs) -> None:
    n = int(readline())
    pos = [tuple(map(int, readline().split())) for _ in range(n)]

    dp = [[IINF] * n for _ in range(2**n)]
    dp[1][0] = 0
    ...
    for s in range(1, 2**n):
        for f in range(n):
            ...
            for t in range(n):
                if not t & (1 << t):
                    nxt = s | (1 << t)
                    dp[nxt][t] = min(dp[nxt][t], dp[s][f] + dist(pos[f], pos[t]))

    ans = IINF
    for f in range(n):
        ans = min(ans, dp[-1][f] + dist(pos[f], pos[0]))
