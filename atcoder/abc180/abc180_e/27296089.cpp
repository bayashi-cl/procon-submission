...
#line 2 "abc180_e/main.cpp"

namespace bys {
struct City {
    int x, y, z;
    City(int x_, int y_, int z_) : x(x_), y(y_), z(z_) {}
};
...
void solve() {
    ...
    for (int s = 1; s < (1 << n); ++s) {
        for (int d = 0; d < n; ++d) {
            if (dp[s][d] == INF) continue;
            for (int to = 0; to < n; ++to) {
                if (pop(s, to)) continue;
                int t = s | (1 << to);
                chmin(dp[t][to], dp[s][d] + dist(pos[d], pos[to]));
            }
        }
    }
    int ans = INF;
    for (int i = 0; i < n; ++i) {
        chmin(ans, dp.back()[i] + dist(pos[i], pos[0]));
    }
    print(ans);
}
}  // namespace bys
