...
void Solver::solve() {
    ...
    vector dp(n + 1, vector<Pa>(m + 1, {INF, INF}));  // {(取ってない), (取った)}
    dp[0][0].second = 0;
    ...
    for (int i : irange(n)) {
        for (int j : irange(m + 1)) {
            ...
            auto nxt = j + a[i];
            if (nxt <= m) {
                chmin(dp[i + 1][nxt].second, dp[i][j].first);
                chmin(dp[i + 1][nxt].second, dp[i][j].second);
            }
            // 取らない
            chmin(dp[i + 1][j].first, dp[i][j].first);
            chmin(dp[i + 1][j].first, dp[i][j].second + 1);
        }
    }
    for (auto mi : irange(1, m + 1)) {
        auto ans = min(dp[n][mi].first, dp[n][mi].second);
        if (ans == INF) ans = -1;
        print(ans);
    }
}
