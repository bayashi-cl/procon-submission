...
void Solver::solve() {
    ...
    int n = s.length();
    vector<int> cnt(26);
    for (auto si : s) cnt[si - 'a']++;

    MultiComb mc(n);

    vector dp(27, vector<mint>(n + 1));
    dp[0][0] = 1;
    for (int i : irange(26)) {
        for (int j : irange(n + 1)) {
            for (auto k : irange(cnt[i] + 1)) {
                if (j + k <= n) {
                    dp[i + 1][j + k] += dp[i][j] * mc.comb(j + k, k);
                }
            }
        }
    }
    print(std::accumulate(dp.back().begin(), dp.back().end(), mint(0)) - 1);
}
}  // namespace bys
