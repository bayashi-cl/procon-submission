...
#line 4 "abc157_e/main.cpp"

namespace bys {
uint cn(char c) { return 1 << (c - 'a'); }

uint op(uint a, uint b) { return a | b; }
uint e() { return 0; }

void solve() {
    auto [n, s, q] = input<int, string, int>();
    vector<uint> v(n);
    for (int i = 0; i < n; ++i) v[i] = cn(s[i]);

    atcoder::segtree<uint, op, e> seg(v);

    for (int qi = 0; qi < q; ++qi) {
        auto com = input<int>();
        if (com == 1) {
            auto [i, c] = input<int, char>();
            seg.set(i - 1, cn(c));
        } else {
            auto [l, r] = input<int, 2>();
            uint uni = seg.prod(l - 1, r);
            print(__builtin_popcount(uni));
        }
    }
}
}  // namespace bys
