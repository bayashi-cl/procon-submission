...
#line 5 "abc128_e/main.cpp"

namespace bys {
void Solver::solve() {
    auto [n, q] = scanner.read<int, 2>();
    auto w = scanner.readvec<array<int, 3>>(n);
    sort(w.begin(), w.end(), ItemSorter<array<int, 3>, 2>());
    set<Pa> d;
    for (int i : irange(q)) {
        auto di = scanner.read<int>();
        d.insert({di, i});
    }
    vector<int> ans(q, -1);
    for (auto [s, t, x] : w) {
        int l = s - x;
        int r = t - x;
        auto itr_l = d.lower_bound({l, -1});
        auto itr_r = d.lower_bound({r, -1});
        for (auto p = itr_l; p != itr_r; ++p) {
            auto [di, idx] = *p;
            ans[idx] = x;
        }
        d.erase(itr_l, itr_r);
