...
#line 2 "arc129_a/main.cpp"

namespace bys {
int bit_length(ll x) { return sizeof(ll) * 8 - __builtin_clzll(x); }
int bit_length(int x) { return sizeof(int) * 8 - __builtin_clz(x); }

void solve() {
    auto [n, l, r] = input<ll, 3>();
    int b = bit_length(n);
    ll ans = 0;
    for (int d = 0; d < b; ++d) {
        if (!pop(n, d)) continue;
        ll mi = max(1LL << d, l);
        ll ma = min(1LL << (d + 1), r + 1);
        ans += max(0LL, ma - mi);
    }
    print(ans);
}
