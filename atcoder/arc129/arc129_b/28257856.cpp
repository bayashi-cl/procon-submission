...
void Solver::solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        ri += ans;
        if (ri < l || r < li) {
            int diff = min(abs(l - ri), abs(li - r));
            int d = ceildiv(diff, 2);
            ans += d;
            l -= d;
            r += d;
            li -= d;
    ...
    //     }
    //     cp.construct();
    //     vector<int> imos(cp.size());
    //     for (auto&& [l, r] : lr) {
    //         imos[cp.get(l - x)]++;
    //         imos[cp.get(r + x + 1)]--;
    //     }
    //     for (int i : Range((int)imos.size() - 1)) imos[i + 1] += imos[i];
    //     return *std::max_element(imos.begin(), imos.end()) == n;
    // };
    // print(meguru_bisect(MOD7, -1, ok));
}
}  // namespace bys
