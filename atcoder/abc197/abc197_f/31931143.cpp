...
void Solver::solve() {
    ...
    vector<map<char, vector<int>>> graph(n);
    for (UV : irange(m)) {
        auto [a, b, c] = scanner.read<Int1, Int1, char>();
    ...
    while (not que.empty()) {
        auto [i, j] = que.front();
        if (i == j) chmin(ans, cost[i][j] * 2);
        que.pop();
        for (char c = 'a'; c <= 'z'; ++c) {
            for (auto p : graph[i][c]) {
                if (p == j) chmin(ans, cost[i][j] * 2 + 1);
                for (auto q : graph[j][c]) {
                    if (cost[p][q] == INF) {
                        cost[p][q] = cost[i][j] + 1;
                        que.push({p, q});
                    }
