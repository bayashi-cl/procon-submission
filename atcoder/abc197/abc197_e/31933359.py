...
def main(**kwargs) -> None:
    n = int(readline())
    lr: Dict[int, Tuple[int, int]] = dict()
    for _ in range(n):
        a, c = map(int, readline().split())
        l, r = lr.get(c, (a, a))
        lr[c] = (min(l, a), max(r, a))

    now = (0, 0)
    dp = (0, 0)
    for ci in sorted(lr.keys()):
        l, r = lr[ci]
        nxt_l = min(dp[0] + abs(now[0] - r) + r - l, dp[1] + abs(now[1] - r) + r - l)
        nxt_r = min(dp[0] + abs(now[0] - l) + r - l, dp[1] + abs(now[1] - l) + r - l)
        dp = (nxt_l, nxt_r)
        now = (l, r)

    print(min(dp[0] + abs(now[0]), dp[1] + abs(now[1])))
