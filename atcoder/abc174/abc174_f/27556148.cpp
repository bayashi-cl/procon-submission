...
#line 4 "abc174_f/main.cpp"

namespace bys {
void Solver::solve() {
    using Que = pair<Pa, int>;
    ...
    sort(que.begin(), que.end(),
         [](Que& a, Que& b) { return a.first.second < b.first.second; });

    vector<int> ans(q);
    atcoder::fenwick_tree<int> bit(n);
    // vector<int> pos(*std::max_element(c.begin(), c.end()) + 1, -1);
    umap<int, int> pos;
    int i = 0;
    for (auto& [lr, id] : que) {
        auto& [l, r] = lr;
        for (; i < r; ++i) {
            bit.add(i, 1);
            // if (pos[c[i]] != -1) bit.add(pos[c[i]], -1);
            if (pos.find(c[i]) != pos.end()) bit.add(pos[c[i]], -1);
            pos[c[i]] = i;
        }
        ans[id] = bit.sum(l, r);
    }
