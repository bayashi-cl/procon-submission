...
void solve() {
    ...
    std::partial_sum(std::begin(red), std::end(red), red.begin());
    std::partial_sum(std::begin(white), std::end(white), white.begin());
    int ans = INF;
    for (int th = 0; th <= n; ++th) {
        int man = max(white[th] - white[0], red[n] - red[th]);
        chmin(ans, man);
    }
    print(ans);
}
