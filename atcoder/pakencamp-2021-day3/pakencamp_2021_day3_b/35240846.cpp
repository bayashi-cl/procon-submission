...
namespace bys {
void Solver::solve() {
    auto [n, x, y] = scanner.read<int, ll, ll>();
    auto a = scanner.readvec<int>(n);
    if (std::accumulate(a.begin(), a.end(), 0LL) != x + y) EXIT("No");

    vector<ll> aa(n * 2);
    for (int i : irange(n)) aa[i] = aa[i + n] = a[i];

    auto bit = fenwick_tree(aa);

    for (int i : irange(n)) {
        auto r = bit.bisect(x + bit.prefix_fold(i));
        if (r == -1) continue;
        if (bit.fold(i, r) == x) EXIT("Yes");
    }
    print("No");
}
}  // namespace bys
