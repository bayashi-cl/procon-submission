...
void Solver::solve() {
    constexpr int n = 30;
    auto ts = scanner.readvec<string>(n);
    vector t_(n, vector<int>(n));
    for (int i : irange(n)) {
        for (int j : irange(n)) {
            t_[i][j] = ts[i][j] - '0';
        }
    ...
    auto r = [](int from, int to) -> int {
        assert(0 <= from and from < 6);
        assert(0 <= to and to < 4);
        if (from >= 5) {
            return (from + to) % 2;
        } else {
            return (to - from + 4) % 4;
    ...
    vector rot(n, vector<int>(n));
    auto calc = [&](int si, int sj, int gi, int gj) -> int {  // semi-open
        int max_area = 0;
        for (int i : irange(si, gi)) {
            for (int j : irange(sj, gj)) {
                for (int k : irange(i + 3, gi)) {
                    for (int l : irange(j + 3, gj)) {  // close
                        if ((k - i) % 2 == 0 or (l - j) % 2 == 0) continue;
                        // -----------------------------------------------------
                        bool flg = true;
                        {  // 1, 2, 5, 6
                            vector<int> ii = {i, k, i + 1, k - 1};
                            for (int jj : irange(j + 1, l)) {
                                for (int ti : irange(4)) {
                                    if (t[ii[ti]][jj] >= 6) flg = false;
                                }
                            }
                        }
                        {  // 3, 4, 7, 8
                            vector<int> jj = {j, l, j + 1, l - 1};
                            for (int ii : irange(i + 1, k)) {
                                for (int ti : irange(4)) {
                                    if (t[ii][jj[ti]] >= 6) flg = false;
                                }
                            }
                        }
                        if (not flg) continue;

                        int area = (k - i - 1) * 4 + (l - j - 1) * 4 - 4;
                        if (not chmax(max_area, area)) continue;

                        // -----------------------------------------------------
                        {  // 1, 2, 5, 6
                            vector<int> tile = {2, 3, 0, 1};
                            vector<int> ii = {i, k, i + 1, k - 1};
                            for (int jj : irange(j + 1, l)) {
                                for (int ti : irange(4)) {
                                    if (t[ii[ti]][jj] >= 6) flg = false;
                                    rot[ii[ti]][jj] = r(t[ii[ti]][jj], tile[ti]);
                                    tile[ti] = 3 - tile[ti];
                                }
                            }
                        }
                        {  // 3, 4, 7, 8
                            vector<int> tile = {2, 1, 0, 3};
                            vector<int> jj = {j, l, j + 1, l - 1};
                            for (int ii : irange(i + 1, k)) {
                                for (int ti : irange(4)) {
                                    if (t[ii][jj[ti]] >= 6) flg = false;
                                    rot[ii][jj[ti]] = r(t[ii][jj[ti]], tile[ti]);
                                    tile[ti] += tile[ti] & 1 ? -1 : 1;
                                }
                            }
                        }
                    }
    ...
    vector<vector<int>> cand = {
        {0, 0, 30, 15, 0, 15, 30, 30},
        {0, 0, 15, 30, 15, 0, 30, 30},
    };

    for (auto&& c : cand) {
        int ds = calc(c[0], c[1], c[2], c[3]) + calc(c[4], c[5], c[6], c[7]);
        if (chmax(score, ds)) {
            int pos = 0;
            for (int i : irange(n)) {
                for (int j : irange(n)) {
