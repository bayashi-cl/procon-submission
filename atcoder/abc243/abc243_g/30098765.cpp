...
void Solver::solve() {
    ...
    ll sqx = isqrt(x);
    vector<ll> dp(isqrt(sqx) + 1, 0);
    for (auto i : irange(1UL, dp.size())) {
        dp[i] = sqx - (i * i) + 1 + dp[i - 1];
    }
    while (dp.size() > 2UL) {
        vector<ll> dp2(isqrt(dp.size()) + 1);
        for (int i : irange(1UL, dp2.size())) {
            dp2[i] = dp.back() - dp[i * i - 1] + dp2[i - 1];
        }
        dp = dp2;
    }
    print(dp[1]);
