...
namespace bys {
auto warshall_floyd(const std::vector<Edge>& graph, std::size_t n_node) {
    vector res(n_node, vector<pair<ll, int>>(n_node, {LINF, 0}));
    for (auto&& e : graph) res[e.src][e.dest] = {e.weight, 1};
    for (std::size_t i = 0; i < n_node; ++i) res[i][i] = {0, 0};
    for (std::size_t k = 0; k < n_node; k++) {
        for (std::size_t i = 0; i < n_node; i++) {
            for (std::size_t j = 0; j < n_node; j++) {
                if (res[i][k].first == LINF || res[k][j].first == LINF) continue;
                if (res[i][j].first > res[i][k].first + res[k][j].first) {
                    res[i][j] = {res[i][k].first + res[k][j].first, 1};
                } else if (res[i][j].first == res[i][k].first + res[k][j].first) {
                    res[i][j].second++;
                }
            }
        }
...
void Solver::solve() {
    ...
    for (auto [a, b, c] : abc) {
        ...
        elist.push_back(Edge(b, a, c));
    }
    auto res = warshall_floyd(elist, n);
    int ans = 0;
    for (auto [a, b, c] : abc) {
        if (res[a][b].first < c) {
            ++ans;
        } else {
            if (res[a][b].second >= 4) ++ans;
        }
    }
    print(ans);
}
