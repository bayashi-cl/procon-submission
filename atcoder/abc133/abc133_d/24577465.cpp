...
namespace bys {
ll calc(int x, vector<ll>& memo, vector<ll>& a) {
    if (memo[x] != -1) return memo[x];
    ll res = 2 * a[x] - calc(x - 1, memo, a);
    memo[x] = res;
    return res;
}
void solve() {
    auto n = input<int>();
    auto a = input<ll>(n);
    ll sm = std::accumulate(std::begin(a), std::end(a), 0LL);
    ll d0 = sm;
    for (int i = 2; i < n; i += 2) d0 -= 2 * a[i];
    vector<ll> memo(n, -1);
    vector<ll> ans;
    memo[0] = d0;
    ans.push_back(calc(n - 1, memo, a));
    for (int i = 0; i < n - 1; ++i) {
        ans.push_back(calc(i, memo, a));
    }
    print(ans);
}
}  // namespace bys
