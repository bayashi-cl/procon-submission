...
#line 2 "abc182_e/main.cpp"

namespace bys {
void solve() {
    auto [h, w, n, m] = input<int, 4>();
    set<Pa> light;
    VecVec board(h + 1, Vec(w + 1));  // -1:ブロック, 0: 暗, 1: 明
    for (int i = 0; i < n; ++i) {
    ...
    for (int i = 0; i < h + 1; ++i) board[i][w] = -1;
    for (int j = 0; j < w + 1; ++j) board[h][j] = -1;
    ...
    for (int i = 0; i < h + 1; ++i) {
        ...
        for (int j = 0; j < w + 1; ++j) {
            if (board[i][j] != -1) {
                if (from == -1) from = j;
                if (light.find({i, j}) != light.end()) flg = true;
            } else {
                if (flg) {
                    for (int k = from; k < j; ++k) board[i][k] = 1;
                }
                from = -1;
                flg = false;
            }
    ...
    for (int j = 0; j < w + 1; ++j) {
        ...
        for (int i = 0; i < h + 1; ++i) {
            if (board[i][j] != -1) {
                ...
                if (flg) {
                    for (int k = from; k < i; ++k) board[k][j] = 1;
                }
                from = -1;
                flg = false;
            }
