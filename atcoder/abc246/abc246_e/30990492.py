...
def main(**kwargs) -> None:
    n = int(readline())
    nn = n * n
    ai, aj = map(int1, readline().split())
    bi, bj = map(int1, readline().split())
    s = [sinput() for _ in range(n)]
    delta = [(-1, -1), (-1, 1), (1, 1), (1, -1)]

    source = [nn * d + ai * n + aj for d in range(4)]
    dest = [nn * d + bi * n + bj for d in range(4)]
    cost = [IINF] * (nn * 4)
    que = deque(source)
    for si in source:
        cost[si] = 0
    ...
    while que:
        ...
        for e in range(4):
            ...
            idx = nn * e + (i * n + j)
            if (d + e) & 1:
                if top_cost + 1 < cost[idx]:
                    cost[idx] = top_cost + 1
                    que.append(idx)
            else:
                if top_cost < cost[idx]:
                    cost[idx] = top_cost
                    que.appendleft(idx)

        if 0 <= ni < n and 0 <= nj < n and s[ni][nj] == ".":
            idx = nn * ((d + 2) % 4) + (ni * n + nj)
            if top_cost < cost[idx]:
                cost[idx] = top_cost
                que.appendleft(idx)

    ans = min(cost[di] for di in dest)
    print(ans + 1 if ans != IINF else -1)
