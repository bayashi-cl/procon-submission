...
void Solver::solve() {
    ...
    auto [ai, aj] = scanner.read<Int1, 2>();
    auto [bi, bj] = scanner.read<Int1, 2>();
    auto s = scanner.readvec<string>(n);

    Grid g(n, n);
    UnionFindTree up(g.area()), down(g.area());
    int pawn = 0;
    for (auto&& si : s) pawn += std::count(si.begin(), si.end(), '#');

    for (int i : irange(n)) {
        for (int j : irange(n)) {
            if (s[i][j] == '#') continue;
            if (g.contain(i - 1, j + 1) and s[i - 1][j + 1] == '.') up.unite(g.index(i, j), g.index(i - 1, j + 1));
            if (g.contain(i + 1, j + 1) and s[i + 1][j + 1] == '.') down.unite(g.index(i, j), g.index(i + 1, j + 1));
        }
    }

    int v = n * n + up.n_tree() + down.n_tree();
    AdjacencyList graph(v);

    int ex = n * n;
    for (auto [leader, gup] : up.groups()) {
        for (auto idx : gup) {
            graph.add_edge(idx, ex, 0);
            graph.add_edge(ex, idx, 1);
        }
        ex++;
    }
    for (auto [leader, gdown] : down.groups()) {
        for (auto idx : gdown) {
            graph.add_edge(idx, ex, 0);
            graph.add_edge(ex, idx, 1);
        }
    ...
    graph.build();
    auto res = zero_one_bfs(graph, g.index(ai, aj));
    auto ans = res.cost[g.index(bi, bj)];
