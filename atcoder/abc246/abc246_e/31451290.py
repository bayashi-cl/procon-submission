...
def main(**kwargs) -> None:
    ...
    nn = n * n
    g = Grid(n, n)
    a = g.index(*map(int1, readline().split()))
    b = g.index(*map(int1, readline().split()))
    s = [sinput() for _ in range(n)]
    nn = g.area()
    graph = LilMatrix.empty(g.area() * 2)
    for i in range(n):
        for j in range(n):
            now = g.index(i, j)
            if s[i][j] == "#":
                continue
            if (i - 1, j + 1) in g and s[i - 1][j + 1] == ".":
                nxt = g.index(i - 1, j + 1)
                graph.add_edge(now, nxt, 0)
                graph.add_edge(nxt, now, 0)
            if (i + 1, j + 1) in g and s[i + 1][j + 1] == ".":
                nxt = g.index(i + 1, j + 1)
                graph.add_edge(now + nn, nxt + nn, 0)
                graph.add_edge(nxt + nn, now + nn, 0)
            graph.add_edge(now, now + nn, 1)
            graph.add_edge(now + nn, now, 1)

    cost, _ = bfs(graph, [a, a + nn])
    ans = min(cost[b], cost[b + nn])
    print(-1 if ans == IINF else ans + 1)
