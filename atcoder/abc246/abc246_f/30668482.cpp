...
namespace bys {
void Solver::solve() {
    auto [n, l] = scanner.read<int, 2>();
    auto str = scanner.readvec<string>(n);
    vector<int> s(n);
    for (auto i : irange(n)) {
        for (auto c : str[i]) s[i] |= (1 << (c - 'a'));
    }
    vector<int> cnt(27);
    for (int i : irange(1 << 26)) {
        bool flg = false;
        for (auto si : s) {
            if ((si & i) == i) flg = true;
        }
        if (flg) cnt[__builtin_popcount(i)]++;
    }
    Mint ans = 0;
    MultiComb mc(26, MOD);
    vector<Mint> rem(27);
    for (int k : irange(1, 27)) {
        if (k > l) break;
        Mint ks = Mint(k).pow(l);
        for (int j : irange(1, k)) ks -= rem[j] * mc.comb(k, j);
        rem[k] = ks;
        ans += ks * cnt[k];
    }
    print(ans);
