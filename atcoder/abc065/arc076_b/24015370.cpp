...
void main() {
    ...
    vector<Edge> graph;
    sort(std::begin(pos), std::end(pos), ItemGetter<Vec, 0>());
    for (auto&& i : irange(n - 1)) {
        Edge e = {abs(pos[i][0] - pos[i + 1][0]), {pos[i][2], pos[i + 1][2]}};
        graph.push_back(e);
    }
    sort(std::begin(pos), std::end(pos), ItemGetter<Vec, 1>());
    for (auto&& i : irange(n - 1)) {
        Edge e = {abs(pos[i][1] - pos[i + 1][1]), {pos[i][2], pos[i + 1][2]}};
        graph.push_back(e);
    }
    ...
    for (auto [cost, node] : graph) {
        // debug(cost, node);
        if (uft.same(node.first, node.second)) continue;
        uft.merge(node.first, node.second);
        ans += cost;
    }
