...
#line 3 "abc179_e/main.cpp"

namespace bys {
void solve() {
    auto [n, x, m] = input<ll, 3>();
    map<ll, ll> pos;
    vector<ll> arr;
    pos[x] = 0;
    arr.push_back(x);
    ...
    DEBUG("case_2");
    ll loop_sum = cs.sum(enter, sz);
    ll n_loop = (n - enter) / loop;
    ll extra = (n - enter) % loop;
    print(cs.sum(0, enter + extra) + n_loop * loop_sum);
}
}  // namespace bys
